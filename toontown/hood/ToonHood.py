...
from . import Hood
from toontown.building import SuitInterior, FlunkyBuildingInterior
...
class ToonHood(Hood.Hood):
    ...
    def __init__(self, parentFSM, doneEvent, dnaStore, hoodId):
        ...
        self.fsm = ClassicFSM.ClassicFSM('Hood', [State.State('start', self.enterStart, self.exitStart, ['townLoader', 'safeZoneLoader']),
         ...
         State.State('flunkyBuildingInterior', self.enterFlunkyBuildingInterior, self.exitFlunkyBuildingInterior, ['quietZone', 'townLoader', 'safeZoneLoader']),
    ...
    def enterFlunkyBuildingInterior(self, requestStatus = None):
        self.placeDoneEvent = 'flunky-interior-done'
        self.acceptOnce(self.placeDoneEvent, self.handleFlunkyBuildingInteriorDone)
        self.place = SuitInterior.SuitInterior(self, self.fsm, self.placeDoneEvent)
        self.place.load()
        self.place.enter(requestStatus)
        base.cr.playGame.setPlace(self.place)

    def exitFlunkyBuildingInterior(self):
        self.ignore(self.placeDoneEvent)
        del self.placeDoneEvent
        self.place.exit()
        self.place.unload()
    ...
    def handleFlunkyBuildingInteriorDone(self):
        doneStatus = self.place.getDoneStatus()
        if self.isSameHood(doneStatus):
            self.fsm.request('quietZone', [doneStatus])
        else:
