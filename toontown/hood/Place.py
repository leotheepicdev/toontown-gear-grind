...
class Place(StateData.StateData, FriendsListManager.FriendsListManager):
    ...
    def handleTeleportQuery(self, response, requesterId):
        base.localAvatar.respondToRequestedTeleport(response, base.localAvatar.defaultShard, base.cr.playGame.getPlaceId(), self.getZoneId(), requesterId)
    ...
    def exitStickerBook(self):
        ...
        if hasattr(base.localAvatar, 'mapPage'):
            if base.localAvatar.mapPage.dialog:
                base.localAvatar.mapPage.dialog.cleanup()
                base.localAvatar.mapPage.dialog = None
        if base.summonDialog:
            base.summonDialog.unload()
    ...
    def _placeTeleportInPostZoneComplete(self, requestStatus):
        ...
        if avId != -1:
            if avId in base.cr.doId2do:
                ...
                base.localAvatar.d_teleportGreeting()
            else:
                ...
                if friend != None:
                    ...
                    base.localAvatar.sendUpdate('requestTeleportGiveup', [])
                    if requestStatus['where'] == 'street':
                        if hasattr(self, '_teleportToFriend'):
                            self._teleportToFriend(requestStatus)
                            return
    ...
    def enterStopped(self, disableEmotes=1):
        base.localAvatar.b_setAnimState('neutral', 1)
        if disableEmotes:
