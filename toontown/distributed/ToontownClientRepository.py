...
from toontown.toonbase.ToontownDiscordPresence import ToontownDiscordPresence

def prepareAvatar(http):
    from toontown.toonbase import GearGrindGlobals

    http.setClientCertificatePem(GearGrindGlobals.getProdCertificate())
...
class ToontownClientRepository(OTPClientRepository.OTPClientRepository):
    ...
    def enterChooseAvatar(self, avList):
        if self.loginScreen:
            self.loginScreen.fsm.request('off')
        if Settings['wantDisclaimer']:
            self.showDisclaimer()
    ...
    def showDisclaimer(self):
        self.disclaimer = TTDialog.TTDialog(text=TTLocalizer.DisclaimerText, style=TTDialog.Acknowledge, command=self.destroyDisclaimer)
        self.disclaimer.show()

    def destroyDisclaimer(self, result):
        self.disclaimer.destroy()
        del self.disclaimer
        Settings['wantDisclaimer'] = False
    ...
    def __handleAvatarChooserDone(self, avList, doneStatus):
        ...
        if done == 'chose':
            ...
            if avatarChoice.allowedName:
                ...
                self.loginFSM.request('createAvatar', [avList, index, None, True])
        elif done == 'create':
    ...
    def enterCreateAvatar(self, avList, index, newDNA = None, isRewriteSession=False):
        if self.music:
            self.music.stop()
            self.music = None
        if newDNA != None:
        ...
        base.transitions.noFade()
        self.avCreate = MakeAToon.MakeAToon(self.loginFSM, avList, 'makeAToonComplete', index, self.isPaid(), isRewriteSession)
        self.avCreate.load()
        self.avCreate.enter()
    ...
    def __sendGetAvatarDetails(self, avId):
        self.toontownFriendsManager.d_getAvatarDetails(avId)
    ...
    def isFriendOnline(self, doId):
        if doId in self.invisibleFriends:
            return False
    ...
    def removeFriend(self, avatarId):
        self.toontownFriendsManager.d_removeFriend(avatarId)
    ...
    def sendGetFriendsListRequest(self):
        ...
        self.friendsListError = 0
        self.toontownFriendsManager.d_getFriendsListRequest()
    ...
    def handleFriendOnline(self, doId, commonChatFlags, whitelistChatFlags, alert = True):
        if doId in self.invisibleFriends:
            self.invisibleFriends.remove(doId)
    ...
    def handleFriendOffline(self, doId, alert):
        self.notify.debug('Friend %d now offline.' % doId)
        if doId in self.invisibleFriends:
            self.invisibleFriends.remove(doId)
        try:
            del self.friendsOnline[doId]
            messenger.send('friendOffline', [doId, alert])
        except:
            pass

    def handleFriendInvisible(self, doId, invisible, alert):
        if invisible and doId not in self.invisibleFriends:
            self.invisibleFriends.append(doId)
        if doId not in self.friendsOnline:
            self.friendsOnline[doId] = self.identifyFriend(doId)
            messenger.send('friendInvisible', [doId, invisible, alert])
            if doId not in self.invisibleFriends and not self.friendsOnline[doId]:
                self.friendPendingChatSettings[doId] = (0, 0)
        else:
            messenger.send('friendInvisible', [doId, invisible, alert])
