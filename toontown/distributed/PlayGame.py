...
from toontown.hood import RRHHood
...
class PlayGame(StateData.StateData):
    ...
    Hood2ClassDict = {ToontownGlobals.ToontownCentral: TTHood.TTHood,
     ...
     ToontownGlobals.ResistanceRangerHideout: RRHHood.RRHHood}
    Hood2StateDict = {ToontownGlobals.ToontownCentral: 'TTHood',
     ...
     ToontownGlobals.ResistanceRangerHideout: 'RRHHood'}
    ...
    def __init__(self, parentFSM, doneEvent):
        ...
        self.fsm = ClassicFSM.ClassicFSM('PlayGame', [State.State('start', self.enterStart, self.exitStart, ['quietZone']),
         ...
         State.State('RRHHood', self.enterRRHHood, self.exitRRHHood, ['quietZone'])], 'start', 'start')
    ...
    def handleHoodDone(self):
        ...
        if doneStatus['where'] == 'party':
            if 'hostId' in doneStatus:
                avId = doneStatus['hostId']
            else:
                avId = doneStatus['avId']
            self.getPartyZoneAndGoToParty(avId, doneStatus['zoneId'])
            return
        how = doneStatus['how']
        if how in ['tunnelIn',
         'teleportIn',
    ...
    def enterRRHHood(self, requestStatus):
        self.accept(self.hoodDoneEvent, self.handleHoodDone)
        self.hood.enter(requestStatus)

    def exitRRHHood(self):
        self._destroyHood()
    ...
    def getPartyZoneAndGoToParty(self, hostId, zoneId):
        ...
        base.cr.partyManager.requestPartyZone(hostId, zoneId, callback=self.goToParty)
        return
