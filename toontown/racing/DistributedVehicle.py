...
class DistributedVehicle(DistributedSmoothNode.DistributedSmoothNode, Kart.Kart, FSM.FSM):
    ...
    def announceGenerate(self):
        ...
        if self.localVehicle and self.controlled:
    ...
    def __wallCollisionStart(self, entry):
        if self.controlled:
            ...
            if 'bellicose-' in name:
                ...
                if not intoNP.isEmpty():
                    intoNP.hide()
                doId = int(name.split('-')[-1])
                if doId not in self.exploding:
                    self.exploding.append(doId)
    ...
    def restartKart(self, restartTime=0.25):
        # Shut off and start up the car right after so we don't have any problems when exiting the tunnel.
        self.stopped = True
        taskMgr.doMethodLater(restartTime, self.startCar, 'restartKart', extraArgs=[])

    def doExplosion(self, doId):
        explosion = globalPropPool.getProp('kapow')
        explosion.setBillboardPointEye()
        explosion.reparentTo(render)
        explosion.setPos(self, 0, 0, 4)
        explosion.setScale(0.4)
        explodeSfx = loader.loadSfx('phase_3.5/audio/sfx/ENC_cogfall_apart_%s.ogg' % random.randint(1, 6))
        explosionTrack = Sequence(Func(base.playSfx, explodeSfx), ActorInterval(explosion, 'kapow'), Func(explosion.delete))
        explosionTrack.start()

        if doId in self.exploding:
            self.exploding.remove(doId)
