...
class DistributedDivingGameAI(DistributedMinigameAI):
    ...
    def setGameReady(self):
        ...
        self.treasureTracking = [0] * self.numPlayers
    ...
    def treasureRecovered(self):
        ...
        if avId not in self.avIdList:
            ...
            return
        if avId not in self.treasureTracking:
            self.air.writeServerEvent('suspicious', avId, 'DivingGameAI.treasureRecovered: no treasure present.')
            return
        self.treasureTracking[self.treasureTracking.index(avId)] = 0
        timestamp = globalClockDelta.getFrameNetworkTime()
    ...
    def pickupTreasure(self, chestId):
        ...
        if avId not in self.avIdList:
            ...
            return
        if avId in self.treasureTracking:
            self.air.writeServerEvent('suspicious', avId, 'DivingGameAI.pickupTreasure: holding treasure.')
            return
        if not (0 <= chestId < len(self.treasureTracking)):
            self.air.writeServerEvent('suspicious', avId, 'DivingGameAI.pickupTreasure: requested invalid chestId: %s' % chestId)
            return
        if self.treasureTracking[chestId]:
            return        
        self.scoreTracking[avId][2] += 1
        self.treasureTracking[chestId] = avId
    ...
    def removeAvIdFromTreasureTracking(self, avId, status):
        if status == 'treasure':
            if avId in self.treasureTracking:
                self.treasureTracking[self.treasureTracking.index(avId)] = 0
            else:
                self.air.writeServerEvent('suspicious', avId, 'DivingGameAI.removeAvIdFromTreasureTracking: tried to drop treasure without holding one.')
    ...
    def handleCrabCollision(self, status):
        ...
        if status == 'normal' or status == 'treasure':
            ...
            self.removeAvIdFromTreasureTracking(avId, status)
        self.treasureDropped()
