...
from lib.libotp.NametagFloat2d import NametagFloat2d
...
class Purchase(PurchaseBase):
    ...
    def load(self):
        ...
        self.backgroundR.setPos(37.5, -25, -5)
    ...
    def enterReward(self):
        ...
        def celebrate(task):
            counterFadeInterval = Parallel()
            for counter in task.counters:
                counterFadeInterval.append(counter.colorScaleInterval(.3, Vec4(1, 1, 1, 0)))
            counterFadeInterval.start()
            ...
            for i in range(len(task.ids)):
                if task.pointsArray[i] == winningPoints:
                    ...
                    if avId in base.cr.doId2do:
                        toon = base.cr.doId2do[avId]
                        Sequence(ActorInterval(toon, 'jump'), Func(toon.loop, 'neutral')).start()
