...
class DistributedRaceGame(DistributedMinigame):
    ...
    def showNumbers(self, task):
        ...
        for i in range(len(task.choiceList)):
            ...
            if choice == 0:
                ...
                diceInstance.setBin('fixed', 11)
                diceInstance.setAttrib(DepthOffsetAttrib.make(1))
    ...
    def showChanceCardText(self, task):
        ...
        avatar = self.getAvatar(task.picker)
        name = avatar.getName()
        ...
        if rewardEntry[1] != -1:
            ...
            if isinstance(avatar, Suit):
                rewardstr_fmt = TTLocalizer.RaceGameCardTextHi2
            else:
            ...
            if avPosition >= RaceGameGlobals.NumberToWin:
                ...
                if avatar:
                    ...
                    if isinstance(avatar, Suit):
                        ...
                        Sequence(ActorInterval(avatar, 'jump'), Func(avatar.loop, 'neutral')).start()
            else:
                avatar = self.getAvatar(avId)
                if isinstance(avatar, Suit):
                    Sequence(ActorInterval(avatar, 'squirt-small-react'), Func(avatar.loop, 'neutral')).start()
    ...
    def walkInPlace(self, avatar, lane, place, time):
        ...
        walkSeq = Sequence(Func(avatar.loop, 'walk'),
    ...
    def setChancePositions(self, positions):
        ...
        for pos in positions:
            ...
            marker.setAttrib(DepthOffsetAttrib.make(1))
