...
class DistributedDivingGame(DistributedMinigame):
        ...
        if toonSD.status == 'treasure' and self.treasures and self.chestIcons:
            for i in range(self.NUMTREASURES):
                if self.treasures[i].grabbedId == avId:
                    ...
                    seq = Sequence()
                    shrink = LerpScaleInterval(self.treasures[i].treasureNode, duration=1.0, startScale=self.treasures[i].treasureNode.getScale(), scale=Vec3(0, 0, 0), blendType='easeIn')
    ...
    def __treasureGrabbed(self, collEntry):
        ...
        toonSD = self.toonSDs[avId]
        if toonSD.status == 'normal' and self.grabbedTreasure == -1:
            self.grabbedTreasure = chestId
    ...
    def __updateLocalToonTask(self, task):
        ...
        z = max(z, -42 + math.tan(base.camLens.getVfov() / 2.0 * math.pi / 180) * 54)
        camera.setZ(z)
