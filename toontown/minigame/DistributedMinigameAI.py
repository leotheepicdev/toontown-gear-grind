...
class DistributedMinigameAI(DistributedObjectAI.DistributedObjectAI):
    ...
    def setExpectedAvatars(self, avIds, minigameZone):
        self.avIdList = avIds
    ...
    def enterFrameworkWaitClientsJoin(self):
        ...
        self.__barrier = ToonBarrier('waitClientsJoin', self.uniqueName('waitClientsJoin'), avIds, JOIN_TIMEOUT, allAvatarsJoined, handleTimeout)
    ...
    def enterFrameworkWaitClientsReady(self):
        ...
        avIds = self.avIdList[:]
        for id in self.nonToonIds:
            avIds.remove(id)

        self.__barrier = ToonBarrier('waitClientsReady', self.uniqueName('waitClientsReady'), avIds, READY_TIMEOUT, allAvatarsReady, handleTimeout)
    ...
    def enterFrameworkWaitClientsExit(self):
        ...
        self.__barrier = ToonBarrier('waitClientsExit', self.uniqueName('waitClientsExit'), avIds, EXIT_TIMEOUT, allAvatarsExited, handleTimeout)
        for avId in list(self.stateDict.keys()):
    ...
    def handleMetagamePurchaseManager(self, scoreList):
        ...
        if self.metagameRound < TravelGameGlobals.FinalMetagameRoundIndex:
            ...
            if numToons == 1 and lastAvId in self.newbieIdList:
                doNewbie = True
            if doNewbie:
                pm = NewbiePurchaseManagerAI.NewbiePurchaseManagerAI(self.air, lastAvId, avIds, scoreList, self.minigameId, self.trolleyZone)
                MinigameCreatorAI.acquireMinigameZone(self.zoneId)
                pm.generateWithRequired(self.zoneId)
            else:
                pm = PurchaseManagerAI.PurchaseManagerAI(self.air, avIds, scoreList, self.minigameId, self.trolleyZone, self.newbieIdList, votesArray, newRound, desiredNextGame)
                pm.generateWithRequired(self.zoneId)
        else:
            ...
            for id in self.newbieIdList:
                pm = NewbiePurchaseManagerAI.NewbiePurchaseManagerAI(self.air, id, avIds, scoreList, self.minigameId, self.trolleyZone)
                MinigameCreatorAI.acquireMinigameZone(self.zoneId)
                pm.generateWithRequired(self.zoneId)

            if len(self.avIdList) > len(self.newbieIdList):
                pm = PurchaseManagerAI.PurchaseManagerAI(self.air, avIds, scoreList, self.minigameId, self.trolleyZone, self.newbieIdList, votesArray=votesArray, metagameRound=self.metagameRound)
                pm.generateWithRequired(self.zoneId)
        return
    ...
    def handleRegularPurchaseManager(self, scoreList):
        ...
        if len(self.avIdList) > len(self.newbieIdList):
            pm = PurchaseManagerAI.PurchaseManagerAI(self.air, avIds, scoreList, self.minigameId, self.trolleyZone, self.newbieIdList)
            pm.generateWithRequired(self.zoneId)
