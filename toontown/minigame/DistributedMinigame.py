...
class DistributedMinigame(DistributedObject.DistributedObject):
    ...
    def setGameReady(self):
        ...
        for avId in self.avIdList:
            ...
            if avatar:
                if isinstance(avatar, Toon.Toon):
    ...
    def setStartingVotes(self, startingVotesArray):
        # TODO: THIS SHOULD PROB BE REWROTE
    
        avIds = []
        for avId in self.avIdList:
            av = base.cr.doId2do.get(avId)
            if av == None:
                # This toon has not yet generated on our end.
                avIds.append(avId)
            else:
                if isinstance(av, Toon.Toon):
                    avIds.append(avId)
    
        if not len(startingVotesArray) == len(avIds):
            self.notify.error('length does not match, startingVotes=%s, avIds=%s' % (startingVotesArray, avIds))
            return
        for index in range(len(avIds)):
            avId = avIds[index]
