...
class TwoDGameToonSD(StateData.StateData):
    ...
    def showScoreText(self, number, scale = 1.25):
        if not number == 0:
            ...
            seq = Sequence(self.scoreText.posInterval(0.5, Point3(0, 0, self.toon.height + 2), blendType='easeOut'), self.scoreText.colorScaleInterval(0.25, Vec4(r, g, b, 0)), Func(self.hideScoreText))
            seq.start()
