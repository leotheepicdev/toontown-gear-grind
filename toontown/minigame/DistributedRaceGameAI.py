...
class DistributedRaceGameAI(DistributedMinigameAI):
    ...
    def setExpectedAvatars(self, avIds, minigameZone):
        ...
        avatarsRemaining = 4 - len(self.avIdList)
        for i in range(avatarsRemaining):
            suit = DistributedSuitAI(self.air, None)
            level = random.randrange(len(SuitDNA.suitsPerLevel))
            suit.dna = SuitDNA.SuitDNA()
            suit.dna.newSuitRandom(level)
            suit.setLevel(level)
            suit.generateWithRequired(minigameZone)
            self.nonToonIds.append(suit.doId)
            self.avIdList.append(suit.doId)
        
        self.numPlayers = len(self.avIdList)
        self.notify.debug('RACE: setExpectedAvatars: expecting avatars: ' + str(self.avIdList))
    ...
    def enterWaitClientsChoices(self):
        ...
        for avId in self.nonToonIds:
            self.avatarChoices[avId] = random.randint(1, 3)
