...
class DataStore:
    ...
    def readDataFromFile(self):
        if self.wantAnyDbm:
            ...
            try:
                ...
                self.notify.debug('Opening backup JSON data file at %s.' % (self.filepath + '.bu',))
                if os.path.exists(self.filepath):
                    os.remove(self.filepath)
            except IOError:
                try:
                    file = open(self.filepath, 'r')
                    self.notify.debug('Opening old JSON data file at %s..' % (self.filepath,))
                except IOError:
                    file = None
                    self.notify.debug('New JSON data file will be written to %s.' % (self.filepath,))
    ...
    def query(self, query):
        if self.data is not None:
            qData = json.loads(query)
            results = self.handleQuery(qData)
            qResults = json.dumps(results)
        else:
            results = None
            qResults = json.dumps(results)
        return qResults
