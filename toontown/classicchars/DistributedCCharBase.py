...
from lib.libotp._constants import CFQuicktalker, CFTimeout, CFSpeech, CFThought
...
class DistributedCCharBase(DistributedChar.DistributedChar):
    ...
    def enterTransitionToCostume(self):
        ...
        def getDustCloudIval():
            dustCloud = DustCloud.DustCloud(self, fBillboard=0, wantSound=1)
            dustCloud.setBillboardAxis(2.0)
            dustCloud.setZ(4)
            dustCloud.setScale(0.6)
            dustCloud.createTrack()
            dustCloud.wrtReparentTo(render)
            return Sequence(dustCloud.track, Func(dustCloud.destroy), name='dustCloadIval')
