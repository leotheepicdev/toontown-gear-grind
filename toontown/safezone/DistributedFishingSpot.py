...
from toontown.fishing.WagerForceAcknowledge import WagerForceAcknowledge
...
class DistributedFishingSpot(DistributedObject.DistributedObject):
    ...
    def __showCastGui(self):
        ...
        self.accept('dailyCatchChange', self.__updateDailyCatch)
    ...
    def __updateDailyCatch(self):
        if hasattr(base.cr, 'newsManager'):
            d_genus, d_species = base.cr.newsManager.getDailyCatch()
            if d_genus != -1:
                self.catchOfTheDay['text'] = TTLocalizer.CatchOfTheDaySpotKnown % TTLocalizer.FishSpeciesNames[d_genus][d_species]
            else:
                self.catchOfTheDay['text'] = ''
        else:
            self.catchOfTheDay['text'] = ''
    ...
    def __makeGui(self):
        ...
        self.catchOfTheDay = DirectLabel(parent=self.castGui, relief=None, text='', text_scale=0.09, text_fg=(0.95, 0.95, 0.95, 1), text_font=ToontownGlobals.getSignFont(), text_shadow=(0, 0, 0, 1), pos=(0, 0, -1.3))
        self.__updateFishTankGui()
        self.__updateDailyCatch()
        ...
        self.howToDialog.hide()
        self.madeGui = 1
        self.accept('escape', self.__userExit)
    ...
    def makeWager(self, offer, value, weight):
        self.offer = offer
        self.value = value
        ...
        weightText = largeStr + smallStr
        
        if offer == 1:
            message = TTLocalizer.WagerOfferS % (name, base.localAvatar.getName(), weightText)
        else:
            message = TTLocalizer.WagerOfferP % (name, base.localAvatar.getName(), offer, weightText)
        self.wagerBox = WagerForceAcknowledge(message=message, doneEvent='WagerDecided')
        self.accept('WagerDecided', self.handleResponse)
        self.wagerBox.show()

    def handleResponse(self):
        doneStatus = self.wagerBox.getDoneStatus()
        if doneStatus == 'ok':
            self.sendUpdate('wagerResponse', [1])
            self.wagerBox.cleanup()
            self.wagerBox = None
            if self.value > self.offer:
                loss = self.value - self.offer
                if loss == 1:
                    message = TTLocalizer.WagerLostS
                else:
                    message = TTLocalizer.WagerLostP % loss
            elif self.value == self.offer:
                message = TTLocalizer.WagerDraw
            else:
                gain = self.offer - self.value
                if gain == 1:
                    message = TTLocalizer.WagerWonS
                else:
                    message = TTLocalizer.WagerWonP % gain
            dialogClass = OTPGlobals.getGlobalDialogClass()
            self.notifyBox = dialogClass(message=message, doneEvent='wagerAck', style=OTPDialog.Acknowledge)
            self.notifyBox.show()
            self.accept('wagerAck', self.wagerAck)         
            
        else:
            self.sendUpdate('wagerResponse', [0])
            self.wagerBox.cleanup()
            self.wagerBox = None
