...
class DistributedCogdoInteriorAI(DistributedObjectAI.DistributedObjectAI):
    ...
    def __init__(self, air, elevator):
        ...
        self.track = self.bldg.tracks[0]
        if self.track == 's':
            self.SOSCard = self.chooseSOSCard(self.difficulty)
        elif self.track == 'l':
            self.SummonsAmount = config.GetInt('field-office-summons', 1)
    ...
    def handleAllAboard(self, seats = None):
        ...
        if not self._CogdoGameRepeat:
            if self.track in ('s', 'l', 'm') and self.currentFloor != 2:
                self.currentFloor += 1

        if self.track in ('m') and self.currentFloor != 2:
            ...
            if tuple:
                ...
                if toon:
                    ...
                    if self.track == 's':
                        ...
                        toon.addSummons(amount=self.SummonsAmount)
