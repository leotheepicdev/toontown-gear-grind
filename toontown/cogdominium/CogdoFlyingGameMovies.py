...
class CogdoFlyingGameIntro(CogdoGameMovie):
    ...
    def load(self):
        ...
        self._ival = Sequence(Func(start), Parallel(flyThroughIval, Sequence(Func(self.displayLine, 'cog', dialogue[0]), Wait(waitDur), Func(self.displayLine, 'toon', dialogue[1]), Wait(waitDur), Func(self.displayLine, 'cog', dialogue[2]), Wait(waitDur))), Func(end))
