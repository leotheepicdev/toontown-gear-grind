...
from direct.interval.IntervalGlobal import Sequence, Wait, Func, LerpHprInterval, Parallel, LerpPosInterval, Track, ParallelEndTogether, LerpFunctionInterval, LerpScaleInterval, LerpPosHprInterval, SoundInterval
...
class DistributedBossbotBoss(DistributedBossCog.DistributedBossCog, FSM.FSM):
    ...
    def __talkAboutPromotion(self, speech):
        ...
        if base.localAvatar.doId == self.mvp:
            speech += TTLocalizer.BossbotRTMVP
        return speech
    ...
    def doDirectedAttack(self, avId, attackCode):
        ...
        if toon:
            ...
            if not self.raised:
                ...
                neutral1Anim = self.actorInterval(neutral, startFrame=48)
            throwAnim = self.getAnim('throw')
            neutral2Anim = self.actorInterval(neutral)
            extraAnim = Sequence()
            if attackCode == ToontownGlobals.BossCogSlowDirectedAttack:
                extraAnim = self.actorInterval(neutral)
    ...
    def doZapToon(self, toon, pos = None, hpr = None, ts = 0, fling = 1, shake = 1):
        ...
        if fling:
            if self.isToonRoaming(toon.doId):
                toonTrack += [toon.actorInterval('slip-backward')]
                toonTrack += [toon.posInterval(0.5, getSlideToPos, fluid=1)]
        else:
            toonTrack += [toon.actorInterval('slip-forward')]
    ...
    def preparePracticeToon(self, toon, i):
        toonsInvolvedLen = len(self.involvedToons)
        if toonsInvolvedLen == 1:
            ...
            index2X = {0: -8,
                       ...
                       6: 6,
                       7: 8}
            toon.setPosHpr(index2X[i], 87.42, 0.03, 0, 0, 0)
