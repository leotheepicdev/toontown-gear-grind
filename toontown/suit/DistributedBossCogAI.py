...
from toontown.coghq import CogBossPlannerAI
...
class DistributedBossCogAI(DistributedAvatarAI.DistributedAvatarAI):
    ...
    def __init__(self, air, dept):
        ...
        self.mvp = 0
        self.mvpReward = 0
        self.accept('toonDied', self.handleBattleToonDied)
    ...
    def __handleUnexpectedExit(self, avId):
        self.removeToon(avId)
        self.d_announceTragedy(0, avId) 
        if avId in self.avId2Score:
            del self.avId2Score[avId]
        
    def handleBattleToonDied(self, avId):
        self.d_announceTragedy(1, avId)
        if avId in self.avId2Score:
            del self.avId2Score[avId]
       
    def d_announceTragedy(self, t_type, avId):
        if avId in self.avId2Name:
            name = self.avId2Name[avId]
        else:
            name = 'Toon'
        self.sendUpdate('announceTragedy', [t_type, avId, name])

    def addToon(self, avId):
        if avId not in self.looseToons and avId not in self.involvedToons:
            av = self.air.doId2do.get(avId)
            avName = av.getName()
            if avId not in self.avId2Name:
                self.avId2Name[avId] = avName
            if avId not in self.avId2Score:
                self.avId2Score[avId] = 0
    ...
    def damageToon(self, toon, deduction):
        ...
        if toon.getHp() <= 0:
            ...
            self.d_announceTragedy(1, toon.doId)
    ...
    def d_setBattleExperience(self):
        if self.getPractice():
            for toonId in self.involvedToons:
                ...
                meritsRecovered[self.deptIndex] = int( round( (7.5 * (7.5 / len(self.involvedToons))) * 100 ) )
                max = CogDisguiseGlobals.getTotalMerits(toon, self.deptIndex)
                if max:
                    if merits[self.deptIndex] + meritsRecovered[self.deptIndex] <= max:
                        merits[self.deptIndex] += meritsRecovered[self.deptIndex]
                    else:
                        merits[self.deptIndex] = max
                    toon.b_setCogMerits(merits)
                    
                    self.toonMerits[toon.doId] = meritsRecovered

        self.sendUpdate('setBattleExperience', [self.getBattleExperience()])

    def getBattleExperience(self):
        result = BattleExperienceAI.getBattleExperience(8, self.involvedToons, self.toonExp, self.toonSkillPtsGained, self.toonOrigQuests, self.toonItems, self.toonOrigMerits, self.toonMerits, self.toonParts, self.suitsKilled, self.helpfulToons, self.mvp, self.avId2SuitsKilled)
        return result
    ...
    def enterWaitForToons(self):
        self.acceptNewToons()
        self.barrier = self.beginBarrier('WaitForToons', self.involvedToons, 5, self.doneWaitForToons)

    def doneWaitForToons(self, toons):
        if self.getPractice():
    ...
    def invokeSuitPlanner(self, buildingCode, skelecog, virtual=0):
        planner = CogBossPlannerAI.CogBossPlannerAI(1, buildingCode, self.dna.dept, self.zoneId)
        ...
        if virtual:
            for suit in suits['activeSuits']:
                suit.b_setVirtual(random.randint(1, 4))
    ...
    def setMVP(self, mvp, reward):
        self.mvp = mvp
        self.mvpReward = reward
        
    def d_setMVP(self, mvp, reward):
        self.sendUpdate('setMVP', [mvp, reward])
        
    def b_setMVP(self, mvp, reward):
        self.setMVP(mvp, reward)
        self.d_setMVP(mvp, reward)
        
    def addScore(self, avId, score=1):
        if avId in self.avId2Score:
            if self.avId2Score[avId] <= 1000000:
                self.avId2Score[avId] += score
            
    def calcMVP(self):
        mvp = 0
        highestScore = 0
        for avId in self.avId2Score:
            av = self.air.doId2do.get(avId)
            if not av:
                continue
