...
from toontown.suit import SellbotBossGlobalsAI
from . import SuitDNA
import random
...
class DistributedSellbotBossAI(DistributedBossCogAI.DistributedBossCogAI, FSM.FSM):
    ...
    def __init__(self, air):
        ...
        if self.nerfed:
            ...
            self.pieHitToonup = SellbotBossGlobalsAI.PieToonupNerfed
            self.pieDamageMult = SellbotBossGlobalsAI.PieDamageMultNerfed
            self.hitCountDamage = SellbotBossGlobalsAI.HitCountDamageNerfed
        else:
            self.bossMaxDamage = ToontownGlobals.SellbotBossMaxDamage
            self.pieHitToonup = SellbotBossGlobalsAI.PieToonup
            self.pieDamageMult = SellbotBossGlobalsAI.PieDamageMult
            self.hitCountDamage = SellbotBossGlobalsAI.HitCountDamage
        self.recoverRate = 0
        self.recoverStartTime = 0
        self.stunLocked = True
    ...
    def getDamageMultiplier(self):
        if self.nerfed:
            return SellbotBossGlobalsAI.AttackMultNerfed
        else:
            return SellbotBossGlobalsAI.AttackMult
    ...
    def generateSuits(self, battleNumber):
        if self.nerfed:
            if battleNumber == 1:
                return self.invokeSuitPlanner(7, 0)
            else:
                return self.invokeSuitPlanner(8, 1)
        else:
            if battleNumber == 1:
    ...
    def enterVictory(self):
        ...
        if self.getPractice():
            ...
            isVP = 1
        self.suitsKilled.append({'type': '', 'level': None, 'track': self.dna.dept, 'giveMerits': 0, 'isSkelecog': 0, 'isForeman': 0, 'isVP': isVP, 'isCFO': 0, 'isSupervisor': 0, 'isVirtual': 0, 'activeToons': self.involvedToons[:]})
        self.barrier = self.beginBarrier('Victory', self.involvedToons, 10, self.__doneVictory)
        return

    def __doneVictory(self, avIds):
        if not self.getPractice():
            mvpReward = random.choice(list(NPCToons.npcFriends.keys()))
        ...
        for toonId in self.involvedToons:
            ...
            if toon:
                ...
                if toonId == self.mvp:
                    toon.attemptAddNPCFriend(self.mvpReward, numCalls=1)
    ...
    def createEasyModeBarrels(self):
        self.barrels = []
        for entId, entDef in SellbotBossGlobalsAI.BarrelDefs.items():
            barrelType = entDef['type']
            barrel = barrelType(self.air, entId)
            SellbotBossGlobalsAI.setBarrelAttr(barrel, entId)
