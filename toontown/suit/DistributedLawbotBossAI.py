...
class DistributedLawbotBossAI(DistributedBossCogAI.DistributedBossCogAI, FSM.FSM):
    ...
    def generateSuits(self, battleNumber):
        cogs = self.invokeSuitPlanner(4, 0)
        virtuals = self.invokeSuitPlanner(5, 1, 1)
        activeSuits = cogs['activeSuits'] + virtuals['activeSuits']
        reserveSuits = cogs['reserveSuits'] + virtuals['reserveSuits']
    ...
    def enterVictory(self):
        ...
        if self.getPractice():
            ...
            isCJ = 1
        self.suitsKilled.append({'type': '', 'level': None, 'track': self.dna.dept, 'giveMerits': 0, 'isSkelecog': 0, 'isForeman': 0, 'isVP': isCJ, 'isCFO': 0, 'isSupervisor': 0, 'isVirtual': 0, 'activeToons': self.involvedToons[:]})
    ...
    def setPieType(self):
        for toonId in self.involvedToons:
            toon = simbase.air.doId2do.get(toonId)
            if toon:
                toon.b_setPieType(ToontownBattleGlobals.MAX_TRACK_INDEX + 1)
