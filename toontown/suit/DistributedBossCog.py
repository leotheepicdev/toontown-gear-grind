...
class DistributedBossCog(DistributedAvatar.DistributedAvatar, BossCog.BossCog):
    ...
    def getMVP(self):
        return (self.mvp, self.mvpReward)
    ...
    def setBattleExperience(self, battleExperience):
        toonsList, self.deathList, self.helpfulToons, mvp = battleExperience
        self.toonRewardDicts = BattleExperience.genRewardDicts(toonsList)
        self.toonRewardIds = [ toonsList[0] for toon in toonsList ]
    ...
    def controlToons(self):
        ...
        if self.hasLocalToon():
            self.toMovieMode()
            Emote.globalDisguiseEmote.disableAll(base.localAvatar)            
    ...
    def toWalkMode(self):
        if self.cr:
            ...
            if place and hasattr(place, 'fsm'):
                place.setState('walk')
        Emote.globalDisguiseEmote.releaseAll(base.localAvatar)
    ...
    def exitWaitForToons(self):
        if self.getPractice():
            for toonId in self.involvedToons:
                toon = self.cr.doId2do.get(toonId)
                i = self.involvedToons.index(toonId)
                if toon:
                    toon.takeOffSuit()
                    self.preparePracticeToon(toon, i)
    ...
    def announceTragedy(self, t_type, avId, name):
        if avId == base.localAvatar.doId:
            return
    
        if t_type == 0:
            message = TTLocalizer.ToonDisconnected % name
        else:
            message = TTLocalizer.ToonDied % name
