...
class DistributedCashbotBoss(DistributedBossCog.DistributedBossCog, FSM.FSM):
    ...
    def makePrepareBattleTwoMovie(self, delayDeletes, crane, safe):
        ...
        goon = self.fakeGoons[0]
        crane = self.cranes[0]
        track = Sequence(Func(self.__hideToons), 
            Func(crane.request, 'Movie'), 
            Func(crane.accomodateToon, rToon), 
            Func(goon.request, 'Stunned'), 
            Func(goon.setPosHpr, 104, -316, 0, 165, 0, 0), 
            Parallel(self.door2.posInterval(4.5, VBase3(0, 0, 30)), self.door3.posInterval(4.5, VBase3(0, 0, 30)), bossTrack), 
            Func(rToon.loop, 'leverNeutral'), 
            Func(base.camera.reparentTo, self.geom), 
            Func(base.camera.setPosHpr, 104.92, -327.09, 4.2, 136.3, 0, 0), 
            Func(rToon.setChatAbsolute, TTLocalizer.ResistanceToonWatchThis, CFSpeech), 
            Wait(2), 
            Func(rToon.setChatAbsolute, TTLocalizer.ResistanceToonCraneInstructions1, CFSpeech),
            Wait(4),
            Func(rToon.setChatAbsolute, TTLocalizer.ResistanceToonCraneInstructions2, CFSpeech),
            Wait(4),
            Func(rToon.setChatAbsolute, TTLocalizer.ResistanceToonCraneInstructions3, CFSpeech),
            Wait(4),
            Func(rToon.setChatAbsolute, TTLocalizer.ResistanceToonCraneInstructions4, CFSpeech),
            Wait(4),
            Func(rToon.clearChat),
            base.camera.posHprInterval(0.5, Point3(101.02, -334.6, 18), Point3(315.0, 10.3, 0), blendType='easeInOut'), 
            Func(self.setChatAbsolute, TTLocalizer.CashbotBossGetAwayFromThat, CFSpeech), 
            Wait(1.5),
            Func(self.clearChat),
            Parallel(base.camera.posHprInterval(1.75, Point3(110, -321, 5), Point3(136.3, 0, 0), blendType='easeInOut'), Sequence(Wait(1.5), Parallel(Func(self.resistanceToon.loop, 'walk'), self.resistanceToon.posInterval(0.5, getSlideToPos, fluid=1)), Func(self.resistanceToon.loop, 'neutral'), Func(self.cranes[0].request, 'Free'))), 
            Wait(2), 
            Func(base.camera.setPosHpr, 91.54, -330.79, 3, 325.01, 0, 0), 
            Func(goon.request, 'Recovery'), 
            Wait(2), 
            Func(base.camera.setPosHpr, 94.17, -321.14, 3.14, 167.1, 5.19, 0), 
            ...
            Parallel(self.door3.posInterval(3, VBase3(0, 0, 0)),rToon.posHprInterval(3, Point3(136, -212.9, 0), VBase3(-14, 0, 0), startPos=Point3(110.8, -292.7, 0), startHpr=VBase3(-14, 0, 0)), goon.posHprInterval(3, Point3(125.2, -243.5, 0), VBase3(-14, 0, 0), startPos=Point3(104.8, -309.5, 0), startHpr=VBase3(-14, 0, 0))), 
            Func(self.__hideFakeGoons),
            Func(base.camera.setPosHpr, 81.01, -356.56, 3.56, 320.19, 4.76, 0),
            Func(self.__makeBattleTwoFlyDownMovie),
            Wait(3),
            Func(base.camera.setPosHpr, 105, -333, 18, -45, 15, 0),
    ...
    def makePrepareBattleThreeMovie(self, delayDeletes):
        ...
        track = Sequence(Func(base.camera.reparentTo, self.geom), Func(base.camera.setPosHpr, 63.42, -383.37, 30.54, 320, -10.09, 0), self.moveToonsToBattleThreePos(self.involvedToons))
    ...
    def __talkAboutPromotion(self, speech):
        ...
        if base.localAvatar.doId == self.mvp:
            menuIndex, itemIndex = ResistanceChat.decodeId(self.mvpReward)
            value = ResistanceChat.getItemValue(self.mvpReward)
            ...
            speech += TTLocalizer.ResistanceToonMVP % instructions    
        return speech
    ...
    def enterPrepareBattleTwo(self):
        ...
        seq = Sequence(self.makePrepareBattleTwoMovie(delayDeletes, self.movieCrane, self.movieSafe), Func(self.__beginBattleTwo), name=intervalName)
    ...
    def enterPrepareBattleThree(self):
        ...
        if self.getPractice():
            battlePos = Point3(ToontownGlobals.CashbotBossBattleThreePosHpr[0], ToontownGlobals.CashbotBossBattleThreePosHpr[1], ToontownGlobals.CashbotBossBattleThreePosHpr[2])
            finalHpr = VBase3(135, 0, 0)
            self.setPos(battlePos)
            self.setHpr(finalHpr)
            self.reparentTo(render)
            self.pelvis.setHpr(self.pelvisReversedHpr)
            self.doAnimate()
        self.__showResistanceToon(False)
    ...
    def enterReward(self):
        ...
        victory, camVictory, skipper = MovieToonVictory.doToonVictory(1, self.involvedToons, self.toonRewardIds, self.toonRewardDicts, self.deathList, self.rewardPanel, allowGroupShot=0, noSkip=True, mvp=self.mvp)
