...
class DistributedBossbotBossAI(DistributedBossCogAI.DistributedBossCogAI, FSM.FSM):
    ...
    def generateSuits(self, battleNumber):
        if battleNumber == 1:
            retval = self.invokeSuitPlanner(6, 0)
            return retval
        else:
    ...
    def enterVictory(self):
        ...
        self.suitsKilled.append({'type': '', 'level': None, 'track': self.dna.dept, 'giveMerits': 0, 'isSkelecog': 0, 'isForeman': 0, 'isVP': isCEO, 'isCFO': 0, 'isSupervisor': 0, 'isVirtual': 0, 'activeToons': self.involvedToons[:]})
    ...
    def givePinkSlipReward(self, toon):
        ...
        if toon.doId == self.mvp:
            add = 2
        else:
            add = 1
        toon.addPinkSlips(self.battleDifficulty + add)
