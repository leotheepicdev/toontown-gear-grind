...
class DistributedPhone(DistributedFurnitureItem.DistributedFurnitureItem):
    ...
    def setupCamera(self, mode):
        ...
        if mode == PhoneGlobals.PHONE_MOVIE_PICKUP:
            quat = Quat()
            quat.setHpr((35, -8, 0))
            self.quatInterval = LerpPosQuatInterval(camera, 1, (4, -4, base.localAvatar.getHeight() - 0.5), quat, blendType='easeOut', other=base.localAvatar).start()
    ...
    def freeAvatar(self):
        if self.hasLocalAvatar:
            ...
            if self.quatInterval:
                self.quatInterval.finish()
                self.quatInterval = None
