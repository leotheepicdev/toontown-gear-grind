...
class LoadGiftAvatar:
    ...
    def __gotAvatar(self, dclass, fields):
        ...
        for key in (
            ...
            try:
                fields[key] = fields[key][0].decode()
            except:
                fields[key] = fields[key][0].decode('cp855')
        ...
        newDict = self.copyDict(fields, 'setDNAString', 'setMailboxContents', 'setAwardMailboxContents',
                                ...
                                'setHatList', 'setGlassesList', 'setBackpackList',
...
class DistributedPhoneAI(DistributedFurnitureItemAI):
    ...
    def requestPurchaseMessage(self, context, blob, optional):
        ...
        if not isinstance(item, CatalogFlooringItem):
            # the code freaks out over floors, so lets just hack this until we know what the hell is going on with them.
        ...
        def handleAccount(dclass, fields):
            ...
            if item.__class__.__name__ == 'CatalogAccessoryItem':
                if item.isHat():
                    if random.random() <= 0.35:
                        retCode = ToontownGlobals.P_ItemOnOrderPet
            ...
            if retCode in (ToontownGlobals.P_ItemOnOrder, ToontownGlobals.P_ItemOnOrderPet, ToontownGlobals.P_ItemAvailable):
                av.takeMoney(price)
    ...
    def requestGiftPurchaseMessage(self, context, targetId, blob, optional):
        ...
        def handleAccount(dclass, fields):
            ...
            if targetId == av.getPetId():
                if not item.allowPets():
                    self.air.writeServerEvent('suspicious', avId, 'av tried to gift an item that isn\'t for pets!')
                    self.sendUpdateToAvatarId(avId, 'requestGiftPurchaseResponse', [context, ToontownGlobals.P_NotAGift])
                    return
                pet = self.air.doId2do.get(targetId)
                if pet:
                    if item.petReachedPurchaseLimit(pet):
                        self.air.writeServerEvent('suspicious', avId, 'av tried to gift an item that a pet already has!')
                        self.sendUpdateToAvatarId(avId, 'requestGiftPurchaseResponse', [context, ToontownGlobals.P_ReachedPurchaseLimit])
                        return
                    ...
                    if price > av.getTotalMoney():
                        self.air.writeServerEvent('suspicious', avId, 'Failed to take away the jellybeans from the avatar')
                        self.sendUpdateToAvatarId(avId, 'requestGiftPurchaseResponse', [context, ToontownGlobals.P_NotEnoughMoney])
                        return

                    av.takeMoney(price)
                    response = item.recordPetPurchase(pet)
                    self.sendUpdateToAvatarId(avId, 'requestGiftPurchaseResponse', [context, response])
                    return
            else:
                if not item.isGift():
