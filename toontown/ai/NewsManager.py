...
class NewsManager(DistributedObject.DistributedObject):
    ...
    def setInvasionStatus(self, msgType, cogList, numRemaining, specialSuit, invType):
        self.notify.info('setInvasionStatus: msgType: %s cogList: %s, numRemaining: %s, specialSuit: %s' % (msgType,
         cogList,
         numRemaining,
         specialSuit))
        
        if invType == SuitInvasionGlobals.INV_NORMAL:
            cogName = SuitBattleGlobals.SuitAttributes[cogList[0]]['name']
            cogNameP = SuitBattleGlobals.SuitAttributes[cogList[0]]['pluralname']
        else:
            cogName = TTLocalizer.InvType2NameS[invType]
            cogNameP = TTLocalizer.InvType2NameP[invType]        
    ...
    def setDailyCatch(self, genus, species):
        ...
        messenger.send('dailyCatchChange')
