...
class CatalogItemPanel(DirectFrame):
    ...
    def updateGiftButton(self, giftUpdate = 0):
        ...
        if giftUpdate == 0:
            return
        self.auxText['text'] = ' '
        numFriends = base.localAvatar.hasPet() + len(base.localAvatar.friendsList) + len(base.cr.avList) - 1
        if numFriends > 0:
            self.giftButton['state'] = DGG.DISABLED
            self.giftButton.show()
            auxText = ' '
            if self.parentCatalogScreen.gotAvatar == 0 and self['item'].isGift() <= 0:
                self.giftButton.show()
                self.giftButton['state'] = DGG.DISABLED
                ...
                if isinstance(avatar, DistributedPet.DistributedPet):
                    if not self['item'].allowPets():
                        self.giftButton.show()
                        self.giftButton['state'] = DGG.DISABLED
                        self.auxText['text'] = TTLocalizer.CatalogNotForPets
                    else:
                        if self['item'].petReachedPurchaseLimit(avatar):
                            self.giftButton.show()
                            self.giftButton['state'] = DGG.DISABLED
                ...
                if self['item'].isGift() <= 0:
                    ...
                    self.auxText['text'] = auxText
                    return
                elif self['item'].forBoysOnly() and avatar.getStyle().getGender() == 'f' or self['item'].forGirlsOnly() and avatar.getStyle().getGender() == 'm':
                    self.giftButton.show()
                    self.giftButton['state'] = DGG.DISABLED
