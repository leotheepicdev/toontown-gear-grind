from panda3d.core import Camera, DisplayRegion, Lens, NodePath, PGButton, PerspectiveLens, Plane, PlaneNode, Point3, TextNode, Vec3, Vec4
...
from lib.libotp.ChatBalloon import ChatBalloon
...
class CatalogScreen(DirectFrame):
    ...
    def loadClarabelle(self):
        ...
        self.clarabelle.setBlend(frameBlend=True)
    ...
    def __makeFFlist(self):
        ...
        if base.wantPets and base.localAvatar.hasPet():
            ...
            handle = base.cr.identifyFriend(petId)
            colorCode = NametagGroup.CCFreeChat
            newFF = (petId, handle.getName(), colorCode)
            self.ffList.append(newFF)
        ...
        if self.frienddoId and self.allowGetDetails:
            if self.giftAvatar:
                if hasattr(self.giftAvatar, 'doId'):
                    ...
                    self.giftAvatar.delete()
                self.giftAvatar = None
            if base.localAvatar.hasPet() and self.frienddoId == base.localAvatar.getPetId():
                self.giftAvatar = DistributedPet.DistributedPet(base.cr)
                self.giftAvatar.doId = self.frienddoId
                self.giftAvatar.forceAllowDelayDelete()
                self.giftAvatar.generate()
                base.cr.getAvatarDetails(self.giftAvatar, self.__handleAvatarDetails, 'DistributedPet')
                self.gotAvatar = 0
