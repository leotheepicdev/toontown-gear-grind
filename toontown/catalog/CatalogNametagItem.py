...
class CatalogNametagItem(CatalogItem.CatalogItem):
    ...
    def reachedPurchaseLimit(self, avatar):
        if avatar.onOrder.count(self) != 0 or avatar.onGiftOrder.count(self) != 0 or avatar.mailboxContents.count(self) != 0 or self in avatar.awardMailboxContents or self in avatar.onAwardOrder:
            return 1
        if avatar.nametagStyle == self.nametagStyle:
            return 1
        return 0
        
    def petReachedPurchaseLimit(self, pet):
        if pet.nametagStyle == self.nametagStyle:
            return 1
        return 0
    ...
    def recordPetPurchase(self, pet):
        if pet:
            pet.b_setNametagStyle(self.nametagStyle)
        return ToontownGlobals.P_ItemAvailable
