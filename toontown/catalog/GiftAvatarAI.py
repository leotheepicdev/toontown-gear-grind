...
class GiftAvatarAI:
    ...
    def setDNAString(self, dnaString):
        ...
        self.style.makeFromNetString(dnaString.encode())

    def setMailboxContents(self, mailboxContents):
        self.mailboxContents = CatalogItemList.CatalogItemList(mailboxContents.encode(),
                                                               store=CatalogItem.Customization)

    def setAwardMailboxContents(self, awardMailboxContents):
        self.awardMailboxContents = CatalogItemList.CatalogItemList(awardMailboxContents.encode(),
                                                                    store=CatalogItem.Customization)

    def setGiftSchedule(self, onOrder):
        self.onGiftOrder = CatalogItemList.CatalogItemList(onOrder.encode(),
                                                           store=CatalogItem.Customization | CatalogItem.DeliveryDate)

    def setDeliverySchedule(self, onOrder):
        self.onOrder = CatalogItemList.CatalogItemList(onOrder.encode(),
                                                       store=CatalogItem.Customization | CatalogItem.DeliveryDate)

    def setAwardSchedule(self, onOrder):
        self.onAwardOrder = CatalogItemList.CatalogItemList(onOrder.encode(),
                                                            store=CatalogItem.Customization | CatalogItem.DeliveryDate)
    ...
    def addToGiftSchedule(self, avId, targetId, item, minutes=0):
        ...
        simbase.air.send(simbase.air.dclassesByName['DistributedToonAI'].aiFormatUpdate('setGiftSchedule', targetId, targetId, OTP_ALL_CLIENTS, [self.getGiftScheduleBlob()]))
