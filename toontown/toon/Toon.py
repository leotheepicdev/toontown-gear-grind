...
from panda3d.core import CollideMask, ColorBlendAttrib, GeomNode, LODNode, NodePath, PartBundle, Plane, PlaneNode, Point3, Texture, VBase3, VBase4, Vec3
...
import importlib
import functools
from lib.libotp import CFThought, CFSpeech, CFTimeout
...
class Toon(Avatar.Avatar, ToonHead):
    ...
    def __init__(self):
        ...
        self.emote = None
        self.disguiseEmoteTrack = None
        self.disguiseEmote = None
        self.disguiseHead = False
        self.stunTrack = None
        self.__bookActors = []
        self.__holeActors = []
        ...
        self.setChatFont(ToontownGlobals.getToonFont())
        self.setAvatarType('toon')
        ...
        self.animFSM = ClassicFSM('Toon', [State('off', self.enterOff, self.exitOff),
         ...
         State('DisguiseEmote', self.enterDisguiseEmote, self.exitDisguiseEmote),
    ...
    def stopAnimations(self):
        ...
        if self.emoteTrack != None:
            ...
            self.emoteTrack = None
        if self.disguiseEmoteTrack != None:
            self.disguiseEmoteTrack.finish()
            self.disguiseEmoteTrack = None
    ...
    def generateToonClothes(self, fromNet = 0):
        ...
        if self.hasLOD():
            if fromNet == 0:
                try:
                    bottomPair = ToonDNA.Shorts[self.style.botTex]
                except:
                    bottomPair = ToonDNA.Shorts[0]
            ...
            try:
                texName = ToonDNA.Shorts[self.style.botTex][0]
            except:
                texName = ToonDNA.Shorts[0][0]

            bottomTex = loader.loadTexture(texName, okMissing=True)
            if bottomTex is None:
                bottomTex = loader.loadTexture(ToonDNA.Shorts[0][0])
    ...
    def getDialogueArray(self):
        if self.isDisguised and self.getDisguiseHead():
    ...
    def enterRun(self, animMultiplier = 1, ts = 0, callback = None, extraArgs = []):
        ...
        Emote.globalDisguiseEmote.disableBody(self, 'toon, enterRun')
        self.setActiveShadow(1)
    ...
    def getDiedInterval(self, autoFinishTrack = 1):
        ...
        shrinkSound = loader.loadSfx('phase_5/audio/sfx/ENC_Lose_shrinking.ogg')
        if hasattr(self, 'uniqueName'):
            trackName = self.uniqueName('died')
        else:
            trackName = 'died'
        ival = Sequence(Func(Emote.globalEmote.disableBody, self), Func(self.sadEyes), Func(self.blinkEyes), Track((0, ActorInterval(self, 'lose')), (2, SoundInterval(sound, node=self)), (5, SoundInterval(shrinkSound, node=self)), (5.333, self.scaleInterval(1.5, VBase3(0.01, 0.01, 0.01), blendType='easeInOut'))), Func(self.detachNode), Func(self.setScale, 1, 1, 1), Func(self.normalEyes), Func(self.blinkEyes), Func(Emote.globalEmote.releaseBody, self), name=trackName, autoFinish=autoFinishTrack)
        return ival
    ...
    def enterDisguiseEmote(self, animMultiplier = 1, ts = 0, callback = None, extraArgs = []):
        if len(extraArgs) > 0:
            emoteIndex = extraArgs[0]
        else:
            return
        ...
        if self.isLocal():
            ...
            self.wakeUp()
        duration = 0
        self.disguiseEmoteTrack, duration = Emote.globalDisguiseEmote.doEmote(self, emoteIndex, ts)
        self.setActiveShadow(1)

    def doDisguiseEmote(self, emoteIndex, animMultiplier = 1, ts = 0, callback = None, extraArgs = []):
       # if not self.isLocal():
        #    if base.cr.avatarFriendsManager.checkIgnored(self.doId):
         #       return
        duration = 0
        if self.isLocal():
            ...
            if self.hasTrackAnimToSpeed():
                self.trackAnimToSpeed(None)
        try:
            self.emoteTrack, duration = Emote.globalDisguiseEmote.doEmote(self, emoteIndex, ts)
        except:
            pass

    def exitDisguiseEmote(self):
        self.stop()
        if self.disguieEmoteTrack != None:
            self.disguiseEmoteTrack.finish()
            self.disguiseEmote = None
    ...
    def putOnSuit(self, suitType, setDisplayName = True, rental = False):
        ...
        if self.isLocal():
            ...
            self.chatMgr.chatInputSpeedChat.addCogEmoteMenu()
    ...
    def takeOffSuit(self):
        ...
        if self.isLocal():
            ...
            self.chatMgr.chatInputSpeedChat.removeCogEmoteMenu()
        self.suit.delete()
        del self.suit
        del self.suitGeom
