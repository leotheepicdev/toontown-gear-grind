from panda3d.core import BitMask32, CallbackObject, CollideMask, CollisionHandler, CollisionHandlerEvent, CollisionNode, CollisionSphere, GeomNode, NodePath, Notify, PandaNode, Point3, PythonCallbackObject, StringStream, TextNode, VBase4, Vec2, Vec3, Vec4
import random
import math
import time
import re
...
from toontown.shtiker import DailyRewardsPage
...
from toontown.toontowngui.JellybeanGainDisplay import JellybeanGainDisplay
...
if (__debug__):
    import pdb
from lib.util import PandaNode
...
from lib.libotp._constants import CFThought, CFTimeout
import sys
...
class LocalToon(DistributedToon.DistributedToon, LocalAvatar.LocalAvatar):
    ...
    def __init__(self, cr):
        try:
            ...
            self.acceptingNewFriends = Settings['acceptingNewFriends'] and base.config.GetBool('accepting-new-friends-default', True)
            self.acceptingNonFriendWhispers = Settings['acceptingNonFriendWhispers'] and base.config.GetBool('accepting-non-friend-whispers-default', True)
            ...
            self.lastTouch = 0
            self.touchPresses = []
            self.jellybeanGainDisplay = None
            self.wantLocalTeleports = True
    ...
    def disable(self):
       ...
       # del self.dailyRewardsPage
        ...
        self.jellybeanGainDisplay.disable()
    ...
    def delete(self):
        try:
            ...
            if self.__piePowerMeter:
                ...
                self.__piePowerMeter = None
            if self.chatLog:
                self.chatLog.cleanup()
                self.chatLog = None
            if self.jellybeanGainDisplay:
                self.jellybeanGainDisplay.removeModels()
                self.jellybeanGainDisplay = None
    ...
    def initInterface(self):
       ...
       # self.dailyRewardsPage = DailyRewardsPage.DailyRewardsPage()
       # self.dailyRewardsPage.load()
       # self.book.addPage(self.dailyRewardsPage, pageName=TTLocalizer.DailyRewardsPageTitle)
        ...
        self.chatLog.hide()
        self.jellybeanGainDisplay = JellybeanGainDisplay()
    ...
    def getDefaultKey(self, key):
        return OTPGlobals.CONTROL_DEFAULT_KEYS[key]

    def removeTouch(self, press):
        press = self.getDefaultKey(press)
    ...
    def displayTalkWhisper(self, fromId, avatarName, message):
        sfx = self.soundWhisper
        message = avatarName + ': ' + message
    ...
    def startChat(self):
        if self.tutorialAck:
            ...
            self.accept('chatUpdateSCGreeting', self.d_swapTeleportGreeting)
    ...
    def stopChat(self):
        ...
        self.ignore('chatUpdateSCGreeting')
    ...
    def loadFurnitureGui(self):
        ...
        self.__furnitureGui = DirectFrame(relief=None, parent=base.a2dTopLeft, pos=(0.143333, 0, -0.67), scale=0.04, image=guiModels.find('**/attic'))
    ...
    def handleBattleFriendsList(self):
        self.setLocalTeleports(False)
        self.friendsListButtonActive = 1
        self.clarabelleButtonObscured = 1
        self.refreshOnscreenButtons()
        
    def handleBattleExit(self):
        self.setLocalTeleports(True)
        self.clarabelleButtonObscured = 0
    ...
    def setLocalTeleports(self, active):
        self.wantLocalTeleports = active
        
    def allowLocalTeleports(self):
        return self.wantLocalTeleports

    def loadGardeningGui(self):
        if self.__gardeningGui:
            return
        gardenGuiCard = loader.loadModel('phase_5.5/models/gui/planting_gui')
        self.__gardeningGui = DirectFrame(relief=None, parent=base.a2dTopLeft, geom=gardenGuiCard, geom_color=GlobalDialogColor, geom_scale=(0.17, 1.0, 0.3), pos=(0.133333, 0, -0.5), scale=1.0)
        self.__gardeningGui.setName('gardeningFrame')
        self.__gardeningGuiFake = DirectFrame(relief=None, parent=base.a2dTopLeft, geom=None, geom_color=GlobalDialogColor, geom_scale=(0.17, 1.0, 0.3), pos=(0.133333, 0, -0.5), scale=1.0)
    ...
    def displayLocalItemGain(self, displayType, amount):
        if self.jellybeanGainDisplay:
            self.jellybeanGainDisplay.queueDisplay(displayType, amount)
            
    def preparePiggyMax(self, piggyMax):
        if self.jellybeanGainDisplay:
            self.jellybeanGainDisplay.preparePiggyMax(piggyMax)

    def showQuestsOnscreenTutorial(self):
        self.questPage.showQuestsOnscreenTutorial()

    def hideQuestsOnscreenTutorial(self):
        self.questPage.hideQuestsOnscreenTutorial()
