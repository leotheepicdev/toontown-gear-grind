...
class ToonTeleportPanel(DirectFrame):
    ...
    def enterCheckAvailability(self):
        base.localAvatar.iWantToTeleportToSomeone(self.avId)
    ...
    def enterOtherShard(self, shardId, hoodId, zoneId):
        ...
        pop = None
        lowPop, midPop, highPop = ToontownGlobals.DefaultDistrictPopLimits
        for shard in base.cr.listActiveShards():
            if shard[0] == shardId:
                pop = shard[2]
                lowPop, midPop, highPop = shard[4]
    ...
    def __handleOk(self):
        messenger.send('toggleGoToButton', [1])
        unloadTeleportPanel()

    def __handleCancel(self):
        base.localAvatar.sendUpdate('receiveTeleportPanelResponse', [2, 0, 0, 0])
        messenger.send('toggleGoToButton', [1])
        unloadTeleportPanel()

    def __handleYes(self):
        base.localAvatar.sendUpdate('receiveTeleportPanelResponse', [1, self.shardId, self.hoodId, self.zoneId])
        self.bYes['state'] = DGG.DISABLED
        self.bNo['state'] = DGG.DISABLED

    def __handleNo(self):
        base.localAvatar.sendUpdate('receiveTeleportPanelResponse', [0, 0, 0, 0])
        messenger.send('toggleGoToButton', [1])
        unloadTeleportPanel()

    def __teleportResponse(self, response, avId, shardId, hoodId, zoneId):
        teleportNotify.debug('__teleportResponse%s' % ((response,
          avId,
          shardId,
          hoodId,
          zoneId),))
