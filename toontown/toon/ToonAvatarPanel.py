...
class ToonAvatarPanel(AvatarPanelBase.AvatarPanelBase):
    ...
    def __init__(self, avatar, playerId = None):
        ...
        self.accept('toggleGoToButton', self.toggleGoToButton)


        if not base.localAvatar.allowLocalTeleports() or base.cr.avatarFriendsManager.checkIgnored(self.avId):
    ...
    def toggleGoToButton(self, value):
        if value == 0:
            self.goToButton['state'] = DGG.DISABLED
        else:
