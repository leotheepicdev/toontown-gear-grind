...
class InventoryNew(InventoryBase.InventoryBase, DirectFrame):
    ...
    PropBonusImageColor2 = Vec4(1.0, 0.7, 0.0, 1)
    ...
    def updateTotalPropsText(self):
        ...
        if localAvatar.getPinkSlips() > 1:
            textTotal = textTotal + '\n\n' + TTLocalizer.InventoryPinkSlips % localAvatar.getPinkSlips()
        elif localAvatar.getPinkSlips() == 1:
            textTotal = textTotal + '\n\n' + TTLocalizer.InventoryPinkSlip
        if localAvatar.getSummons() >= 1:
            textTotal = textTotal + '\n' + TTLocalizer.InventorySummons % localAvatar.getSummons()
    ...
    def load(self):
        ...
        self.accept('update-summons', self.updateTotalPropsText)
        self.trackRows = []
        ...
        for track in range(0, len(Tracks)):
            ...
            for item in range(0, len(Levels[track])):
                button = ToontownButton(parent=self.trackRows[track], image=(self.upButton,
                 self.downButton,
                 self.rolloverButton,
    ...
    def __handleTaunt(self):
        messenger.send('inventory-taunt')
    ...
    def showDetail(self, track, level, event = None):
        ...
        if self.activateMode == 'purchase' or self.activateMode == 'storePurchase':
            if (level+1) > 1:
                text = TTLocalizer.InventoryDetailCostP
            else:
                text = TTLocalizer.InventoryDetailCostS
            self.detailDataLabel['text'] = text % (level + 1)
            return
        ...
        accString = AvTrackAccStrings[track] % acc
        if (organicBonus or propBonus) and track == LURE_TRACK:
            accString = TTLocalizer.BattleGlobalLureAccMedium % acc
        if track == LURE_TRACK and level == UBER_GAG_LEVEL_INDEX:
            accString = TTLocalizer.BattleGlobalLureAccMedium % acc
        self.detailDataLabel.configure(text=TTLocalizer.InventoryDetailData % {'accuracy': accString,
         'damageString': self.getTrackDmgStr(track, level),
         'damage': damage,
            ...
            if self.toon.hasTrackAccess(track):
                ...
                for level in range(len(Levels[track])):
                    ...
                    if self.itemIsUsable(track, level):
                        ...
                        if self.numItem(track, level) >= self.getMax(track, level) or (self.totalProps == self.toon.getMaxCarry() and level != UBER_GAG_LEVEL_INDEX):
    ...
    def makeBookUnpressable(self, button, track, level):
        ...
        if organicBonus:
            button.configure(image0_color=self.PropBonusPressableImageColor, image2_color=self.PropBonusImageColor2)
        else:
            button.configure(image0_color=self.BookUnpressableImage0Color, image2_color=self.BookUnpressableImage2Color)
    ...
    def showTrack(self, trackIndex):
        ...
        if curExp >= regMaxSkill:
            self.trackBars[trackIndex]['range'] = regMaxSkill
            self.trackBars[trackIndex]['text'] = TTLocalizer.RewardPanelMeritsMaxed
        else:
    ...
    def getTrackDmgStr(self, track, level):
        if track == HEAL_TRACK:
            return TTLocalizer.InventoryToonUpString
    ...
    def loadBattleFrame(self):
        ...
        self.runButton = ToontownButton(parent=self.battleFrame, relief=None, anim='stretch', pos=(0.73, 0, -0.398), text=TTLocalizer.InventoryRun, text_scale=TTLocalizer.INrunButton, text_pos=(0, -0.02), text_fg=Vec4(1, 1, 1, 1), textMayChange=0, image=(self.upButton, self.downButton, self.rolloverButton), image_scale=1.05, image_color=(0, 0.6, 1, 1), command=self.__handleRun)
        self.sosButton = ToontownButton(parent=self.battleFrame, relief=None, anim='stretch', pos=(0.96, 0, -0.398), text=TTLocalizer.InventorySOS, text_scale=0.05, text_pos=(0, -0.02), text_fg=Vec4(1, 1, 1, 1), textMayChange=0, image=(self.upButton, self.downButton, self.rolloverButton), image_scale=1.05, image_color=(0, 0.6, 1, 1), command=self.__handleSOS)
        self.passButton = ToontownButton(parent=self.battleFrame, relief=None, anim='stretch', pos=(0.96, 0, -0.242), text=TTLocalizer.InventoryPass, text_scale=TTLocalizer.INpassButton, text_pos=(0, -0.02), text_fg=Vec4(1, 1, 1, 1), textMayChange=1, image=(self.upButton, self.downButton, self.rolloverButton), image_scale=1.05, image_color=(0, 0.6, 1, 1), command=self.__handlePass)
        self.fireButton = ToontownButton(parent=self.battleFrame, relief=None, anim='stretch', pos=(0.73, 0, -0.242), text=TTLocalizer.InventoryFire, text_scale=TTLocalizer.INfireButton, text_pos=(0, -0.02), text_fg=Vec4(1, 1, 1, 1), textMayChange=0, image=(self.upButton, self.downButton, self.rolloverButton), image_scale=1.05, image_color=(0, 0.6, 1, 1), command=self.__handleFire)
        self.tauntButton = ToontownButton(parent=self.battleFrame, relief=None, anim='stretch', pos=(-0.96, 0, -0.465), text=TTLocalizer.InventoryTaunt, text_scale=TTLocalizer.INfireButton, text_pos=(0, -0.016), text_fg=Vec4(1, 1, 1, 1), textMayChange=0, image=(self.upButton, self.downButton, self.rolloverButton), image_scale=(1.05, 1.05, 0.5), image_color=(0, 0.6, 1, 1), command=self.__handleTaunt)
        self.sueButton = ToontownButton(parent=self.battleFrame, relief=None, anim='stretch', pos=(-0.76, 0, -0.465), text=TTLocalizer.InventorySue, text_scale=TTLocalizer.INfireButton, text_pos=(0, -0.016), text_fg=Vec4(1, 1, 1, 1), textMayChange=0, image=(self.upButton, self.downButton, self.rolloverButton), image_scale=(1.05, 1.05, 0.5), image_color=(0, 0.6, 1, 1), command=self.__handleSue)
