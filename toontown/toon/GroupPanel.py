...
class GroupPanel(DirectObject.DirectObject):
    ...
    def __handlePracticeButton(self):
    ...
    def setPractice(self, practice):
        ...
        if self.practiceEnabledText:
            if self.practice:
                self.practiceEnabledText['text'] = TTLocalizer.BoardingPracticeEnabled
            else:
                self.practiceEnabledText['text'] = ''
    ...
    def cleanup(self):
        ...
        if self.practiceButton:
            ...
            self.practiceButton = None
        if self.practiceEnabledText:
            self.practiceEnabledText.destroy()
            self.practiceEnabledText = None
    ...
    def __load(self):
        ...
        if self.boardingParty.getPracticeEnabled():
            if base.localAvatar.doId == self.leaderId:
                ...
                self.practiceButton = DirectButton(parent=self.frame, relief=None, image=matchingModel.find('**/minnieCircle'), image_scale=0.4, command=self.__handlePracticeButton,
                                                   text=('', TTLocalizer.BoardingTogglePractice, TTLocalizer.BoardingTogglePractice, ''), text_scale=0.06, text_fg=Vec4(1, 1, 1, 1), text_shadow=Vec4(0, 0, 0, 1),
                                                   text_pos=(0.045, 0), text_align=TextNode.ALeft, pos=(0.223, 0, practiceButtonZPos))
                self.practiceButton.setBin('gui-popup', 1000)
            self.practiceEnabledText = DirectLabel(parent=self.frame, relief=None, text_fg=Vec4(1, 1, 1, 1), text_shadow=Vec4(0, 0, 0, 1),
                                                   pos=(0.018, 0, 0.36), text='', textMayChange=1, text_font=ToontownGlobals.getMinnieFont(), text_scale=0.06)
        self.frame.show()
        ...
        if self.boardingParty.getPracticeEnabled() and base.localAvatar.doId == self.leaderId:
