    ...
    def __showData(self):
        ...
        if online:
            ...
            if self.playerInfo:
                ...
                text = TTLocalizer.AvatarDetailPanelOnline % {'district': shardName, 'location': hoodName, 'toonId': toonId}
        else:
            text = TTLocalizer.AvatarDetailPanelOffline % {'toonId': toonId, 'lastSeen': TTLocalizer.getLastSeenTimestampString(self.avatar.getLastSeenTimestamp())}
                ...
                for item in range(0, len(Levels[track])):
                    ...
                    if curExp >= level:
                        ...
                        if numItems == 0:
                            ...
                            geom_color = None
                        DirectLabel(parent=self, image=buttonModel, image_scale=(0.92, 1, 1), image_color=image_color, geom=base.localAvatar.inventory.invModels[track][item], geom_color=geom_color, geom_scale=0.6, relief=None, pos=(xOffset + item * xSpacing, 0, yOffset + track * ySpacing))
                    else:
                        break
