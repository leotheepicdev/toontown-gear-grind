...
from . import TTEmote, DisguiseEmote
from toontown.shtiker.OptionsPage import speedChatStyles, chatStyles
...
from . import InventoryNew, InventoryBase
...
if (__debug__):
    import pdb
from lib.libotp._constants import *
from lib.libotp.WhisperPopup import WhisperPopup
from lib.libotp import NametagGroup
...
class DistributedToon(DistributedPlayer.DistributedPlayer, Toon.Toon, DistributedSmoothNode.DistributedSmoothNode, DelayDeletable):
    ...
    def setExperience(self, experience):
        ...
        if self.inventory and isinstance(self.inventory, InventoryNew.InventoryNew):
    ...
    def d_battleSOS(self, toId):
        self.sendUpdate('requestBattleSOS', [toId])
    ...
    def setDefaultShard(self, shard):
        ...
        if self.doId == base.localAvatar.doId:
            try:
                districtName = base.cr.activeDistrictMap[shard].name
            except:
                districtName = 'Unknown District'
            base.updateDistrict(districtName)
    ...
    def setTalk(self, fromAV, fromAC, avatarName, message, flags):
        ...
        base.talkAssistant.receiveOpenTalk(fromAV, avatarName, fromAC, None, message)
    ...
    def setTalkWhisper(self, fromAV, fromAC, avatarName, message, flags):
        ...
        base.talkAssistant.receiveWhisperTalk(fromAV, avatarName, fromAC, None, self.doId, self.getName(), message)

    def receiverIsSleeping(self, name):
        self.setSystemMessage(0, TTLocalizer.sleep_auto_reply % name)
    ...
    def setWhisperSCDisguiseEmoteFrom(self, fromId, emoteId):
        handle = base.cr.identifyFriend(fromId)
        if handle == None:
            return
        if not self._isValidWhisperSource(handle):
        ...
        if base.cr.avatarFriendsManager.checkIgnored(fromId):
            self.d_setWhisperIgnored(fromId)
            return
        chatString = SCDecoders.decodeSCDisguiseEmoteWhisperMsg(emoteId, handle.getName())
        if chatString:
            self.displayWhisper(fromId, chatString, WhisperPopup.WTEmote)
            base.talkAssistant.receiveAvatarWhisperSpeedChat(TalkAssistant.SPEEDCHAT_DISGUISE_EMOTE, emoteId, fromId)
    ...
    def b_setDisguiseEmoteState(self, animIndex, animMultiplier):
        self.setDisguiseEmoteState(animIndex, animMultiplier)
        self.d_setDisguiseEmoteState(animIndex, animMultiplier)

    def d_setDisguiseEmoteState(self, animIndex, animMultiplier):
        timestamp = globalClockDelta.getFrameNetworkTime()
        self.sendUpdate('setDisguiseEmoteState', [animIndex, animMultiplier, timestamp])

    def setDisguiseEmoteState(self, animIndex, animMultiplier, timestamp = None):
        if animIndex == DisguiseEmote.EmoteClear:
            return
        if timestamp == None:
            ts = 0.0
        else:
        ...
        extraArgs = []
        extraArgs.insert(0, animIndex)
        self.doDisguiseEmote(animIndex, animMultiplier, ts, callback, extraArgs)
    ...
    def b_setChatStyleIndex(self, index):
        realIndexToSend = 0
        if type(index) == type(0) and 0 <= index and index < len(chatStyles):
            realIndexToSend = index
        else:
            base.cr.centralLogger.writeClientEvent('Hacker alert b_setSpeedChatStyleIndex invalid')
        self.setChatStyleIndex(realIndexToSend)
        self.d_setChatStyleIndex(realIndexToSend)
    ...
    def d_setChatStyleIndex(self, index):
        ...
        if type(index) == type(0) and 0 <= index and index < len(chatStyles):
            ...
            base.cr.centralLogger.writeClientEvent('Hacker alert d_setSpeedChatStyleIndex invalid')
        self.sendUpdate('setChatStyleIndex', [realIndexToSend])
    ...
    def setChatStyleIndex(self, index):
        ...
        if type(index) == type(0) and 0 <= index and index < len(chatStyles):
            ...
            base.cr.centralLogger.writeClientEvent('Hacker victim setChatStyleIndex invalid attacking toon = %d' % self.doId)
        self.chatStyleIndex = realIndexToUse
        nameKey, color = chatStyles[realIndexToUse]
        self.nametag.setBalloonColor(Vec4(color[0], color[1], color[2], color[3]))
    ...
    def d_reqSueCog(self):
        self.sendUpdate('reqSueCog')

    def sueCogResponse(self, reward, amount, type):
        messenger.send('sue-cog-response', [reward, amount, type])

    def setSummons(self, summons):
        oldSummons = self.summons
        self.summons = summons
        if base.localAvatar == self:
            if oldSummons != summons:
                messenger.send('update-summons')
    ...
    def toonUp(self, hpGained, extraStrCode=-1, hasInteractivePropBonus = False):
        ...
        if hpGained > 0:
            self.showHpText(hpGained, hasInteractivePropBonus=hasInteractivePropBonus, extraStrCode=extraStrCode)
            self.hpChange(quietly=0)
        return

    def showHpText(self, number, bonus = 0, scale = 1, hasInteractivePropBonus = False, extraStrCode=-1):
        if self.HpTextEnabled and not self.ghostMode:
            if number != 0:
                ...
                if number < 0:
                    ...
                    if extraStrCode > -1:
                        hpGainedStr += '\n' + TTLocalizer.ExtraHpTerms[extraStrCode]
                ...
                seq = Sequence(self.hpText.posInterval(1.0, Point3(0, 0, self.height + 1.5), blendType='easeOut'), Wait(0.85), self.hpText.colorScaleInterval(0.1, Vec4(r, g, b, 0)), Func(self.hideHpText))
                seq.start()

    def displayJellybeanGain(self, amount):
        if base.localAvatar == self:
            base.localAvatar.displayLocalItemGain(displayType=0, amount=amount)

    def setName(self, name = 'unknownDistributedAvatar'):
        DistributedPlayer.DistributedPlayer.setName(self, name)
        self._handleGMIcon()

    def _handleGMIcon(self):
        if self._isGM:
            self.setGMIcon(self._gmType)
        else:
            self.removeGMIcon()
    ...
    def setGMIcon(self, gmType = None):
        ...
        iconInfo = (('phase_3.5/models/gui/tt_m_gui_gm_toontroop_whistle', '**/*whistleIcon*', 4),
         ...
         ('phase_3.5/models/gui/tt_m_gui_gm_staff_gg', '**/*eyesIcon*', 4),
         ('phase_3.5/models/gui/tt_m_gui_gm_content_creator', '**/megaphoneIcon*', 4))
    ...
    def setGMPartyIcon(self):
        ...
        iconInfo = ('phase_3.5/models/gui/tt_m_gui_gm_toonResistance_fist', 'phase_3.5/models/gui/tt_m_gui_gm_toontroop_whistle',
                    ...
                    'phase_3.5/models/gui/tt_m_gui_gm_staff_gg', 'phase_3.5/models/gui/tt_m_gui_gm_content_creator')
        if gmType > len(iconInfo) - 1:
            return
    ...
    def getCarInterest(self):
        return self.carInterest

    def updateCarActive(self):
        if self.getCarActive():
            if self.racingZoneCheck() and not self.getCarInterest():
                self.carInterest = base.cr.addInterest(self.doId, [100], 'kart-%d' % self.doId)
            return
        if self.getCarInterest() and self.isGenerated():
            base.cr.removeInterest(self.carInterest)
            self.carInterest = None

    def racingZoneCheck(self):
    ...
    def setFriendsList(self, friendsList):
        ...
        for friendId, trueFriend in self.friendsList:
            ...
            if friend:
                base.cr.toontownFriendsManager.friendOnline(friendId, 0, 0, False)
    ...
    def setLastSeenTimestamp(self, lastSeenTimestamp):
    ...
    def changeDisguiseTypeResp(self):
        messenger.send('changeDisguiseTypeResp')

    def d_dumpFish(self):
        self.sendUpdate('dumpFish', [])
    ...
    def d_swapTeleportGreeting(self, msgIndex):
        i = OTPLocalizer.GreetingMsg2Id[msgIndex]
        self.sendUpdate('swapTeleportGreeting', [i])

    def d_toggleDisguiseHead(self):
        self.sendUpdate('toggleDisguiseHead', [])
