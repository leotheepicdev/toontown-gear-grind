...
class DistributedNPCToonBase(DistributedToon.DistributedToon):
    ...
    def setChatAbsolute(self, chatString, chatFlags, dialogue = None, interrupt = 1):
        DistributedToon.DistributedToon.setChatAbsolute(self, chatString, chatFlags, dialogue, interrupt)
        if chatString == '':
            return
        if self.isLocalToon:
            if base.localAvatar.chatLog:
                chatMsgs = chatString.split('\x07')
                for msg in chatMsgs:
                    base.localAvatar.chatLog.log(self.getName(), msg)
