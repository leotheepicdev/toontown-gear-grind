...
class NPCFriendCard(DirectFrame):
    ...
    def __handleDeleteNPCFriend(self):
        if self.confirmDialog is not None:
            ...
            self.confirmDialog = None
        self.confirmDialog = TTDialog.TTDialog(style=TTDialog.YesNo, text=TTLocalizer.NPCFriendPageDeleteConfirmation % TTLocalizer.NPCToonNames[self['NPCID']], command=self.confirmCallback)
        self.confirmDialog.show()
        
    def confirmCallback(self, value):
        if value > 0:
            self.__deleteNPCFriend()
        self.__closeConfirmDialog()
        
    def __deleteNPCFriend(self):
        base.localAvatar.sendUpdate('removeNPCFriend', [self['NPCID']])
        self.update(None)
