...
from otp.distributed.OtpDoGlobals import OTP_ALL_CLIENTS
...
class DistributedToonAI(DistributedPlayerAI.DistributedPlayerAI, DistributedSmoothNodeAI.DistributedSmoothNodeAI, PetLookerAI.PetLookerAI):
    ...
    def __init__(self, air):
        ...
        self.codesRedeemed = []
        self.carActive = 0
        self.carToggleAllowed = False
        self.gloveTickets = 0
        self.fishingSpotId = 0
        ...
        self.nextKnockKnockHeal = 0
        self.allowedRewrite = 0
        self.clothingTickets = 0
        
        self.piggyBank = (0, 0, 0)
    ...
    def announceGenerate(self):
        ...
        if self.isPlayerControlled():
            ...
            self.sendUpdate('setLastSeenTimestamp', [int(time.time())])
        self.startPing()
    ...
    def delete(self):
        ...
        del self.kart
        del self.fishingSpotId
        del self.teleportAvId
        del self.lastTeleportTime
        del self.lastWhisperTime
        self.cleanupPing()
        self.stopPing()
    ...
    def verifyDNA(self):
        changed = False
        if self.isPlayerControlled():
            allowedColors = ToonDNA.defaultColorList
    ...
    def removeNPCFriend(self, npcFriend):
        del self.NPCFriendsDict[npcFriend]
        self.d_setNPCFriendsDict(self.NPCFriendsDict)
    ...
    def takeDamage(self, hpLost, quietly = 0, sendTotal = 1, wantRelocate=True):
        ...
        if not self.hpOwnedByBattle:
            ...
            if sendTotal:
                self.d_setHp(self.hp, wantRelocate)
            if self.getCarActive() and self.hp <= 0:
                self.deleteKart()
    ...
    def dumpFish(self):
        try:
            self.fishTank.removeFishAtIndex(len(self.fishTank.fishList) - 1) 
            netlist = self.fishTank.getNetLists()
            self.fishTank.makeFromNetLists(netlist[0], netlist[1], netlist[2])
            self.d_setFishCollection(netlist[0], netlist[1], netlist[2])
            self.d_setFishTank(netlist[0], netlist[1], netlist[2])
        except:
            pass
    ...
    def addResistanceMessage(self, textId, amount=2):
        success = True
    ...
    def b_setChatStyleIndex(self, index):
        self.setChatStyleIndex(index)
        self.d_setChatStyleIndex(index)

    def d_setChatStyleIndex(self, index):
        self.sendUpdate('setChatStyleIndex', [index])

    def setChatStyleIndex(self, index):
        self.chatStyleIndex = index

    def getChatStyleIndex(self):
        return self.chatStyleIndex
    ...
    def addMoney(self, deltaMoney, display=False):
        ...
        if display:
            self.displayJellybeanGain(deltaMoney)
    ...
    def reqThrowSummons(self):
        if self.pieType != 7:
            self.b_setNumPies(self.getSummons())
            self.b_setPieType(7)
        else:
            self.b_setNumPies(0)
            self.b_setPieType(0)
    ...
    def toonUp(self, hpGained, quietly = 0, sendTotal = 1, extraStrCode=-1):
        if hpGained > self.maxHp:
            hpGained = self.maxHp
        if not quietly:
            self.sendUpdate('toonUp', [hpGained, extraStrCode])
    ...
    def displayJellybeanGain(self, amount):
        self.sendUpdate('displayJellybeanGain', [amount])
    ...
    def doResistanceEffect(self, msgIndex):
        ...
        if msgType == ResistanceChat.RESISTANCE_TOONUP:
            ...
            if msgValue == -1:
                self.addMoney(999999, display=True)
            else:
                self.addMoney(msgValue, display=True)
    ...
    def toggleKart(self):
        ...
        if self.getHp() <= 0:
            return
        # TODO: add a time check here?
        if self.getCarActive():
            self.b_setCarActive(0)
        else:
            self.b_setCarActive(1)
        if self.getCarActive():
            if not self.kart:
                from toontown.racing import DistributedVehicleAI
                self.kart = DistributedVehicleAI.DistributedVehicleAI(self.air, self.doId)
                self.kart.setControlled(1)
    ...
    def reqSueCog(self):
        if not self.removeSummons(ToontownGlobals.SUE_COST):
            return
        chance = 0.8
        if random.random() <= chance:
            amount = random.choice([30, 40, 50, 60, 70, 80, 90, 100, 110, 120, 130, 140, 150])
            self.addMoney(amount)
            self.sendUpdate('sueCogResponse', [ToontownGlobals.SUE_REWARD_JELLYBEANS, amount, 0])
            return
        if random.random() <= chance:
            ticketChoice = random.choice([ToontownGlobals.SUE_REWARD_CLOTHING_TICKET, ToontownGlobals.SUE_REWARD_GLOVE_TICKET])
            if ticketChoice == ToontownGlobals.SUE_REWARD_CLOTHING_TICKET:
                amount = random.randint(1, 2)
                self.addClothingTickets(amount)
            elif ticketChoice == ToontownGlobals.SUE_REWARD_GLOVE_TICKET:
                amount = random.randint(1, 2)
                self.addGloveTickets(amount)
            self.sendUpdate('sueCogResponse', [ticketChoice, amount, 0])
            return
        reward = random.choice([ToontownGlobals.SUE_REWARD_PINK_SLIP, ToontownGlobals.SUE_REWARD_UNITE, ToontownGlobals.SUE_REWARD_SOS])
        if reward == ToontownGlobals.SUE_REWARD_PINK_SLIP:
            amount = random.randint(1, 2)
            type = 0
            self.addPinkSlips(amount)
        elif reward == ToontownGlobals.SUE_REWARD_UNITE:
            amount = 1
            type = ResistanceChat.getRandomId()
            self.addResistanceMessage(type, amount)
        elif reward == ToontownGlobals.SUE_REWARD_SOS:
            amount = 1
            type = random.choice(NPCToons.npcFriendsMinMaxStars(3, 5))
            self.attemptAddNPCFriend(type, amount)
            
        self.sendUpdate('sueCogResponse', [reward, amount, type])

    def reqCogSummons(self, type, suitIndex):
        if type not in ('single', 'building', 'invasion', 'office', 'skeleinvasion', 'dance'):
        ...
        if suitIndex >= len(SuitDNA.suitHeadTypes):
            ...
            return
        if type == 'single':
            cost = ToontownGlobals.COG_SUMMONS_COST
        elif type == 'building':
            cost = ToontownGlobals.BUILDING_SUMMONS_COST
        elif type == 'office':
            cost = ToontownGlobals.OFFICE_SUMMONS_COST
        elif type == 'invasion':
            cost = ToontownGlobals.INVASION_SUMMONS_COST
        elif type == 'skeleinvasion':
            cost = ToontownGlobals.SKELE_INVASION_SUMMONS_COST
        elif type == 'dance':
            cost = ToontownGlobals.DANCE_SUMMONS_COST
        if not self.getSummons() >= cost:
        ...
        if type == 'single':
            ...
            returnCode = self.doOfficeTakeover(suitIndex)
        elif type == 'invasion':
            returnCode = self.doCogInvasion(suitIndex)
        elif type == 'skeleinvasion':
            returnCode = self.doCogInvasion(suitIndex, ToontownGlobals.INV_SKELE)
        elif type == 'dance':
            returnCode = self.doDanceUnite(suitIndex)
        if returnCode:
            if returnCode[0] == 'success':
                self.air.writeServerEvent('cogSummoned', self.doId, '%s|%s|%s' % (type, suitIndex, self.zoneId))
                self.removeSummons(cost)
    ...
    def doSummonSingleCog(self, suitIndex):
        ...
        for zoneId in zones:
            if zoneId in map:
                points = map[zoneId][:]
                suit = sp.createNewSuit([], points, suitName=suitName, summon=True)
                if suit:
    ...
    def doBuildingTakeover(self, suitIndex):
        ...
        if building == None:
            return ['badlocation', suitIndex, 0]
        if self.air.bldgMgr.getStreetNumBuildings(streetId) >= len(self.air.bldgMgr.getStreetBuildingWeight(streetId)):
            return ['capacity', suitIndex, 0]
        level = None
        if suitIndex >= len(SuitDNA.suitHeadTypes):
            self.notify.warning('Bad suit index: %s' % suitIndex)
            return ['badIndex', suitIndex, 0]
        ...
        building.suitTakeOver(track, False)
        return ['success', suitIndex, building.doId]
        
    def doOfficeTakeover(self, suitIndex):
        streetId = ZoneUtil.getBranchZone(self.zoneId)
        if streetId not in self.air.suitPlanners:
            self.notify.warning('Street %d is not known.' % streetId)
            return ['badlocation', suitIndex, 0]
        ...
        suitName = SuitDNA.suitHeadTypes[suitIndex]
        track = SuitDNA.getSuitDept(suitName)
        building.cogdoTakeOver(track)
        return ['success', suitIndex, building.doId]

    def doCogInvasion(self, suitIndex, specialSuit=ToontownGlobals.INV_NORMAL):
        ...
        if invMgr.getInvading():
            ...
            if suitIndex >= len(SuitDNA.suitHeadTypes):
                self.notify.warning('Bad suit index: %s' % suitIndex)
                return ['badIndex', suitIndex, 0]
            cogList = [SuitDNA.suitHeadTypes[suitIndex]]
            numCogs = 1000
            if invMgr.startInvasion(cogList, numCogs, specialSuit):
                returnCode = 'success'
            else:
                returnCode = 'fail'
    ...
    def doDanceUnite(self, suitIndex):
        success = self.addResistanceMessage(300, 1)
        if success:
            returnCode = 'success'
        else:
            returnCode = 'capacity'
    ...
    def b_setSummons(self, summons):
        self.d_setSummons(summons)
        self.setSummons(summons)

    def d_setSummons(self, summons):
        self.sendUpdate('setSummons', [summons])

    def setSummons(self, summons):
    ...
    def restockAllCogSummons(self):
        self.b_setSummons(ToontownGlobals.MAX_SUMMONS)

    def addSummons(self, amount=1):
        if self.getSummons() == ToontownGlobals.MAX_SUMMONS:
            return
        if self.getSummons() + amount > ToontownGlobals.MAX_SUMMONS:
            summons = ToontownGlobals.MAX_SUMMONS
        else:
            summons = self.getSummons() + amount
        self.b_setSummons(summons)

    def removeSummons(self, amount=1):
        summons = self.getSummons()
        if summons >= amount:
            self.b_setSummons(summons-amount)
            return True
        return False
    ...
    def b_setNametagColor(self, nametagColor):
        self.d_setNametagColor(nametagColor)
        self.setNametagColor(nametagColor)

    def d_setNametagColor(self, nametagColor):
        self.sendUpdate('setNametagColor', [nametagColor])
    ...
    def b_setNametagPanelColor(self, nametagPanelColor):
        self.d_setNametagPanelColor(nametagPanelColor)
        self.setNametagPanelColor(nametagPanelColor)

    def d_setNametagPanelColor(self, nametagPanelColor):
        self.sendUpdate('setNametagPanelColor', [nametagPanelColor])

    def setNametagPanelColor(self, nametagPanelColor):
        self.nametagPanelColor = nametagPanelColor

    def getNametagPanelColor(self):
        return self.nametagPanelColor
    ...
    def iWantToTeleportToSomeone(self, toId):
        if time.time() - self.lastTeleportTime <= 1:
            self.air.moderationManager.serverKick(self.doId, 'Teleporting too fast.')
            return
        self.lastTeleportTime = time.time()

        if toId == 0 or toId == self.doId:
            self.air.moderationManager.serverKick(self.doId, 'Attempted to teleport to invalid toId.')
            return
        self.teleportAvId = toId

        # Let the toId know that we want to request to them.
        dg = self.dclass.aiFormatUpdate('someoneWantsToTeleportToMe', toId, toId, OTP_ALL_CLIENTS, [self.doId, self.air.districtId])
        self.air.send(dg)
        taskMgr.doMethodLater(10, self.cancelTeleport, 'cancel-teleport-%s' % self.doId)

    def personRespondedToTeleportRequest(self, responseId, shardId, hoodId, zoneId):
        senderId = self.air.getAvatarIdFromSender()
        if senderId != self.teleportAvId:
            # Person may of lagged really hard or is trying to mess around with our toon.
            return
        toId, flag = self.getTeleportInfo()
        if taskMgr.hasTaskNamed('cancel-teleport-%s' % self.doId):
            # Avatar responded! Remove the cancel task.
            taskMgr.remove('cancel-teleport-%s' % self.doId)
        if responseId == 0:
            self.sendUpdate('teleportResponse', [0, self.teleportAvId, 0, 0, 0])
            self.setTeleportInfo(0, 0)
        elif responseId == 1: # They are on another shard.
            self.teleportFlag = 2
            self.sendUpdate('teleportResponse', [1, self.teleportAvId, shardId, hoodId, zoneId])
        elif responseId == 2:
            self.sendUpdate('teleportResponse', [2, self.teleportAvId, 0, 0, 0])
            self.setTeleportInfo(0, 0)
        elif responseId == 3: # Person has allowed us to respond. Teleport!
            self.teleportToRequestedAvatar(shardId, hoodId, zoneId)

    def cancelTeleport(self, task=None):
        # The person logged out or is having a rough time with the server.
        # We do not want to have ourselves hanging for too long.
        self.sendUpdate('teleportResponse', [0, self.teleportAvId, 0, 0, 0])
        self.setTeleportInfo(0, 0)
        if task:
            return task.done

    def receiveTeleportPanelResponse(self, response, shardId, hoodId, zoneId):
        # response 0 - No (We do not want to teleport to the avatar)
        # response 1 - Yes (We want to teleport to the avatar.)
        # response 2 - Cancel (If the avatar has not responded, we can cancel the teleport.)
        toId, flag = self.getTeleportInfo()
        if flag != 2:
            # Our teleport flag is not the cross-district response. We canceled AFTER the avatar responded.
            return
        if response == 1:
            # We want to teleport to the avatar.
            # We cannot cancel this now, so if the avatar is not there, oh well!
            self.teleportToRequestedAvatar(shardId, hoodId, zoneId)
        if response in [0, 2]:
            # We do not want to teleport to the avatar, therefore we not associate with them anymore.
            self.setTeleportInfo(0, 0)

    def teleportToRequestedAvatar(self, shardId, hoodId, zoneId):
        # We are allowed to teleport to our avatar. Store our flag in-case we teleport to another district.
        toId, flag = self.getTeleportInfo()
        if flag != 1:
            self.b_setTeleportInfo(self.teleportAvId, 1)
            self.sendUpdate('teleportResponse', [3, self.teleportAvId, shardId, hoodId, zoneId])

    def requestTeleportGreeting(self):
        toId, flag = self.getTeleportInfo()
        if flag == 1 and toId in self.air.doId2do:
            self.b_setTeleportInfo(0, 0)
            if not self.ghostMode:
                self.sendUpdate('teleportGreetingSuccess', [toId])

    def requestTeleportGiveup(self):
        toId, flag = self.getTeleportInfo()
        if flag == 1 and toId in self.air.doId2do:
            self.b_setTeleportInfo(0, 0)
            av = self.air.doId2do[toId]
            av.sendUpdate('teleportGiveup', [self.doId])

    def d_setTeleportInfo(self, avId, flag):
        self.sendUpdate('setTeleportInfo', [avId, flag])

    def setTeleportInfo(self, avId, flag):
        self.teleportAvId = avId
        self.teleportFlag = flag

    def b_setTeleportInfo(self, avId, flag):
        self.setTeleportInfo(avId, flag)
        self.d_setTeleportInfo(avId, flag)

    def getTeleportInfo(self):
        return self.teleportAvId, self.teleportFlag
    ...
    def setCodesRedeemed(self, codesRedeemed):
        self.codesRedeemed = codesRedeemed

    def d_setCodesRedeemed(self, codesRedeemed):
        self.sendUpdate('setCodesRedeemed', [codesRedeemed])

    def b_setCodesRedeemed(self, codesRedeemed):
        self.setCodesRedeemed(codesRedeemed)
        self.d_setCodesRedeemed(codesRedeemed)

    def getCodesRedeemed(self):
        return self.codesRedeemed

    def addToDeliverySchedule(self, item, minutes = 0):
        if config.GetBool('want-instant-delivery', True):
            minutes = 0
    ...
    def setGloveTickets(self, gloveTickets):
        self.gloveTickets = gloveTickets

    def d_setGloveTickets(self, gloveTickets):
        self.sendUpdate('setGloveTickets', [gloveTickets])

    def b_setGloveTickets(self, gloveTickets):
        self.setGloveTickets(gloveTickets)
        self.d_setGloveTickets(gloveTickets)

    def getGloveTickets(self):
        return self.gloveTickets

    def addGloveTickets(self, amount):
        gloveTicketNum = self.getGloveTickets()
        if gloveTicketNum == 255:
            return
        if gloveTicketNum + amount > 255:
            newAmount = 255
        else:
            newAmount = gloveTicketNum + amount
        self.b_setGloveTickets(newAmount)

    def removeGloveTicket(self):
        gloveTicketNum = int(self.getGloveTickets())
        if gloveTicketNum >= 1:
            self.b_setGloveTickets(int(gloveTicketNum - 1))
            return 1
        return 0

    def requestBattleSOS(self, toId):
        battleId = self.getBattleId()
        if not battleId:
            self.air.moderationManager.serverKick(self.doId, 'Requested battle SOS when not in a battle.')
            return
        battle = self.air.doId2do.get(battleId)
        if battle and battle.battleCalc:
            if self.doId in battle.battleCalc.sosCalls:
                battle.battleCalc.removeBattleSOS(self.doId)
                dg = self.dclass.aiFormatUpdate('battleSOS', toId, toId, OTP_ALL_CLIENTS, [self.doId])
                self.air.send(dg)
            else:
                self.air.moderationManager.serverKick(self.doId, 'Requested battle SOS more than once in a round.')

    def setFishingSpotId(self, fishingSpotId):
        self.fishingSpotId = fishingSpotId

    def getFishingSpotId(self):
        return self.fishingSpotId

    def exitFromFishingSpot(self):
        self.fishingSpotId = 0

    def setSleepFlag(self, sleepFlag):
        self.sleepFlag = sleepFlag

    def getSleepFlag(self):
        return self.sleepFlag

    def receiverIsSleeping(self):
        self.sendUpdate('receiverIsSleeping')

    def setTalkWhisperAI(self, senderId, account, name, message, flag):
        # Someone wants to whisper to me!
        if senderId == self.doId:
            # What is this madness? I am trying to whisper to myself!
            self.air.moderationManager.serverKick(self.doId, 'Sent setTalkWhisperAI to self.')
            return
        # I am sleeping.
        if self.sleepFlag:
            dg = self.dclass.aiFormatUpdate('receiverIsSleeping', senderId, senderId, OTP_ALL_CLIENTS, [self.getName()])
            self.air.send(dg)
        # Tell my client the message.
        self.sendUpdate('setTalkWhisper', [senderId, account, name, message, flag])

    def setTalkWhisperSCAI(self, senderId, name, type, msgIndex):
        # Someone wants to whisper to me!
        if senderId == self.doId:
            # What is this madness? I am trying to whisper to myself!
            self.air.moderationManager.serverKick(self.doId, 'Sent setTalkWhisperAI to self.')
            return
        if type == 1:
            field = 'setWhisperSCFrom'
        elif type == 2:
            field = 'setWhisperSCEmoteFrom'
        elif type == 3:
            field = 'setWhisperSCCustomFrom'
        elif type == 4:
            field = 'setWhisperSCDisguiseEmoteFrom'
        # I am sleeping.
        if self.sleepFlag:
            dg = self.dclass.aiFormatUpdate('receiverIsSleeping', senderId, senderId, OTP_ALL_CLIENTS, [self.getName()])
            self.air.send(dg)
        # Tell my client the message.
        self.sendUpdate(field, [senderId, msgIndex])

    def setNextKnockKnockHeal(self, nextKnockKnockHeal):
        self.nextKnockKnockHeal = nextKnockKnockHeal

    def d_setNextKnockKnockHeal(self, nextKnockKnockHeal):
        self.sendUpdate('setNextKnockKnockHeal', [nextKnockKnockHeal])

    def b_setNextKnockKnockHeal(self, nextKnockKnockHeal):
        self.setNextKnockKnockHeal(nextKnockKnockHeal)
        self.d_setNextKnockKnockHeal(nextKnockKnockHeal)

    def getNextKnockKnockHeal(self):
        return self.nextKnockKnockHeal

    def setAllowedRewrite(self, allowedRewrite):
        self.allowedRewrite = allowedRewrite

    def d_setAllowedRewrite(self, allowedRewrite):
        self.sendUpdate('setAllowedRewrite', [allowedRewrite])

    def b_setAllowedRewrite(self, allowedRewrite):
        self.setAllowedRewrite(allowedRewrite)
        self.d_setAllowedRewrite(allowedRewrite)

    def getAllowedRewrite(self):
        return self.allowedRewrite

    def setClothingTickets(self, clothingTickets):
        self.clothingTickets = clothingTickets

    def d_setClothingTickets(self, clothingTickets):
        self.sendUpdate('setClothingTickets', [clothingTickets])

    def b_setClothingTickets(self, clothingTickets):
        self.setClothingTickets(clothingTickets)
        self.d_setClothingTickets(clothingTickets)

    def getClothingTickets(self):
        return self.clothingTickets

    def addClothingTickets(self, amount):
        clothingTicketNum = self.getClothingTickets()
        if clothingTicketNum == 255:
            return
        if clothingTicketNum + amount > 255:
            newAmount = 255
        else:
            newAmount = clothingTicketNum + amount
        self.b_setClothingTickets(newAmount)

    def removeClothingTicket(self):
        clothingTicketNum = self.getClothingTickets()
        if clothingTicketNum >= 1:
            self.b_setClothingTickets(clothingTicketNum - 1)
            return 1
        return 0

    def changeDisguiseType(self, dept, cogIndex):
        if dept < 0 or dept > 3:
            return
        if cogIndex < 0 or cogIndex > 7:
            return
        if self.getCogLevels()[dept] != ToontownGlobals.MaxCogSuitLevel:
            return
        self.cogTypes[dept] = cogIndex
        self.d_setCogTypes(self.cogTypes)
        self.sendUpdate('changeDisguiseTypeResp')
        
    def removePetHat(self):
        pet = self.air.doId2do.get(self.getPetId())
        if pet:
            pet.b_setHat(0, 0, 0)
            
    def setHouseColor(self, houseColor):
        self.houseColor = houseColor
        
    def d_setHouseColor(self, houseColor):
        self.sendUpdate('setHouseColor', [houseColor])
        
    def b_setHouseColor(self, houseColor):
        self.setHouseColor(houseColor)
        self.d_setHouseColor(houseColor)

    def getHouseColor(self):
        return self.houseColor
        
    def setTeleportGreeting(self, teleportGreeting):
        self.teleportGreeting = teleportGreeting
        
    def d_setTeleportGreeting(self, teleportGreeting):
        self.sendUpdate('setTeleportGreeting', [teleportGreeting])
        
    def b_setTeleportGreeting(self, teleportGreeting):
        self.setTeleportGreeting(teleportGreeting)
        self.d_setTeleportGreeting(teleportGreeting)
        
    def getTeleportGreeting(self):
        return self.teleportGreeting
        
    def swapTeleportGreeting(self, teleportGreeting):
        greetingLength = len(OTPLocalizer.TeleportGreetings)
        if teleportGreeting > greetingLength:
            teleportGreeting = greetingLength
        self.b_setTeleportGreeting(teleportGreeting)
        
    def setDisguiseHead(self, disguiseHead):
        self.disguiseHead = disguiseHead
        
    def d_setDisguiseHead(self, disguiseHead):
        self.sendUpdate('setDisguiseHead', [disguiseHead])
        
    def b_setDisguiseHead(self, disguiseHead):
        self.setDisguiseHead(disguiseHead)
        self.d_setDisguiseHead(disguiseHead)
        
    def getDisguiseHead(self):
        return self.disguiseHead
        
    def toggleDisguiseHead(self):
        self.b_setDisguiseHead(not self.getDisguiseHead())
        
    def setPiggyBank(self, amount, max, timeTilNext):
        self.piggyBank = (amount, max, timeTilNext)
        
    def d_setPiggyBank(self, amount, max, timeTilNext):
        self.sendUpdate('setPiggyBank', [amount, max, timeTilNext])
        
    def b_setPiggyBank(self, amount, max, timeTilNext):
        self.setPiggyBank(amount, max, timeTilNext)
        self.d_setPiggyBank(amount, max, timeTilNext)
        
    def getPiggyBank(self):
        return self.piggyBank
        
    def addPiggyBank(self, amount):
        oldAmount, oldMax, timeTilNext = self.getPiggyBank()
        amountAdded = 0
        
        if oldAmount + amount >= oldMax:
            newAmount = 0
            amountAdded = oldMax - oldAmount
            self.addMoney(oldMax)
            max = Quests.Tier2PiggyMax[self.getRewardTier()]
            timeTilNext = int(time.time() + 86400)
        else:
            max = oldMax
            newAmount = oldAmount + amount
            amountAdded = amount
            
        self.b_setPiggyBank(newAmount, max, timeTilNext)
        return amountAdded