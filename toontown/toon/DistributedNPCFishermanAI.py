...
class DistributedNPCFishermanAI(DistributedNPCToonBaseAI):
    ...
    def avatarEnter(self):
        ...
        if av.getFishingSpotId():
            self.air.moderationManager.serverKick(avId, 'Attempted to talk to a fisherman while on a fishing spot.')
            return
        self.busy = avId
        self.acceptOnce(self.air.getAvatarExitEvent(avId), self.__handleUnexpectedExit, extraArgs=[avId])
        value = av.fishTank.getTotalValue()[0]
        if value > 0:
        ...
        if sell:
            ...
            if av:
                ...
                if trophyResult == 1:
                    ...
                    movieType = NPCToons.SELL_MOVIE_TROPHY_BUCKET
                    extraArgs = [len(av.fishCollection), FishGlobals.getTotalNumFish()]
                else:
