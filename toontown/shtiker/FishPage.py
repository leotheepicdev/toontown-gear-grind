...
from toontown.fishing import DailyCatchPanel
FishPage_Tank = 0
FishPage_Collection = 1
FishPage_Trophy = 2
FishPage_DailyCatch = 3
TROPHIES_PER_ROW = 5
...
class FishPage(ShtikerPage.ShtikerPage):
    ...
    def enter(self):
        ...
        self.accept('dailyCatchChange', self.updateDailyCatch)
    ...
    def exit(self):
        ...
        if hasattr(self, 'browser'):
            self.browser.hide()
        if hasattr(self, 'dailyCatchPanel'):
            self.dailyCatchPanel.hide()
        self.ignore(localAvatar.uniqueName('fishTankChange'))
        self.ignore('dailyCatchChange')
    ...
    def load(self):
        ...
        self.dailyCatchTab = DirectButton(parent=self, relief=None, text=TTLocalizer.FishPageDailyCatch, text_pos=(-0.02, 0), text_scale=TTLocalizer.FPtankTab, text_align=TextNode.ALeft, image=gui.find('**/tabs/polySurface3'), image_pos=(-0.28, 1, -0.91), image_hpr=(0, 0, -90), image_scale=(0.033, 0.033, 0.035), image_color=normalColor, image1_color=clickColor, image2_color=rolloverColor, image3_color=diabledColor, text_fg=Vec4(0.2, 0.1, 0, 1), command=self.setMode, extraArgs=[FishPage_DailyCatch], pos=(0.92, 0, -0.7))
        self.tankTab.setPos(-0.75, 0, 0.775)
        self.collectionTab.setPos(-0.35, 0, 0.775)
        self.trophyTab.setPos(0.05, 0, 0.775)
        self.dailyCatchTab.setPos(0.45, 0, 0.775)
    ...
    def createDailyCatchPanel(self):
        if not hasattr(self, 'dailyCatchPanel'):
            self.dailyCatchPanel = DailyCatchPanel.DailyCatchPanel()
            self.dailyCatchPanel.load()
    ...
    def setMode(self, mode, updateAnyways = 0):
        ...
        if mode == FishPage_Tank:
            ...
            self.dailyCatchTab['state'] = DGG.NORMAL
            ...
            self.title['text'] = TTLocalizer.FishPageTitleDailyCatch
            if hasattr(self, 'picker'):
                self.picker.hide()
            if hasattr(self, 'browser'):
                self.browser.hide()
            if hasattr(self, 'trophyFrame'):
                self.trophyFrame.hide()
            if not hasattr(self, 'dailyCatchPanel'):
                self.createDailyCatchPanel()
            self.dailyCatchPanel.show()
            self.tankTab['state'] = DGG.NORMAL
            self.collectionTab['state'] = DGG.NORMAL
            self.trophyTab['state'] = DGG.NORMAL
            self.dailyCatchTab['state'] = DGG.DISABLED
        self.updatePage()
    ...
    def updatePage(self):
        ...
        if self.mode == FishPage_Tank:
            ...
            if hasattr(self, 'trophies'):
                trophies = base.localAvatar.getFishingTrophies()
                for trophyId in range(len(self.trophies)):
                    if trophyId in trophies:
                        self.trophies[trophyId].setNameLabel(FishGlobals.TrophyDict[trophyId][0])
                        self.trophies[trophyId].setLevel(trophyId)
                    else:
                        self.trophies[trophyId].setNameLabel(TTLocalizer.UnknownFish)
                        self.trophies[trophyId].setUnobtained(trophyId)
                    
    def updateDailyCatch(self):
        if hasattr(self, 'dailyCatchPanel'):
            self.dailyCatchPanel.update()
...
class FishingTrophy(DirectFrame):
    ...
    def __unobtained(self):
        self.top.setColorScale(0.4, 0.4, 0.4, 1)
        self.bowlTop.setColor(0.4, 0.4, 0.4, 1)
