...
class RacingTrophiesUI(DirectFrame):
    ...
    def shrink(self, index, pos):
        if base.localAvatar.getKartingTrophies()[index]:
            imageColor = Vec4(1.0, 1.0, 1.0, 1.0)
        else:
            imageColor = Vec4(0.8, 0.8, 0.8, 1.0)
        self.trophyPanels[index]['image_color'] = imageColor
    ...
    def updateTrophies(self):
        for t in range(len(self.trophyPanels)):
            ...
            self.trophyPanels[t].bind(DGG.ENTER, self.grow, extraArgs=[t])
            self.trophyPanels[t].bind(DGG.EXIT, self.shrink, extraArgs=[t])
