...
class ShtikerBook(DirectFrame, StateData.StateData):
    ...
    def __init__(self, doneEvent):
        ...
        self.pageOrder = [TTLocalizer.OptionsPageTitle,
         ...
         TTLocalizer.DailyRewardsPageTitle,
    ...
    def enter(self):
        ...
        if not self.safeMode:
            if ZoneUtil.isCogHQZone(base.localAvatar.lastHood):
                self.safeZoneButton['text'] = TTLocalizer.ShtikerBookBackToCogHQ
            else:
                self.safeZoneButton['text'] = TTLocalizer.ShtikerBookBackToPlayground
            if zone and ZoneUtil.isPlayground(zone):
                self.safeZoneButton['state'] = DGG.DISABLED
            else:
                self.safeZoneButton['state'] = DGG.NORMAL
            if base.cr.playGame.getPlaceId() == ToontownGlobals.MyEstate and base.cr.playGame.hood.loader.atMyEstate():
                self.goHomeButton['state'] = DGG.DISABLED
            elif base.housingEnabled:
                self.goHomeButton['state'] = DGG.NORMAL
    ...
    def load(self):
        ...
        self.resetFrameSize()
        self.bookOpenButton = DirectButton(image=(bookModel.find('**/BookIcon_CLSD'), bookModel.find('**/BookIcon_OPEN'), bookModel.find('**/BookIcon_RLVR')), relief=None, pos=(-0.1275, 0, 0.1275), parent=base.a2dBottomRight, scale=0.25, command=self.__open)
        self.bookCloseButton = DirectButton(image=(bookModel.find('**/BookIcon_OPEN'), bookModel.find('**/BookIcon_CLSD'), bookModel.find('**/BookIcon_RLVR2')), relief=None, pos=(-0.1275, 0, 0.1275), parent=base.a2dBottomRight, scale=0.25, command=self.__close)
    ...
    def createMiscButtons(self):
        self.buttonFrame = DirectFrame(
            ...
            image_scale=(1.9, 1, 0.18),
            image_pos=(0, 0, -0.86),
            image_color=ToontownGlobals.GlobalDialogColor[:3] + (0.8,),
            borderWidth=(0.01, 0.01),
            sortOrder=NO_FADE_SORT_INDEX)
        self.buttonFrame.setBin('fixed', 10)
        ...
        self.safeZoneButton = DirectButton(
            ...
            image3_color=(0.6, 0.6, 0.6, 1),
            pos=(0.6, 0, -0.868),
            text=TTLocalizer.ShtikerBookBackToPlayground,
        ...
        self.exitButton = DirectButton(
            ...
            image3_color=(0.6, 0.6, 0.6, 1),
            pos=(-0.6, 0, -0.868),
            text=TTLocalizer.ShtikerBookExitToontown,
            text_scale=TTLocalizer.MPsafeZoneButton,
            text_pos=(0, -0.02),
            command=self.exitToontown)
    ...
    def addPageTab(self, page, pageIndex, pageName = 'Page'):
        ...
        elif pageName == TTLocalizer.DailyRewardsPageTitle:
            iconModels = loader.loadModel('phase_3.5/models/gui/sos_textures')
            iconGeom = iconModels.find('**/switch')
            iconModels.detachNode()
    ...
    def backToSafeZone(self):
        # TODO: we should also have confirmations for toons in cog buildings + boss battles
        ...
        if hasattr(base.cr.playGame, 'getPlace'):
            try:
                ...
                pass
        if zoneId in ToontownGlobals.FacilityZones:
            facilityName = TTLocalizer.GlobalStreetNames[base.cr.playGame.getPlace().zoneId][2]
            message = TTLocalizer.MessageConfirmLeaveFacility % facilityName
            dialogClass = ToontownGlobals.getDialogClass()
            self.dialog = dialogClass(text=message, dialogName='leaveFacilityDialog', command=self.handleBackToSafeZoneConfirmation, style=YesNo)
            self.dialog.show()
        else:
            self.teleportToLastHood()
            
    def handleBackToSafeZoneConfirmation(self, status):
        self.dialog.cleanup()
        self.dialog = None
        if status == 1:
            self.teleportToLastHood()
            
    def teleportToLastHood(self):
    ...
    def goHome(self):
        ...
        if hasattr(base.cr.playGame, 'getPlace'):
            try:
                ...
                pass
        if zoneId in ToontownGlobals.FacilityZones:
            facilityName = TTLocalizer.GlobalStreetNames[base.localAvatar.zoneId][2]
            message = TTLocalizer.MessageConfirmLeaveFacility % facilityName
            dialogClass = ToontownGlobals.getDialogClass()
            self.dialog = dialogClass(text=message, dialogName='leaveFacilityDialog', command=self.handleGoHomeConfirmation, style=YesNo)
            self.dialog.show()
        else:
            self.teleportToHome()
            
    def handleGoHomeConfirmation(self, status):
        self.dialog.cleanup()
        self.dialog = None
        if status == 1:
            self.teleportToHome()       
    ...
    def exitToontown(self):
        self.dialog = TTDialog.TTGlobalDialog(doneEvent='dialogDone', message=TTLocalizer.OptionsPageExitConfirm, style=TTDialog.TwoChoice)
        self.dialog.show()
        self.doneStatus = {'mode': 'exit',
         'exitTo': 'closeShard'}
        self.accept('dialogDone', self.__handleDialogDone)

    def __handleDialogDone(self):
        status = self.dialog.doneStatus
        self.ignore('dialogDone')
        self.dialog.cleanup()
        self.dialog = None
        if status == 'ok':
