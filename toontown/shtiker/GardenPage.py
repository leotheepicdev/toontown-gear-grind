...
class GardenPage(ShtikerPage.ShtikerPage):
    ...
    def updatePage(self):
        ...
        if self.mode == GardenPage_Collection:
            ...
            if hasattr(self, 'trophies'):
                trophies = base.localAvatar.getGardenTrophies()
                for trophyId in range(len(self.trophies)):
                    if trophyId in trophies:
                        self.trophies[trophyId].setNameLabel(GardenGlobals.TrophyDict[trophyId][0])
...
class GardenTrophy(DirectFrame):
    ...
    def __unobtained(self):
        ...
        self.bowlTop.setColorScale(0.4, 0.4, 0.4, 1)
        self.metalTrophy.setColorScale(0.4, 0.4, 0.4, 1)
