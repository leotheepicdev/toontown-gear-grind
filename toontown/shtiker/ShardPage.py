from panda3d.core import ButtonMap, TextNode, Vec4
...
class ShardPage(ShtikerPage.ShtikerPage):
    ...
    def makeShardButton(self, shardId, shardName, shardPop, districtPopLimits):
        shardButtonParent = DirectFrame()
        shardButtonL = DirectButton(parent=shardButtonParent, relief=None, text=shardName, text_scale=0.06, text_align=TextNode.ALeft, text1_bg=self.textDownColor, text2_bg=self.textRolloverColor, text3_fg=self.textDisabledColor, textMayChange=0, command=self.getPopChoiceHandler(shardPop, districtPopLimits), extraArgs=[shardId])
        if self.showPop:
    ...
    def getPopColor(self, pop, districtPopLimits):
        lowPop, midPop, highPop = districtPopLimits
    ...
    def getPopText(self, pop, districtPopLimits):
        lowPop, midPop, highPop = districtPopLimits    
    ...
    def getPopChoiceHandler(self, pop, districtPopLimits):
        lowPop, midPop, highPop = districtPopLimits    
    ...
    def updateScrollList(self):
        ...
        if base.cr.welcomeValleyManager:
            curShardTuples.append((ToontownGlobals.WelcomeValleyToken, TTLocalizer.WelcomeValley[-1], 0, 0, ToontownGlobals.DefaultDistrictPopLimits))
        ...
        for i in range(len(curShardTuples)):
            shardId, name, pop, WVPop, districtPopLimits = curShardTuples[i]
            ...
            if buttonTuple == None:
                buttonTuple = self.makeShardButton(shardId, name, pop, districtPopLimits)
                self.shardButtonMap[shardId] = buttonTuple
                anyChanges = 1
            elif self.showPop:
                buttonTuple[1]['text'] = str(pop)
            else:
                buttonTuple[1]['image_color'] = self.getPopColor(pop, districtPopLimits)
                if not base.cr.productName == 'JP':
                    buttonTuple[1]['text'] = self.getPopText(pop, districtPopLimits)
                    buttonTuple[1]['command'] = self.getPopChoiceHandler(pop, districtPopLimits)
                    buttonTuple[2]['command'] = self.getPopChoiceHandler(pop, districtPopLimits)
        ...
        if buttonTuple:
            if self.showPop:
                ...
                buttonTuple[1]['image_color'] = self.getPopColor(totalWVPop, districtPopLimits)
                if not base.cr.productName == 'JP':
                    buttonTuple[1]['text'] = self.getPopText(totalWVPop, districtPopLimits)
                    buttonTuple[1]['command'] = self.getPopChoiceHandler(totalWVPop, districtPopLimits)
                    buttonTuple[2]['command'] = self.getPopChoiceHandler(totalWVPop, districtPopLimits)
        if anyChanges:
            self.regenerateScrollList()
