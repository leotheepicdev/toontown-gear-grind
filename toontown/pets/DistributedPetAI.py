...
from lib.libtoontown.pets.CPetChase import CPetChase
from lib.libtoontown.pets.CPetFlee import CPetFlee
...
class DistributedPetAI(DistributedSmoothNodeAI.DistributedSmoothNodeAI, PetLookerAI.PetLookerAI, PetBase.PetBase):
    ...
    def setDNA(self, dna):
        head, ears, nose, tail, body, color, colorScale, eyes, gender, hat = dna
        self.b_setHead(head)
        self.b_setEars(ears)
        self.b_setNose(nose)
        self.b_setTail(tail)
        ...
        self.b_setHat(hat[0], hat[1], hat[2])
    ...
    def b_setHat(self, id, texture, color):
        self.d_setHat(id, texture, color)
        self.setHat(id, texture, color)

    def d_setHat(self, id, texture, color):
        self.sendUpdate('setHat', [id, texture, color])

    def setHat(self, id, texture, color):
        self.hat = (id, texture, color)
