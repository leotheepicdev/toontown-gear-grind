...
class PetAvatarPanel(AvatarPanel.AvatarPanel):
    ...
    def __init__(self, avatar):
        ...
        if self.petIsLocal and base.localAvatar.getPetId() == avatar.doId and avatar.hat[0] != 0:
            self.makeRemoveAccessoriesButton()
        else:
            self.removeAccessories = None
    ...
    def __fillPetInfo(self, avatar):
        ...
        self.nameLabel = DirectLabel(parent=self.frame, pos=(0, 0, 5.2), relief=None, text=avatar.getName(), text_font=ToontownGlobals.getInterfaceFont(), text_fg=Vec4(0, 0, 0, 1), text_pos=(0, 0), text_scale=0.4, text_wordwrap=7.5, text_shadow=(1, 1, 1, 1))
        self.stateLabel = DirectLabel(parent=self.frame, pos=TTLocalizer.PAPstateLabelPos, relief=None, text='', text_font=ToontownGlobals.getInterfaceFont(), text_fg=Vec4(0, 0, 0, 1), text_scale=TTLocalizer.PAPstateLabel, text_wordwrap=TTLocalizer.PAPstateLabelWordwrap, text_shadow=(1, 1, 1, 1))
    ...
    def __refreshPetInfo(self, avatar):
        ...
        self.petModel.updateHatStyle(avatar.hat)
        self.stateLabel['text'] = TTLocalizer.PetMoodAdjectives[mood]
        self.nameLabel['text'] = avatar.getName()
        if self.removeAccessories:
            if avatar.hat[0] == 0:
                self.removeAccessories.removeNode()
                self.removeAccessories = None
        else:
            if self.petIsLocal and base.localAvatar.getPetId() == avatar.doId and avatar.hat[0] != 0:
                self.makeRemoveAccessoriesButton()
    ...
    def makeRemoveAccessoriesButton(self):
        guiButton = loader.loadModel('phase_3/models/gui/quit_button')
        self.removeAccessories = DirectButton(parent=self.frame, pos=(0, 0, -0.3), image=(guiButton.find('**/QuitBtn_UP'), guiButton.find('**/QuitBtn_DN'), guiButton.find('**/QuitBtn_RLVR')), 
                                 image_scale=(5.7, 6, 6), text=TTLocalizer.PetRemoveHat, text_fg=(0, 0, 0, 1), text_scale=0.4, text_pos=(0, -0.12), 
                                 text_align=TextNode.ACenter, relief=None, command=self.__handleRemoveAccessories)
        guiButton.removeNode()
        
    def __handleRemoveAccessories(self):
        base.localAvatar.sendUpdate('removePetHat')
