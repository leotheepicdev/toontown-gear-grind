...
from toontown.pets import PetDNA, AccessoryGlobals
...
from lib.libotp._constants import CFSpeech, CFThought
import random, types
...
class Pet(Avatar.Avatar):
    ...
    def generatePet(self):
        ...
        self.eyesOpen()
        self.updateHat(self.style[9])
        self.setBlend(frameBlend=True)

    def updateHat(self, hat):
        if self.hatGeom:
            self.hatGeom.removeNode()
            self.hatGeom = None
        id, texture, color = hat
        if id != 0:
            self.hatGeom = loader.loadModel(PetDNA.HatModels[id], okMissing=True)
            if self.hatGeom:
                if texture != 0:
                    texName = PetDNA.HatTextures[texture]
                    tex = loader.loadTexture(texName, okMissing=True)
                    if tex:
                        tex.setMinfilter(Texture.FTLinearMipmapLinear)
                        tex.setMagfilter(Texture.FTLinear)
                        self.hatGeom.setTexture(tex, 1)
                
                if id in AccessoryGlobals.HatTransTable:
                    hatTransTable = AccessoryGlobals.HatTransTable[id]
                    pos, hpr, scale = hatTransTable[0], hatTransTable[1], hatTransTable[2]
                else:
                    pos = (0, 0.15, 0.4)
                    scale = (0.3, 0.3, 0.3)
                    hpr = (180, 0, 0)                    
                
                self.hatGeom.reparentTo(self.find('**/joint_tongueBase'))
                self.hatGeom.setPos(*pos)
                self.hatGeom.setScale(*scale)
                self.hatGeom.setHpr(*hpr)
