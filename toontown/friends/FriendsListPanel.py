...
class FriendsListPanel(DirectFrame, StateData.StateData):
    ...
    def load(self):
        ...
        self.invisibleButton = DirectButton(parent=self, relief=None, image=matchingModel.find('**/minnieCircle'), image_scale=0.3, command=self.__handleInvisibleButton,
                                            text=('', '', '', ''), text_scale=0.06, text_fg=Vec4(1, 1, 1, 1), text_shadow=Vec4(0, 0, 0, 1), textMayChange=1,
                                            text_pos=(0.045, 0), text_align=TextNode.ARight, pos=(-0.07, 0, -0.38))
        self.setInvisibleButtonInfo(base.localAvatar.getInvisibleMode())
        self.accept('updateInvisibleButton', self.setInvisibleButtonInfo)
        gui.removeNode()
        auxGui.removeNode()
    ...
    def unload(self):
        ...
        self.isLoaded = 0
        self.exit()
        taskMgr.remove('enable-invisible-button')
        self.ignoreAll()
        del self.title
        del self.scrollList
        del self.close
    ...
    def enter(self):
        ...
        self.accept('friendInvisible', self.__friendInvisible)
    ...
    def __friendInvisible(self, doId, invisible, alert):
        if self.panelType == FLPOnline:
            self.__updateScrollList()

    def __friendOffline(self, doId, alert):
        if self.panelType == FLPOnline:
            self.__updateScrollList()
    ...
    def setInvisibleButtonInfo(self, invisible):
        if not invisible:
            self.invisibleButton.setColor(0, 1, 0, 1)
            self.invisibleButton['text'] = ('', TTLocalizer.TurnInvisible, TTLocalizer.TurnInvisible, '')
        else:
            self.invisibleButton.setColor(0.5, 0.5, 0.5, 1)
            self.invisibleButton['text'] = ('', TTLocalizer.TurnVisible, TTLocalizer.TurnVisible, '')
            
    def setInvisibleDisable(self, invisibleMode):
        self.invisibleButton['state'] = DGG.DISABLED
        if invisibleMode:
            self.invisibleButton['image3_color'] = (0, 1, 0, 0.5)
        else:
            self.invisibleButton['image3_color'] = (0.5, 0.5, 0.5, 0.5)
         
    def __handleInvisibleButton(self):
        invisibleMode = base.localAvatar.getInvisibleMode()
        self.setInvisibleButtonInfo(invisibleMode)
        self.setInvisibleDisable(invisibleMode)
        taskMgr.doMethodLater(12, self.enableInvisibleButton, 'enable-invisible-button')
        if invisibleMode:
            base.cr.toontownFriendsManager.d_toggleInvisible(0)
        else:
            base.cr.toontownFriendsManager.d_toggleInvisible(1)
        
    def enableInvisibleButton(self, task=None):
        try:
            self.invisibleButton['state'] = DGG.NORMAL
        except:
            pass
        if task:
            return task.done
