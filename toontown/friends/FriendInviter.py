...
class FriendInviter(DirectFrame):
    ...
    def enterNo(self):
        if self.toonName in TTLocalizer.SuitFriendResponses:
            textChoices = TTLocalizer.SuitFriendResponses[self.toonName]
            chosenIndex = random.randrange(len(textChoices))
            self['text'] = textChoices[chosenIndex] % self.toonName
        else:
