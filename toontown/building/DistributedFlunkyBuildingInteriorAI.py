...
from toontown.building.FlunkyBuildingPlannerAI import FlunkyBuildingPlannerAI

class DistributedFlunkyBuildingInteriorAI(DistributedObjectAI.DistributedObjectAI):
    def __init__(self, air, zoneId, bldg, toonIds):
        self.air = air
        DistributedObjectAI.DistributedObjectAI.__init__(self, air)
        self.zoneId = zoneId
        self.planner = FlunkyBuildingPlannerAI(self.zoneId)
        ...
        self.ignoreIntroductionDone = 0
        self.ignoreIntroductionBossDone = 0
        self.ignoreReserveJoinDone = 0
        self.ignoreEpilogueDone = 0
        self.toonIds = copy.copy(toonIds)
        for toonId in self.toonIds:
            if toonId is not None:
                self.__addToon(toonId)
        self.savedByMap = {}
        self.fsm = ClassicFSM.ClassicFSM(
            'DistributedFlunkyBuildingInteriorAI',
            [
                ...
                State.State('Elevator',
                            ...
                            ['Introduction', 'IntroductionBoss', 'Battle']),
                State.State('Introduction',
                            self.enterIntroduction, self.exitIntroduction,
                            ['Battle']),
                State.State('IntroductionBoss',
                            self.enterIntroductionBoss, self.exitIntroductionBoss,
                            ['Battle']),
                State.State('Battle',
    ...
    def __handleUnexpectedExit(self, toonId):
        ...
        if len(self.toons) == 0:
            ...
            if self.fsm.getCurrentState().getName() == 'Resting':
                ...
                self.bldg.destroyFlunkyBuildingInterior(self.zoneId)
    ...
    def setAvatarJoined(self):
        ...
        if self.toons.count(avId) == 0:
            self.air.writeServerEvent('suspicious', avId, 'DistributedCogTowerInteriorAI.setAvatarJoined from toon not in %s.' % self.toons)
    ...
    def elevatorDone(self):
        ...
        if self.__allToonsResponded() and self.ignoreElevatorDone == 0:
            if self.currentFloor == self.topFloor:
                self.b_setState('IntroductionBoss')
            else:
                self.b_setState('Introduction')
            
    def introductionDone(self):
        toonId = self.air.getAvatarIdFromSender()
        if self.ignoreResponses == 1:
            return
        elif self.fsm.getCurrentState().getName() != 'Introduction':
            self.notify.warning('elevatorDone() - in state: %s' % self.fsm.getCurrentState().getName())
            return
        elif self.toons.count(toonId) == 0:
            self.notify.warning('elevatorDone() - toon not in toon list: %d' % toonId)
            return
        self.responses[toonId] += 1
        if self.__allToonsResponded() and self.ignoreIntroductionDone == 0:
            self.b_setState('Battle')
            
    def introductionBossDone(self):
        toonId = self.air.getAvatarIdFromSender()
        if self.ignoreResponses == 1:
            return
        elif self.fsm.getCurrentState().getName() != 'IntroductionBoss':
            self.notify.warning('elevatorDone() - in state: %s' % self.fsm.getCurrentState().getName())
            return
        elif self.toons.count(toonId) == 0:
            self.notify.warning('elevatorDone() - toon not in toon list: %d' % toonId)
    ...
    def epilogueDone(self):
        toonId = self.air.getAvatarIdFromSender()
        if self.ignoreResponses == 1:
            return
        elif self.fsm.getCurrentState().getName() != 'Epilogue':
            self.notify.warning('elevatorDone() - in state: %s' % self.fsm.getCurrentState().getName())
            return
        elif self.toons.count(toonId) == 0:
            self.notify.warning('elevatorDone() - toon not in toon list: %d' % toonId)
    ...
    def __createFloorBattle(self):
        if self.currentFloor == self.topFloor:
            ...
            maxSuits = 4
            bossBattle = 0
        self.battle = DistributedBattleBldgAI.DistributedBattleBldgAI(self.air, self.zoneId, self.__handleRoundDone, self.__handleBattleDone, maxSuits = maxSuits, bossBattle = bossBattle)
        self.battle.suitsKilled = self.suitsKilled
        self.battle.suitsKilledPerFloor = self.suitsKilledPerFloor
        self.battle.avId2SuitsKilled = self.avId2SuitsKilled
    ...
    def __handleBattleDone(self, zoneId, toonIds):
        if len(toonIds) == 0:
            taskName = self.taskName('deleteInterior')
            taskMgr.doMethodLater(42, self.__doDeleteInterior, taskName)
        else:
            if self.currentFloor == self.topFloor:
    ...
    def enterIntroduction(self):
        self.__resetResponses()
        self.d_setState('Introduction')
        self.timer.startCallback(22, self.__serverIntroductionDone)
        
    def __serverIntroductionDone(self):
        self.ignoreIntroductionDone = 1
        self.b_setState('Battle')
    ...
    def enterIntroductionBoss(self):
        self.__resetResponses()
        self.d_setState('IntroductionBoss')
        self.timer.startCallback(44, self.__serverIntroductionBossDone)
        
    def __serverIntroductionBossDone(self):
        self.ignoreIntroductionBossDone = 1
        self.b_setState('Battle')
    ...
    def enterEpilogue(self):
        self.__resetResponses()
        self.d_setState('Epilogue')
        self.timer.startCallback(40, self.__serverEpilogueDone)
        
    def __serverEpilogueDone(self):
        self.ignoreEpilogueDone = 1
        self.b_setState('Reward')
    ...
    def enterReward(self):
        for toonId in self.toons:
            ...
            if toon:
                toon.addMoney(100)
                simbase.air.questManager.toonKilledTomBuilding(toon, self.toons)
