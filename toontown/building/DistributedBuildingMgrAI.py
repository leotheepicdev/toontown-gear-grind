...
from . import CogBuildingGlobals, CogBuildingGlobalsAI, DistributedBuildingAI, HQBuildingAI, GagshopBuildingAI, PetshopBuildingAI
...
class DistributedBuildingMgrAI:
    ...
    serverDatafolder = simbase.config.GetString('building-data-backup-folder', '')
    ...
    def getBuildingTracks(self, blockNumber):
        return self.__buildings[blockNumber].tracks
    ...
    def newBuilding(self, blockNumber, blockData=None):
        ...
        if blockData:
            building.tracks = blockData.get('tracks', ['c'])
            ...
            if 'suitBldgType' in blockData:
                if self.air.wantUndergroundBldgs:
                    building.suitBldgType = int(blockData['suitBldgType'])
                else:
                    building.suitBldgType = 0
            else:
                if self.air.wantUndergroundBldgs and random.random() <= CogBuildingGlobalsAI.UNDERGROUND_CHANCE:
                    building.suitBldgType = CogBuildingGlobals.SUIT_BLDG_UNDERGROUND
                else:
                    building.suitBldgType = CogBuildingGlobals.SUIT_BLDG_NORMAL
            ...
            if blockData['state'] == 'suit':
                self.air.bldgMgr.addBuilding(ZoneUtil.getCanonicalZoneId(self.branchID))
