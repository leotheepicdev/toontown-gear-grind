...
class DistributedBoardingPartyAI(DistributedObjectAI.DistributedObjectAI, BoardingPartyBase.BoardingPartyBase):
    ...
    def requestInvite(self, inviteeId):
        ...
        if not isinstance(invitee, DistributedToonAI):
            kickMessage = 'Tried to request invite to a non-toon object!'
            simbase.air.moderationManager.serverKick(inviterId, kickMessage)
            return
        ...
        if inviterId in self.avIdDict:
            ...
            self.groupListDict[leaderId] = [[leaderId], [inviteeId], [], 0]
    ...
    def requestGoToSecondTime(self, elevatorId):
        ...
        practice = self.getGroupPractice(callerId)
        if self.testGoButtonRequirements(callerId, elevatorId):
            for avId in avList:
                self.sendUpdateToAvatarId(avId, 'acceptGoToSecondTime', [elevatorId])

            THREE_SECONDS = 3.0
            taskMgr.doMethodLater(THREE_SECONDS, self.sendAvatarsToDestinationTask, self.uniqueName('sendAvatarsToDestinationTask'), extraArgs=[elevatorId, avList, practice], appendTask=True)

    def sendAvatarsToDestinationTask(self, elevatorId, avList, practice, task):
        ...
        if len(avList):
            if elevatorId in self.elevatorIdList:
                ...
                if elevator:
                    ...
                    elevator.sendAvatarsToDestination(avList, practice)
        return Task.done
    ...
    def addToGroup(self, leaderId, inviteeId, post=1):
        ...
        if group:
            ...
            if post:
                ...
                self.sendUpdate('postGroupInfo', [leaderId, group[0], group[1], group[2], group[3]])
    ...
    def requestPractice(self, practice):
        leaderId = self.air.getAvatarIdFromSender()
        if not self.isGroupLeader(leaderId):
            self.air.writeServerEvent('suspicious: ', leaderId, 'has requested practice when not leader!')
            return
        if not self.getPracticeEnabled():
            self.air.writeServerEvent('suspicious: ', leaderId, 'has tried to request practice when not enabled')
            return
        ...
        if group:
            ...
            group[3] = practice
            for avId in memberList:
                self.sendUpdateToAvatarId(avId, 'postPractice', [practice])
