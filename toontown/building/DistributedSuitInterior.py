...
from . import ElevatorUtils, CogBuildingGlobals
...
class DistributedSuitInterior(DistributedObject.DistributedObject):
    ...
    def generate(self):
        ...
        self.elevatorModelBasement = loader.loadModel('phase_4/models/modules/elevator')
        self.leftDoorBasement = self.elevatorModelBasement.find('**/left-door')
        self.rightDoorBasement = self.elevatorModelBasement.find('**/right-door')

    def announceGenerate(self):
        if self.suitBldgType == CogBuildingGlobals.SUIT_BLDG_NORMAL:
            ...
            self.waitMusic = base.loader.loadMusic('phase_7/audio/bgm/encntr_toon_winning_indoor_underground.ogg')
            self.elevatorMusic = base.loader.loadMusic('phase_7/audio/bgm/tt_elevator_underground.ogg')
        DistributedObject.DistributedObject.announceGenerate(self)

    def setElevatorLights(self, elevatorModel):
        cfloor = self.currentFloor
        npc = elevatorModel.findAllMatches('**/floor_light_?;+s')
        for i in range(npc.getNumPaths()):
            np = npc.getPath(i)
            floor = int(np.getName()[-1:]) - 1
            if floor == cfloor:
                if self.getSuitBldgType() == CogBuildingGlobals.SUIT_BLDG_UNDERGROUND:
                    np.setColor(LIGHT_ON_COLOR_UNDERGROUND)
                else:
    ...
    def setSuitBldgType(self, suitBldgType):
    ...
    def __playElevator(self, ts, name, callback):
        ...
        if self.floorModel:
            self.floorModel.removeNode()
        bossFloor =  self.numFloors - 1
        wantLava = 0
            
        if self.getSuitBldgType() == CogBuildingGlobals.SUIT_BLDG_UNDERGROUND:
            if self.currentFloor == 0:
                self.floorModel = loader.loadModel('phase_7/models/modules/suit_interior_underground')
                SuitHs = self.BottomFloor_SuitHs
                SuitPositions = self.BottomFloor_SuitPositions
            elif self.currentFloor == bossFloor:
                self.floorModel = loader.loadModel('phase_7/models/modules/boss_suit_office_underground')
                SuitHs = self.BossOffice_SuitHs
                SuitPositions = self.BossOffice_SuitPositions
            else:
                self.floorModel = loader.loadModel('phase_7/models/modules/cubicle_underground')
       ...
       # if self.currentFloor == 0:
        #    elevBasement = self.floorModel.find('**/elevator-basement')
       ...
       # if self.currentFloor == 0:
        #    self.elevatorModelBasement.reparentTo(elevBasement)
         #   self.leftDoorBasement.setPos(3.5, 0, 0)
          #  self.rightDoorBasement.setPos(-3.5, 0, 0)
        ...
        if wantLava:
            seq = Sequence(Wait(4), Func(self.startLavaSeq))
            seq.start()

    def startLavaSeq(self):
        lava = self.floorModel.find('**/Lava')
        lavapos = lava.getPos()
        lavay = lavapos[2]
        self.lavaSeq = Sequence(
                           LerpPosInterval(lava, 6, (lavapos[0], lavapos[1], lavay + 0.8), startPos=lavapos),
                           LerpPosInterval(lava, 6, lavapos, startPos=(lavapos[0], lavapos[1], lavay + 0.8))
                       )
    ...
    def enterElevator(self, ts = 0):
        ...
        self.setElevatorLights(self.elevatorModelBasement)
