from panda3d.core import DecalEffect, DepthOffsetAttrib, Lens, NodePath, Point3, TextNode, VBase4, Vec3
...
class DistributedBuilding(DistributedObject.DistributedObject):
    ...
    def setSuitData(self, tracks, difficulty, numFloors, suitBldgType):
        self.tracks = tracks
    ...
    def loadElevator(self, newNP, cogdo = False):
        ...
        extraTrackNum2Pos = {
                        2: [(0, 6.79, 4.7)],
                        3: [(-0.5, 6.79, 4.7), (0.5, 6.79, 4.7)],
                        4: [(-1, 6.79, 4.7), (0, 6.79, 4.5), (1, 6.79, 4.7)]
                       }
        ...
        if self.mode == 'suit':
            ...
            for i in range(len(self.tracks)):
                ...
                if i == 0:
                    ...
                    pos = extraTrackNum2Pos[len(self.tracks)][i-1]
                    scale = 1
                if dept == 'c':
    ...
    def setupSuitBuilding(self, nodePath):
        ...
        suitNP = dnaStore.findNode('suit_landmark_' + self.tracks[0] + str(self.numFloors))
        ...
        if len(self.tracks) == 1:
            buildingTitle += '\n%s' % SuitDNA.getDeptFullname(self.tracks[0])
        else:
            buildingTitle += '\n%s %s' % (SuitDNA.getDeptFullname(self.tracks[0]), TTLocalizer.BuildingMajority)
        if self.suitBldgType == SUIT_BLDG_UNDERGROUND:
            buildingTitle += ' %s' % TTLocalizer.BuildingUnderground
        ...
        if self.suitBldgType == SUIT_BLDG_UNDERGROUND:
            backgroundNP.setColorScale(0.6, 0.6, 0.6, 1)
    ...
    def setupCogdo(self, nodePath):
        dnaStore = self.cr.playGame.dnaStore
        suitNP = dnaStore.findNode(FO_DICT[self.tracks[0]])
        zoneId = dnaStore.getZoneFromBlockNumber(self.block)
        zoneId = ZoneUtil.getTrueZoneId(zoneId, self.interiorZoneId)
        newParentNP = base.cr.playGame.hood.loader.zoneDict[zoneId]
        suitBuildingNP = suitNP.copyTo(newParentNP)
