...
class DistributedBossElevator(DistributedElevatorExt.DistributedElevatorExt):
    ...
    def handleToonBoarded(self, avatar):
        elevator = self.getPlaceElevator()
        if not elevator or not elevator.practiceButton:
            return
        if avatar == base.localAvatar and not avatar.readyForPromotion(ELEVATOR_2_DEPT[self.type]):
            elevator.promotionNotReadyText.show()
