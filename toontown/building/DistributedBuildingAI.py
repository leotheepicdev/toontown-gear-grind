...
from . import DistributedToonInteriorAI, DistributedToonHallInteriorAI, DistributedSuitInteriorAI, DistributedDoorAI, DoorTypes, DistributedElevatorExtAI, DistributedKnockKnockDoorAI, CogBuildingGlobals, CogBuildingGlobalsAI, FADoorCodes
...
class DistributedBuildingAI(DistributedObjectAI.DistributedObjectAI):
    ...
    def __init__(self, air, blockNumber, zoneId, trophyMgr):
        ...
        self.difficulty = 1
        self.numFloors = 0
        self.suitBldgType = CogBuildingGlobals.SUIT_BLDG_NORMAL
        self.savedBy = None
    ...
    def getPickleData(self):
        pickleData = {'state': str(self.fsm.getCurrentState().getName()), 'block': str(self.block), 'tracks': self.tracks, 'difficulty': str(self.difficulty), 'numFloors': str(self.numFloors), 'suitBldgType': str(self.suitBldgType), 'savedBy': self.savedBy, 'becameSuitTime': self.becameSuitTime}
        return pickleData

    def _getMinMaxFloors(self, difficulty):
        return CogBuildingGlobalsAI.CogBuildingInfo[1][difficulty][0]

    def suitTakeOver(self, suitTrack, wantMixedBuildings=True):
        if not self.isToonBlock():
            return
        if self.suitPlannerExt:
            self.air.bldgMgr.addBuilding(self.suitPlannerExt.zoneId)
        self.updateSavedBy(None)
        difficulty = self.air.bldgMgr.pickBuildingDifficulty(self.suitPlannerExt.zoneId)
        minFloors, maxFloors = self._getMinMaxFloors(difficulty)
        numFloors = random.randint(minFloors, maxFloors)
        
        if wantMixedBuildings and random.random() <= CogBuildingGlobalsAI.MIXED_CHANCE:
            lenChoice = random.randint(2, 4)
            availableTracks = ['c', 'l', 's', 'm']
            tracks = []
            for i in range(lenChoice):
                if i == 0:
                    tracks.append(suitTrack)
                    availableTracks.remove(suitTrack)
                else:
        ...
        if self.air.wantUndergroundBldgs and random.random() <= CogBuildingGlobalsAI.UNDERGROUND_CHANCE:
            self.suitBldgType = CogBuildingGlobals.SUIT_BLDG_UNDERGROUND
        else:
            self.suitBldgType = CogBuildingGlobals.SUIT_BLDG_NORMAL
        self.becameSuitTime = time.time()
        self.fsm.request('clearOutToonInterior')

    def cogdoTakeOver(self, suitTrack):
        # TODO: make custom difficulties for field offices due to them being different than cog bldgs
        if not self.isToonBlock():
            return
        if suitTrack in ['c', 'm']:
            # We put this here for now because we have not implemented Bossbot Field Offices.
            suitTrack = random.choice(['s', 'l'])
        if self.suitPlannerExt:
            self.air.bldgMgr.addBuilding(self.suitPlannerExt.zoneId)
        self.updateSavedBy(None)
        numFloors = self.FieldOfficeNumFloors
        self.tracks = [suitTrack]
        self.difficulty = self.air.bldgMgr.pickBuildingDifficulty(self.suitPlannerExt.zoneId)
        self.numFloors = numFloors
        self.becameSuitTime = time.time()
    ...
    def getSuitData(self):
        return [self.tracks, self.difficulty, self.numFloors, self.suitBldgType]
    ...
    def enterWaitForVictors(self, victorList, savedBy):
        ...
        for t in victorList:
            ...
            if t:
                toon = self.getToon(t)
            if toon != None:
                self.air.questManager.toonKilledBuilding(toon, self.tracks, self.difficulty, self.numFloors, self.zoneId, activeToons)
    ...
    def enterWaitForVictorsFromCogdo(self, victorList, savedBy):
        ...
        if self.buildingDefeated:
            for t in victorList:
                ...
                if t:
                    toon = self.getToon(t)
                    self.air.writeServerEvent('buildingDefeated', t, '%s|%s|%s|%s' % (self.tracks, self.numFloors, self.zoneId, victorList))
                if toon != None:
    ...
    def enterBecomingSuit(self):
        self.sendUpdate('setSuitData', [self.tracks, self.difficulty, self.numFloors, self.suitBldgType])
        self.d_setState('becomingSuit')
        name = self.taskName(str(self.block) + '_becomingSuit-timer')
        taskMgr.doMethodLater(CogBuildingGlobals.TO_SUIT_BLDG_TIME, self.becomingSuitTask, name)
    ...
    def exitSuit(self):
        self.air.bldgMgr.removeBuilding(self.suitPlannerExt.zoneId)
        if hasattr(self, 'elevator'):
            self.elevator.requestDelete()
            del self.elevator
    ...
    def enterBecomingCogdo(self):
        ...
        taskMgr.doMethodLater(CogBuildingGlobals.TO_SUIT_BLDG_TIME, self.becomingCogdoTask, name)
    ...
    def enterCogdo(self):
        ...
        self.planner = SuitPlannerCogdoInteriorAI(self._cogdoLayout, self.difficulty, self.tracks, self.suitBldgType, interiorZoneId)
