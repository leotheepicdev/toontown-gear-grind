...
from .SuitPlannerInteriorAI import SuitPlannerInteriorAI
import copy
...
class DistributedSuitInteriorAI(DistributedObjectAI.DistributedObjectAI):
    ...
    def __init__(self, air, elevator):
        ...
        self.suitBldgType = elevator.bldg.suitBldgType
    ...
    def enterElevator(self):
        toonLen = len(self.getToons())
        if toonLen == 0:
            self.fsm.request('Off')
            return
        if self.currentFloor == 0:
            self.planner = SuitPlannerInteriorAI(self.numFloors, self.elevator.bldg.difficulty, self.elevator.bldg.tracks, self.suitBldgType, self.zoneId, toonLen=toonLen)
    ...
    def __createFloorBattle(self):
        if self.currentFloor == self.topFloor:
            ...
            bossBattle = 0
        self.battle = DistributedBattleBldgAI.DistributedBattleBldgAI(self.air, self.zoneId, self.__handleRoundDone, self.__handleBattleDone, bossBattle=bossBattle, suitBldgType=self.getSuitBldgType())
