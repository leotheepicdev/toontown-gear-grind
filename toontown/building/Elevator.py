...
class Elevator(StateData.StateData):
    ...
    def __init__(self, elevatorState, doneEvent, distElevator):
        ...
        self.promotionNotReadyText = None
    ...
    def enableExitButton(self):
        self.exitButton = ToontownButton(relief=None, text=TTLocalizer.ElevatorHopOff, text_fg=(0.9, 0.9, 0.9, 1), text_pos=(0, -0.23), text_scale=TTLocalizer.EexitButton, image=(self.upButton, self.downButton, self.rolloverButton), image_color=(0.5, 0.5, 0.5, 1), image_scale=(20, 1, 11), pos=(0, 0, 0.8), scale=0.15, command=lambda self = self: self.fsm.request('requestExit'))
        self.practiceButton = ToontownButton(parent=base.a2dBottomRight, relief=None, text=TTLocalizer.ElevatorPractice, text_fg=(0.9, 0.9, 0.9, 1), text_pos=(0, -0.23), text_scale=TTLocalizer.EexitButton, image=(self.upButton, self.downButton, self.rolloverButton), image_color=(0.5, 0.5, 0.5, 1), image_scale=(20, 1, 11), pos=(-0.2, 0, 0.08), scale=0.1, command=self.__handlePracticeButton)
        self.practiceButton.hide()
        self.promotionNotReadyText = DirectLabel(parent=base.a2dBottomCenter, relief=None, text=TTLocalizer.BossElevatorPromotionNotReady, text_font=ToontownGlobals.getMinnieFont(), text_fg=(0.9, 0.9, 0.9, 1), text_scale=0.12, pos = (0, 0, 0.15))
        self.promotionNotReadyText.hide()        
    ...
    def disableExitButton(self):
        ...
        if self.practiceButton:
            self.practiceButton.destroy()
            self.practiceButton = None
        if self.promotionNotReadyText:
            self.promotionNotReadyText.destroy()
            self.promotionNotReadyText = None
