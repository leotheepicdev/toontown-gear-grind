...
class DistributedDoor(DistributedObject.DistributedObject, DelayDeletable):
    ...
    def setTriggerName(self):
        ...
        try:
            ...
            pass
        if self.doorType in [DoorTypes.EXT_STANDARD, DoorTypes.EXT_COGHQ, DoorTypes.INT_COGHQ]:
            doorTrigger = building.find('**/' + self.getTriggerName())
            if not doorTrigger.isEmpty():
                if '_gag_shop_' in building.getName():
                    doorTrigger.setY(doorTrigger.getY() + 1.00)
                else:
                    doorTrigger.setY(doorTrigger.getY() - 1.00)
                doorTrigger.setZ(doorTrigger.getZ() + 0.25)
        if self.doorType == DoorTypes.INT_STANDARD:
            doorTrigger = building.find('**/' + self.getTriggerName())
            if not doorTrigger.isEmpty():
                if 'door_origin' in building.getName():
                    doorTrigger.setY(doorTrigger.getY() - 1.00)
                else:
                    if 'gagShop_interior_' or 'PetShopInterior' in building.getName():
                        doorTrigger.setY(doorTrigger.getY() + 1.00)
                    else:
                        doorTrigger.setY(doorTrigger.getY() - 1.00)
                    doorTrigger.setZ(doorTrigger.getZ() + 0.25)
        if self.doorType == DoorTypes.INT_HQ:
            ...
            if not doorTrigger.isEmpty():
                if self.doorIndex == 1:
                    doorTrigger.setY(doorTrigger.getY() - 1)
                    doorTrigger.setZ(doorTrigger.getZ() + 3)
                else:
                    doorTrigger.setY(doorTrigger.getY() + 1)
                    doorTrigger.setZ(doorTrigger.getZ() + 3)
