...
NAME_POSITION = (0, 0, 0.26)
DELETE_POSITION = (0.27, 0, -0.26)
LAFF_METER_POSITION = (-0.286, 0, 0.11)
LAFF_METER_POSITION_MONKEY = (-0.276, 0, 0.11)

class AvatarChoice(ToontownButton):
    ...
    def __init__(self, av = None, position = 0, paid = 0, okToLockout = 1):
        ...
        ToontownButton.__init__(self, relief=None, text='', text_font=ToontownGlobals.getSignFont(), anim='stretchslow')
        ...
        if self.mode is not AvatarChoice.MODE_LOCKED:
            if not av:
                ...
                self.allowedRewrite = av.allowedRewrite
                self.hp = av.hp
                self.maxHp = av.maxHp
        ...
        self.buttonBgs = []
        self.buttonBgs.append(self.pickAToonGui.find('**/pickAToon/**/red'))
        self.buttonBgs.append(self.pickAToonGui.find('**/pickAToon/**/green'))
        self.buttonBgs.append(self.pickAToonGui.find('**/pickAToon/**/purple'))
        self.buttonBgs.append(self.pickAToonGui.find('**/pickAToon/**/blue'))
        self.buttonBgs.append(self.pickAToonGui.find('**/pickAToon/**/pink'))
        self.buttonBgs.append(self.pickAToonGui.find('**/pickAToon/**/yellow'))
        ...
        if self.mode is AvatarChoice.MODE_LOCKED:
            ...
            self.nameText = DirectLabel(parent=self, relief=None, scale=0.08, pos=NAME_POSITION, text=self.name, text_fg=(1, 1, 1, 1), text_shadow=(0, 0, 0, 1), text_wordwrap=8, text_font=ToontownGlobals.getToonFont(), state=DGG.DISABLED)
            if self.approvedName != '':
                self.nameText['text'] = self.approvedName
            guiButton = loader.loadModel('phase_3/models/gui/quit_button')
            self.nameYourToonButton = ToontownButton(parent=self, relief=None, anim='stretch', image=(guiButton.find('**/QuitBtn_UP'), guiButton.find('**/QuitBtn_DN'), guiButton.find('**/QuitBtn_RLVR')), text=(TTLocalizer.AvatarChoiceNameYourToon, TTLocalizer.AvatarChoiceNameYourToon, TTLocalizer.AvatarChoiceNameYourToon), text_fg=(1, 1, 1, 1), text_shadow=(0, 0, 0, 1), text_scale=0.15, text_pos=(0, 0.03), text_font=ToontownGlobals.getInterfaceFont(), pos=(-0.19, 0, -0.3), scale=0.45, image_scale=(2, 1, 3), command=self.__handleNameYourToon)
            ...
            elif self.allowedRewrite:
                self.nameYourToonButton['text'] = TTLocalizer.AvatarChoiceRewrite
                self.nameYourToonButton.show()
                self.statusText['text'] = ''
            else:
                self.nameYourToonButton.hide()
            ...
            self.laffMeter = LaffMeter.LaffMeter(self.dna, self.hp, self.maxHp)
            self.laffMeter.reparentTo(self)
            self.laffMeter.setPos(LAFF_METER_POSITION_MONKEY if self.dna.getAnimal() == 'monkey' else LAFF_METER_POSITION)
            self.laffMeter.setScale(0.05)
            self.laffMeter.start()
            trashcanGui = loader.loadModel('phase_3/models/gui/trashcan_gui')
            self.deleteButton = ToontownButton(parent=self, image=(trashcanGui.find('**/TrashCan_CLSD'), trashcanGui.find('**/TrashCan_OPEN'), trashcanGui.find('**/TrashCan_RLVR')), text=('', TTLocalizer.AvatarChoiceDelete, TTLocalizer.AvatarChoiceDelete), text_fg=(1, 1, 1, 1), text_shadow=(0, 0, 0, 1), text_scale=0.15, text_pos=(0, -0.1), text_font=ToontownGlobals.getInterfaceFont(), relief=None, pos=DELETE_POSITION, scale=0.45, command=self.__handleDelete)
