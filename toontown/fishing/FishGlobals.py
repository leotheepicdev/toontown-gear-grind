...
DailyCatchMultiplier = 2
DailyCatchExtraChance = 0.1
MAX_RARITY = 10
GlobalRarityDialBase = 3.6
FishingAngleMax = 50.0
OVERALL_VALUE_SCALE = 13
...
def getRandomFishVitals(zoneId, rodId, rNumGen = None):
    ...
    if game.process == 'server':
        dailyCatch = simbase.air.newsManager.getDailyCatch()
        d_genus = dailyCatch[0]
        d_species = dailyCatch[1]
        if d_genus == None or d_species == None:
            pass
        else:
            if fishList == None:
                pass
            else:
                if (d_genus, d_species) not in fishList:
                    if random.random() <= DailyCatchExtraChance:
                        fishList.append((d_genus, d_species))
    if fishList:
        if rNumGen is None:
