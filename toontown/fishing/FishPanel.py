...
class FishPanel(DirectFrame):
    ...
    def load(self, dump=False):
        ...
        self.extraLabel = DirectLabel(parent=self, relief=None, state=DGG.NORMAL, text='', text_fg=(1, 0.5, 0, 1), text_font=ToontownGlobals.getSignFont(), text_scale=0.08, pos=(0, 0, 0.26))
        self.dailyCatch = DirectLabel(parent=self, pos=(0, 0, -0.4), relief=None, state=DGG.NORMAL, text=TTLocalizer.FishCatchOfTheDay, text_fg=(1, 0, 0, 1), text_font=ToontownGlobals.getInterfaceFont(), text_scale=0.04, text_shadow=(0, 0, 0, 1))
        ...
        if dump:
            self.dump =  DirectButton(parent=self, pos=(-0.275, 0, -0.375), relief=None, state=DGG.NORMAL, image=(buttons.find('**/CloseBtn_UP'), buttons.find('**/CloseBtn_DN'), buttons.find('**/CloseBtn_Rllvr')), image_color=(1, 0, 0, 1), image_scale=(0.6, 1, 0.6), text=('', TTLocalizer.FishGuiDump, TTLocalizer.FishGuiDump, ''), text_scale=0.04, text_pos=(0.04, -0.01), text_align=TextNode.ALeft, command=self.handleDump)
        buttons.removeNode()
        self.photo = FishPhoto.FishPhoto(parent=self)
        self.update(self.fish)
        
    def handleDump(self):
        base.localAvatar.d_dumpFish()
        self.handleCancel()
    ...
    def show(self, code = FishGlobals.FishItem):
        ...
        if hasattr(base.cr, 'newsManager'):
            ...
            if d_genus == self.fish.getGenus() and d_species == self.fish.getSpecies():
                self.dailyCatch.show()
