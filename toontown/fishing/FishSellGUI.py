...
class FishSellGUI(DirectFrame):
    ...
    def __updateFishValue(self):
        ...
        num = len(fishTank)
        value, dailyCatch = fishTank.getTotalValue()
        if dailyCatch:
            text = TTLocalizer.FishTankValueDailyCatch
        else:
            text = TTLocalizer.FishTankValue
        self['text'] = text % {'name': base.localAvatar.getName(),
         'num': num,
         'value': value}
        self.setText()
