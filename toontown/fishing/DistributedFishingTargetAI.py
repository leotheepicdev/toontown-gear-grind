...
class DistributedFishingTargetAI(DistributedNodeAI):
    notify = directNotify.newCategory('DistributedFishingTargetAI')
    ...
    def delete(self):
        taskMgr.remove('update-fishing-target-{0}'.format(self.doId))
    ...
    def __updateState(self, task = None):
        ...
        taskMgr.doMethodLater(time + random.uniform(5, 2.5), self.__updateState, 'update-fishing-target-{0}'.format(self.doId))
