...
class QuestPoster(DirectFrame):
    ...
    def __init__(self, parent = aspect2d, **kw):
        ...
        self.questProgress = DirectWaitBar(parent=self.questFrame, relief=DGG.SUNKEN, frameSize=(-0.95,
         0.95,
         -0.1,
         0.12), borderWidth=(0.025, 0.025), scale=0.2, frameColor=(0.945, 0.875, 0.706, 1.0), barColor=(0.5, 0.7, 0.5, 1), text='0/0', text_scale=0.19, text_fg=(0.05, 0.14, 0.4, 1), text_align=TextNode.ACenter, text_pos=(0, -0.04), pos=(0, 0, -0.165))
    ...
    def showChoicePoster(self, questId, fromNpcId, toNpcId, rewardId, callback):
        ...
        if not hasattr(self, 'chooseButton'):
            ...
            self.chooseButton = ToontownButton(parent=self.questFrame, relief=None, anim='stretch', image=(guiButton.find('**/QuitBtn_UP'), guiButton.find('**/QuitBtn_DN'), guiButton.find('**/QuitBtn_RLVR')), image_scale=(0.7, 1, 1), text=TTLocalizer.QuestPageChoose, text_scale=0.06, text_pos=(0, -0.02), pos=(0.285, 0, 0.245), scale=0.65)
    ...
    def update(self, questDesc):
        ...
        if quest.getType() == Quests.DeliverGagQuest or quest.getType() == Quests.DeliverItemQuest:
            ...
            if not fComplete:
                ...
                rIconGeomScale = 1
        elif quest.getType() == Quests.TutorialTomBuildingQuest:
            frameBgColor = 'blue'
            lIconGeom = loader.loadModel('phase_4/models/modules/suit_landmark_corp')
            if lIconGeom:
                self.loadElevator(lIconGeom, 0, False)
                lIconGeom.setH(180)
                self.fitGeometry(lIconGeom, fFlip=0)
                lIconGeomScale = IMAGE_SCALE_SMALL
            else:
                lIconGeomScale = 0.13
            if not fComplete:
                infoText = TTLocalizer.GlobalStreetNames[2400][2]
            ...
            frameBgColor = 'blue'
            lIconGeom = loader.loadModel('phase_11/models/lawbotHQ/LB_bookshelfA')
            lIconGeomScale = 0.014
            lQuestY = -0.05
            if not fComplete:
                infoText = quest.getLocationName()
                if infoText == '':
                    infoText = TTLocalizer.QuestPosterAnywhere
        elif quest.getType() == Quests.CountryClubQuest:
            frameBgColor = 'blue'
            lIconGeom = loader.loadModel('phase_12/models/bossbotHQ/Coggolf_cart3')
            lIconGeomScale = 0.014
            lQuestY = -0.05
            if not fComplete:
                infoText = quest.getLocationName()
        ...
        self.headline['text_scale'] = headlineScale
        ...
        self.lQuestIcon['geom_pos'] = (0, 10, lQuestY)
        if lIconGeom:
    ...
    def showDeleteButton(self, questDesc):
        ...
        self.deleteButton = ToontownButton(parent=self.questFrame, image=(trashcanGui.find('**/TrashCan_CLSD'), trashcanGui.find('**/TrashCan_OPEN'), trashcanGui.find('**/TrashCan_RLVR')), text=('', TTLocalizer.QuestPosterDeleteBtn, TTLocalizer.QuestPosterDeleteBtn), text_fg=(1, 1, 1, 1), text_shadow=(0, 0, 0, 1), text_scale=0.18, text_pos=(0, -0.12), relief=None, pos=(0.3, 0, 0.145), scale=0.3, command=self.onPressedDeleteButton, extraArgs=[questDesc])
