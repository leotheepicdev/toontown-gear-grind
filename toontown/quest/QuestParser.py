...
from lib.libotp import _constants
from lib.libotp._constants import CFReversed, CFSpeech, CFTimeout
        ...
        for arg in args:
            if type(arg) == type(0):
                ...
                if len(arg) > 2 and arg[:2] == 'CF':
                    extraChatFlags = getattr(_constants, arg)
                else:
...
if AppRunnerGlobal.appRunner:
    ...
    searchPath.appendDirectory(Filename.fromOsSpecific('resources/phase_3/etc'))
    searchPath.appendDirectory(Filename('.'))

    if sys.platform == 'android':
        searchPath.appendDirectory(Filename.fromOsSpecific(currentDirectory + '/toontown/src/quest'))
