...
class TTChatInputSpeedChat(DirectObject.DirectObject):
    ...
    def __init__(self, chatMgr):
        ...
        listenForSCEvent(TTSpeedChatGlobals.TTSCCogEmoteMsgEvent, self.handleCogEmoteMsg)
        ...
        listenForSCEvent(TTSpeedChatGlobals.TTSCGreetingMsgEvent, self.handleGreetingMsg)
    ...
    def createSpeedChat(self):
        ...
        structure.append([TTSCGreetingMenu, OTPLocalizer.SCMenuGreeting])
    ...
    def handleStaticTextMsg(self, textId, cfType):
        if self.whisperAvatarId is None:
    ...
    def handleCustomMsg(self, textId, cfType):
        if self.whisperAvatarId is None:
    ...
    def handleCogEmoteMsg(self, emoteId):
        if self.whisperAvatarId is None:
            self.chatMgr.sendSCDisguiseEmoteChatMessage(emoteId)
        else:
            self.chatMgr.sendSCDisguiseEmoteWhisperMessage(emoteId, self.whisperAvatarId)
        self.toPlayer = 0
    ...
    def handleGreetingMsg(self, textId):
        self.chatMgr.sendSCGreetingChatMessage(textId)
    ...
    def addCogEmoteMenu(self):
        if self.cogEmoteMenu == None:
            menu = TTSCCogEmoteMenu()
            self.cogEmoteMenu = SCMenuHolder(OTPLocalizer.SCMenuCogEmote, menu=menu)
            self.speedChat[3:3] = [self.cogEmoteMenu]
            
    def removeCogEmoteMenu(self):
        if self.cogEmoteMenu:
            i = self.speedChat.index(self.cogEmoteMenu)
            del self.speedChat[i]
            self.cogEmoteMenu.destroy()
            self.cogEmoteMenu = None
