...
class SueCogPanel(StateData.StateData):
    notify = DirectNotifyGlobal.directNotify.newCategory('SueCogPanel')
    ...
    def load(self):
        ...
        self.textFrame['text'] = TTLocalizer.SueCogTitle % localAvatar.getSummons()
        self.avatarButtons = []
        for i in range(6):
    ...
    def __placeButtons(self, numAvatars, invalidTargets, localNum):
        ...
        if cansue:
            ...
            self.textFrame['text'] = TTLocalizer.SueCogLowTitle % localAvatar.getSummons()
        if numAvatars == 1:
