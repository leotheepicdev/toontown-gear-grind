...
def chooseSoundOpenShot(sounds, targets, attackDuration):
    ...
    elif numSounds >= 2 and numSounds <= 6:
...
def chooseThrowOpenShot(throws, suitThrowsDict, attackDuration):
    ...
    if numThrows == 1:
        ...
        shotChoices = [avatarCloseUpThrowShot,
         ...
         allGroupLowShot,
         suitGroupThreeQuarterLeftBehindShot]
    elif numThrows >= 2 and numThrows <= 5:
        shotChoices = [allGroupLowShot, suitGroupThreeQuarterLeftBehindShot]
    elif numThrows >= 6:
        shotChoices = [allGroupLowShotHigh, suitGroupThreeQuarterLeftBehindShotHigh]
    else:
...
def chooseSquirtOpenShot(squirts, suitSquirtsDict, attackDuration):
    ...
    if numSquirts == 1:
        ...
        shotChoices = [avatarCloseUpThrowShot,
         ...
         allGroupLowShot,
         suitGroupThreeQuarterLeftBehindShot]
    elif numSquirts >= 2 and numSquirts <= 6:
        shotChoices = [allGroupLowShot, suitGroupThreeQuarterLeftBehindShot]
    else:
...
def chooseDropOpenShot(drops, suitDropsDict, attackDuration):
    ...
    if numDrops == 1:
        ...
        shotChoices = [avatarCloseUpThrowShot,
         ...
         allGroupLowShot,
         suitGroupThreeQuarterLeftBehindShot]
    elif numDrops >= 2 and numDrops <= 5 or numDrops == 0:
        shotChoices = [allGroupLowShot, suitGroupThreeQuarterLeftBehindShot]
    elif numDrops >= 6:
        shotChoices = [allGroupLowShotHigh, suitGroupThreeQuarterLeftBehindShotHigh]
    else:
...
def chooseSuitShot(attack, attackDuration):
    ...
    if name == AUDIT:
        ...
        camTrack.append(toonGroupShot(target, 2))
    else:
        notify.warning('unknown attack id in chooseSuitShot: %d using default cam' % name)
        camTrack.append(defaultCamera())
    pbpText = attack['playByPlayText']
    if attack['ditto'] > 0:
        displayName = TTLocalizer.Ditto
        pbpTrack = pbpText.getShowInterval(displayName, 3.5)
        soundTrack = SoundInterval(base.loader.loadSfx('phase_5/audio/sfx/SA_static.ogg'), node=suit)
        return Parallel(soundTrack, camTrack, pbpTrack)
    displayName = TTLocalizer.SuitAttackNames[attack['name']]
    if attack['status']:
        pbprText = attack['playByPlayRoundsText']
        pbpTrack = Parallel()
        pbpTrack.append(pbpText.getShowInterval(displayName, 3.5))
        if attack['desc'] != '':
            pbpdText = attack['playByPlayDescText']
            pbpTrack.append(pbpdText.getShowInterval(attack['desc'], (0.0, 0.65), 3.5))
            if attack['rounds'] > 1:
                r_text = TTLocalizer.SuitAttackRounds % attack['rounds']
            elif attack['rounds'] == 1:
                r_text = TTLocalizer.SuitAttackRoundsS
            else:
                r_text = TTLocalizer.SuitAttackLastRound
            pbpTrack.append(pbprText.getShowInterval(r_text, (0.0, 0.6), 3.5))
        else:
            if attack['rounds'] > 1:
                r_text = TTLocalizer.SuitAttackRounds % attack['rounds']
            elif attack['rounds'] == 1:
                ...
                r_text = TTLocalizer.SuitAttackLastRound
            pbpTrack.append(pbprText.getShowInterval(r_text, (0.0, 0.65), 3.5))
    else:
        pbpTrack = Parallel()
        pbpTrack.append(pbpText.getShowInterval(displayName, 3.5))
        if attack['desc'] != '' and target['hp'] > 0:
            pbpdText = attack['playByPlayDescText']
            if attack['name'] in TTLocalizer.AttackExtendDesc:
                y = 0.45
            else:
                y = 0.65
            pbpTrack.append(pbpdText.getShowInterval(attack['desc'], (0.0, y), 3.5))
...
def allGroupLowShotHigh(avatar, duration): # TODO
    return heldShot(20, -0.5, 3, 89, 0, 0, duration, 'allGroupLowShotHigh')
...
def suitGroupThreeQuarterLeftBehindShotHigh(avatar, duration):
    if random.random() > 0.5:
        x = 14.37
        h = 134.61
    else:
        x = -14.37
        h = -134.61
    return heldShot(x, 9.5, 9.16, h, -22.7, 0, duration, 'suitGroupThreeQuarterLeftBehindShotHigh')
...
def chooseFireOpenShot(throws, suitThrowsDict, attackDuration):
    ...
    if numThrows == 1:
        ...
        shotChoices = [avatarCloseUpThrowShot,
         ...
         allGroupLowShot,
         suitGroupThreeQuarterLeftBehindShot]
    elif numThrows >= 2 and numThrows <= 6:
        shotChoices = [allGroupLowShot, suitGroupThreeQuarterLeftBehindShot]
    else:
        notify.error('Bad number of throws: %s' % numThrows)
