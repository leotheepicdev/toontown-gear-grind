...
def __createFishingPoleMultiTrack(lure, dollar, dollarName, lureLevel):
    toon = lure['toon']
    target = lure['target']
    battle = lure['battle']
    ...
    if sidestep == 0:
        if kbbonus == 1 or hp > 0:
            ...
            suitTrack.append(Func(suit.showHpText, ToontownBattleGlobals.NumRoundsLured[lureLevel], openEnded=0, attackTrack=LURE_TRACK))
            ...
            wantDeathTrack = True
            if hp > 0:
                damageTrack, wantDeathTrack = __createSuitDamageTrack(battle, suit, hp, revived, lure, trapProp)
                suitTrack.append(damageTrack)
            if revived != 0:
                suitTrack.append(MovieUtil.createSuitReviveTrack(suit, toon, battle))
            if died != 0 and wantDeathTrack:
    ...
    for target in targets:
        ...
        if sidestep == 0:
            ...
            if kbbonus == 1 or hp > 0:
                ...
                elif died != 0 and wantDeathTrack:
    ...
    for target in targets:
        ...
        if sidestep == 0:
            ...
            if kbbonus == 1 or hp > 0:
                ...
                suitTrack.append(Func(suit.loop, 'neutral'))
                suitTrack.append(Wait(suitDelay))
                suitTrack.append(Func(suit.showHpText, ToontownBattleGlobals.NumRoundsLured[5], openEnded=0, attackTrack=LURE_TRACK))
...
def __lureOneDollar(lure):
    ...
    return __createFishingPoleMultiTrack(lure, dollar, dollarProp, lureLevel=0)
...
def __lureFiveDollar(lure):
    dollarProp = '5dollar'
    dollar = globalPropPool.getProp(dollarProp)
    return __createFishingPoleMultiTrack(lure, dollar, dollarProp, lureLevel=2)


def __lureLargeMagnet(lure, npcs = []):
    magnet = globalPropPool.getProp('big-magnet')
    pos = Point3(-0.27, 0.08, 0.47)
...
def __lureTenDollar(lure):
    dollarProp = '10dollar'
    dollar = globalPropPool.getProp(dollarProp)
    return __createFishingPoleMultiTrack(lure, dollar, dollarProp, lureLevel=4)
...
def __createSuitDamageTrack(battle, suit, hp, revived, lure, trapProp):
    ...
    if trapName == 'banana':
        ...
        if suit.getHP() - hp <= 0 and revived == 0:
            resetPos = False
            ...
            if suit.style.body == 'c':
                ...
                yVal = 2.3
            gearPoint = Point3(suitPos.getX(), suitPos.getY() - yVal, suitPos.getZ() + suit.height - zVal)            
            
            suitTrack = Sequence(ActorInterval(suit, 'slip-backward', endFrame=24), Parallel(MovieUtil.createKapowExplosionTrack(battle, explosionPoint=gearPoint), 
                                 SoundInterval(base.loader.loadSfx('phase_3.5/audio/sfx/ENC_cogfall_apart.ogg'), node=suit), Wait(1.1), Func(suit.hide)))
        else:
        ...
        if suit.getHP() - hp <= 0 and revived == 0:
            resetPos = False
            moveTrack = Sequence(Wait(0.9), Func(suit.hideNametag3d), Func(suit.hideDropShadow), LerpPosInterval(suit, 0.9, sinkPos1, other=battle), LerpPosInterval(suit, 0.4, sinkPos2, other=battle))
            ...
            result.append(Parallel(trapTrack, moveTrack, animTrack, soundTrack))
        else:
        ...
        if suit.getHP() - hp <= 0 and revived == 0:
            resetPos = False
            moveTrack = Sequence(Wait(2.2), Func(suit.hideNametag3d), Func(suit.hideDropShadow), LerpPosInterval(suit, 0.4, sinkPos, other=battle))
            animTrack = Sequence(getSplicedLerpAnimsTrack(suit, 'flail', 0.7, 0.25), Func(trapProp.setColor, Vec4(0, 0, 0, 1)), ActorInterval(suit, 'flail', startTime=0.7, endTime=0), ActorInterval(suit, 'neutral', duration=0.5), ActorInterval(suit, 'flail', startTime=1.1))
...
def createMarblesExplosionTrack(parent, explosionPoint = None, trapProp = None, relativeTo = render):
    explosionTrack = Sequence()
    ...
    for target in targets:
        ...
        if sidestep == 0:
            ...
            if kbbonus == 1 or hp > 0:
                ...
                suitTrack.append(Func(suit.loop, 'neutral'))
                suitTrack.append(Wait(suitDelay))
                suitTrack.append(Func(suit.showHpText, ToontownBattleGlobals.NumRoundsLured[6], openEnded=0, attackTrack=LURE_TRACK))
                suitTrack.append(ActorInterval(suit, 'hypnotized', duration=3.1))
                suitTrack.append(Func(suit.setPos, battle, reachPos))
                suitTrack.append(Func(suit.loop, 'neutral'))
                suitTrack.append(Func(battle.lureSuit, suit))
