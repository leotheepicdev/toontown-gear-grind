...
def __doDamage(toon, dmg, died):
    if dmg > 0 and toon.hp != None:
        toon.takeDamage(dmg, wantRelocate=died)
...
def doSuitAttack(attack):
    ...
    elif name == MYSTERIOUS_DISAPEARANCE:
    ...
    elif name == PAWER_TRIP:
        suitTrack = doPawerTrip(attack)
    elif name == QUAKE:
    ...
    elif name == TAKE_ARTICLE:
        suitTrack = doTakeArticle(attack)
    elif name == TEE_OFF:
    ...
    if suit:
        ...
        if not attack['status'] and battle.isSuitLured(suit):
...
def getToonTrack(attack, damageDelay = 1e-06, damageAnimNames = None, dodgeDelay = 0.0001, dodgeAnimNames = None, splicedDamageAnims = None, splicedDodgeAnims = None, target = None, showDamageExtraTime = 0.01, showMissedExtraTime = 0.5):
    ...
    if dmg > 0:
        animTrack.append(getToonTakeDamageTrack(toon, target['died'], statusId, dmg, damageDelay, damageAnimNames, splicedDamageAnims, showDamageExtraTime))
        return animTrack
    else:
...
def getToonTakeDamageTrack(toon, died, statusId, dmg, delay, damageAnimNames = None, splicedDamageAnims = None, showDamageExtraTime = 0.01):
    toonTrack = Sequence()
    ...
    if damageAnimNames:
        ...
        if statusId != -1:
            attackName = SuitAttackNames[statusId]
            text = TTLocalizer.EffectDesc[attackName]
            indicatorTrack.append(Wait(0.5))
            indicatorTrack.append(Func(MovieUtil.indicateStatus, toon, text))
    else:
...
def doSacked(attack):
    ...
    initialScale = Point3(0.55, 1.37, 1.18)
    scaleUpPoint = Point3(0.95, 1.57, 0.88) * 3.1
...
def doDoubleCross(attack):
    ...
    dodgeDelay = 1
    suitName = suit.getStyleName()
    posPoints = [Point3(0.8, 4.65, 4.01), VBase3(-155.0, 20.0, 90.0)]
    posPoints2 = [Point3(-0.8, 4.65, 4.01), VBase3(-155.0, 20.0, 90.0)]
    appearDelay = 0.8
    suitHoldStart = 1.06
    suitHoldStop = 1.69
    ...
    xAppearTrack = Sequence(Wait(suitHoldStart), Func(__showProp, x, suit, posPoints[0], posPoints[1]), LerpScaleInterval(x, suitHoldDuration, Point3(1.2, 1.2, 1.2)), Wait(xHoldDuration * 0.3), Func(battle.movie.needRestoreRenderProp, x), Func(x.wrtReparentTo, battle))
    toonFace = __toonFacePoint(toon, parent=battle)
    if dmg > 0:
        lerpInterval = LerpPosInterval(x, moveDuration, toonFace)
    else:
        lerpInterval = LerpPosInterval(x, moveDuration, Point3(toonFace.getX(), toonFace.getY() - 5, toonFace.getZ() - 2))
    xMoveTrack = lerpInterval
    xPropTrack = Sequence(xAppearTrack, xMoveTrack, Func(battle.movie.clearRenderProp, x), Func(MovieUtil.removeProp, x))
    
    x2AppearTrack = Sequence(Wait(suitHoldStart), Func(__showProp, x2, suit, posPoints2[0], posPoints2[1]), LerpScaleInterval(x2, suitHoldDuration, Point3(1.5, 1.5, 1.5)), Wait(xHoldDuration * 0.3), Func(battle.movie.needRestoreRenderProp, x2), Func(x2.wrtReparentTo, battle))
    if dmg > 0:
        lerpInterval2 = LerpPosInterval(x2, moveDuration, toonFace)
    else:
        lerpInterval2 = LerpPosInterval(x2, moveDuration, Point3(toonFace.getX(), toonFace.getY() - 5, toonFace.getZ() - 2))
    x2MoveTrack = lerpInterval2
    x2PropTrack = Sequence(x2AppearTrack, x2MoveTrack, Func(battle.movie.clearRenderProp, x2), Func(MovieUtil.removeProp, x2))
...
def doEvilEye(attack):
    ...
    suitSplicedAnims.append(['glower', 0.01, suitHoldStop])
    suitTrack = getSuitTrack(attack, splicedAnims=suitSplicedAnims)
    eyeAppearTrack = Sequence(Wait(suitHoldStart), Func(__showProp, eye, suit, posPoints[0], posPoints[1]), LerpScaleInterval(eye, suitHoldDuration, Point3(3, 3, 3)), Wait(eyeHoldDuration * 0.3), LerpHprInterval(eye, 0.02, Point3(205, 40, 0)), Wait(eyeHoldDuration * 0.7), Func(battle.movie.needRestoreRenderProp, eye), Func(eye.wrtReparentTo, battle))
    toonFace = __toonFacePoint(toon, parent=battle)
    if dmg > 0:
...
def doTakeArticle(attack):
    suit = attack['suit']
    battle = attack['battle']
    target = attack['target']
    dmg = target['hp']
    article = globalPropPool.getProp('shredder-paper')
    suitTrack = getSuitTrack(attack)
    articlePosPoints = [Point3(-1, 0.45, -0.25), VBase3(136.424, -46.434, -129.712)]
    articlePropTrack = getPropTrack(article, suit.getRightHand(), articlePosPoints, 0.6, 0.55, scaleUpPoint=Point3(1.41, 1.41, 1.41))
    toonTrack = getToonTrack(attack, 0.6, ['cringe'], 0.01, ['sidestep'])
    multiTrackList = Parallel(suitTrack, toonTrack)
    if dmg > 0:
        soundTrack = getSoundTrack('SA_pick_pocket.ogg', delay=0.2, node=suit)
        multiTrackList.append(articlePropTrack)
        multiTrackList.append(soundTrack)
    return multiTrackList
...
def doPawerTrip(attack):
    suit = attack['suit']
    battle = attack['battle']
    centerColor = Vec4(0.1, 0.1, 0.1, 0.4)
    edgeColor = Vec4(0.4, 0.9, 0.1, 0.7)
    powerBar1 = BattleParticles.createParticleEffect(file='powertrip')
    powerBar2 = BattleParticles.createParticleEffect(file='powertrip2')
    powerBar1.setPos(0, 6.1, 0.4)
    powerBar1.setHpr(-60, 0, 0)
...
def doBurn(attack):
    ...
    for target in targets:
        ...
        flameTracks.append(baseFlameTrack)
        flameTracks.append(flameTrack)
        flameTracks.append(flecksTrack)
    ...
    soundTrack = getSoundTrack('SA_hot_air.ogg', delay=1.0)    
 
    return Parallel(flameTracks, toonTracks, soundTrack)
...
def doMysteriousDisappearance(attack):
    ...
    toonTrack = Parallel()
    if dmg > 0:
        dustCloudHideIval = Sequence(Func(dustCloud.reparentTo, render), Func(dustCloud.setPos, Point3(toonPos.getX(), toonPos.getY(), toonPos.getZ() + 3)), 
                                 dustCloud.track, Func(dustCloud.detachNode), name='dustCloadIval')
        dustCloudShowIval = Sequence(Func(dustCloud.reparentTo, render), Func(dustCloud.setPos, Point3(toonPos.getX(), toonPos.getY(), toonPos.getZ() + 3)), 
                                 dustCloud.track, Func(dustCloud.detachNode), Func(dustCloud.destroy), name='dustCloadIval2')
    ...
    toonTrack.append(getToonTrack(attack, 3.6, ['conked'], 2.8, ['jump']))
