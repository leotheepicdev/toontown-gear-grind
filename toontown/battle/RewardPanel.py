...
class RewardPanel(DirectFrame):
    ...
    def __init__(self, name):
        ...
        self.mvpLabel = DirectLabel(parent=self, relief=None, pos=(0.7, 0, 0.2), hpr=(0, 0, 30), text=TTLocalizer.RewardPanelMVP, text_scale=0.06)
        self.mvpSeq = None
    ...
    def initGagFrame(self, toon, expList, meritList, noSkip = False, mvp=0):
        self.avNameLabel['text'] = toon.getName()
        self.mvpLabel.hide()
        if toon.doId == mvp:
            self.mvpLabel.show()
            self.mvpSeq = Sequence(LerpScaleInterval(self.mvpLabel, 1, 1.1, 0.9, blendType='easeInOut'), LerpScaleInterval(self.mvpLabel, 1, 0.9, 1.1, blendType='easeInOut'))
            self.mvpSeq.loop()
        self.endTrackFrame.hide()
    ...
    def getQuestIntervalList(self, toon, deathList, toonList, origQuestsList, itemList, helpfulToonsList = []):
        ...
        for death in deathList:
            cogType, cogLevel, cogTrack, activeToonBits, flags = death
            activeToonIds = []
    ...
    def getExpTrack(self, toon, origExp, earnedExp, deathList, origQuestsList, itemList, missedItemList, origMeritList, meritList, partList, piggyAmount, piggyMax, toonList, helpfulToonsList, noSkip = False, mvp=0):
        track = Sequence(Func(self.initGagFrame, toon, origExp, origMeritList, noSkip=noSkip, mvp=mvp), Wait(1.0))
        if piggyAmount >= 1:
            if toon.doId == base.localAvatar.doId:
                track.append(Func(base.localAvatar.preparePiggyMax, piggyMax))
                track.append(Func(base.localAvatar.displayLocalItemGain, 1, piggyAmount))
        endTracks = [0,
         0,
         0,
         0,
        ...
        for trackIndex in range(len(earnedExp)):
            if earnedExp[trackIndex] > 0 and origExp[trackIndex] < ToontownBattleGlobals.MaxSkill:
