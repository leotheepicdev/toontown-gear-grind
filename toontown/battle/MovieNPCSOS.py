...
def __doSprinkle(attack, recipients, hp = 0):
    ...
    def face90(targets, node, battle):
        point = Point3(0, 0, 0)
        for target in targets:
            point += target.getPos(battle)
        point /= len(targets)
        vec = Point3(point - node.getPos(battle))
        vec.setZ(0)
        temp = vec[0]
        vec.setX(-vec[1])
        vec.setY(temp)
        targetPoint = Point3(node.getPos(battle) + vec)
        node.headsUp(battle, targetPoint)
    ...
    for target in targets:
        ...
        sprinkle = battle.attachNewNode('sprinkle')
        sprinkle.setPos(toon.getPos())
        face90((target,), sprinkle, battle)
        mtrack = Parallel(__getPartTrack(sprayEffect, 1.5, 0.5, [sprayEffect, sprinkle, 0]), __getPartTrack(dropEffect, 1.9, 2.0, [dropEffect, target, 0]), __getPartTrack(explodeEffect, 2.7, 1.0, [explodeEffect, toon, 0]), 
                          __getPartTrack(poofEffect, 3.4, 1.0, [poofEffect, target, 0]), __getPartTrack(wallEffect, 4.05, 1.2, [wallEffect, toon, 0]), Sequence(Wait(delay), Func(__healToon, target, hp), Func(sprinkle.removeNode)))
        effectTrack.append(mtrack)

    effectTrack.append(Parallel(__getSoundTrack(level, 2, duration=3.1, node=toon), Sequence(Func(face90, targets, toon, battle), ActorInterval(toon, 'sprinkle-dust'))))
    track.append(effectTrack)
    track.append(Func(toon.setHpr, Vec3(180.0, 0.0, 0.0)))
    track.append(teleportOut(attack, toon))
    return track

def __doUnite(attack, hp = 0, index = 108, delay=1, playRate=0.75):
    toon = NPCToons.createLocalNPC(attack['npcId'])
    if toon == None:
        return
    ...
    track = Sequence(teleportIn(attack, toon))
    uniteTrack = Parallel(Func(toon.setChatAbsolute, chatString, CFSpeech | CFTimeout), 
                          Sequence(Func(toon.setPlayRate, playRate, 'victory'), ActorInterval(toon, 'victory', playRate=playRate, startFrame=0, endFrame=9), 
                          ActorInterval(toon, 'victory', playRate=playRate, startFrame=9, endFrame=0), Func(toon.loop, 'neutral')), Func(ResistanceChat.doEffect, index, toon, targets))
    track.append(uniteTrack)
