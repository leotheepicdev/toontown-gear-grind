...
def insertDeathSuit(suit, deathSuit, battle = None, pos = None, hpr = None):
    ...
    if suit.getVirtual() > 0:
        suit.makeVirtual(suit.getVirtual())
    avatarHide(suit)
...
def createSuitDeathTrack(suit, toon, battle, npcs = []):
    ...
    if hasattr(suit, 'battleTrapProp') and suit.battleTrapProp and suit.battleTrapProp.getName() == 'traintrack' and not suit.battleTrapProp.isHidden():
        suitTrack.append(createTrainTrackAppearTrack(suit, toon, battle, npcs))
    if suit.getVirtual():
        suitTrack.append(Parallel(Func(suit.loop, 'neutral'), LerpColorScaleInterval(suit, 0.7, (0, 0, 0, 0), suit.getColorScale(), blendType='easeIn')))
        deathSoundTrack = Sequence(SoundInterval(loader.loadSfx('phase_11/audio/sfx/LB_laser_beam_off_2.ogg'), node=suit))
...
def createSuitTauntMultiTrack(suit, delay = 0.01):
    suitTrack = Sequence(Wait(delay), Parallel(ActorInterval(suit, 'victory', startTime=0.5, endTime=1.9), Func(suit.setChatAbsolute, random.choice(TTLocalizer.SuitBattleTaunts), CFSpeech | CFTimeout)), Func(suit.loop, 'neutral'), Wait(1), Func(suit.clearChat))
    tauntedTrack = Sequence(Wait(delay + 0.2), Func(indicateTaunted, suit, 0.9))
    return Parallel(suitTrack, tauntedTrack)
...
def indicateTaunted(actor, duration = 1.1, scale = 0.7):
    actor.showHpString(TTLocalizer.AttackTaunted, duration=duration, scale=scale)
    
def indicateSued(actor, duration = 1.1, scale = 0.7):
    actor.showHpString(TTLocalizer.AttackSued, duration=duration, scale=scale)
    
def indicateStatus(actor, text, duration = 1.1, scale = 0.5):
    actor.showStatusString(text, duration=duration, scale=scale)
...
def createSuitStunInterval(suit, before, after):
    ...
    return Sequence(Wait(before), Func(stars.reparentTo, head), Func(stars.setZ, max(0.0, p2[2] - 1.0)), Func(stars.loop, 'stun'), Wait(after), stars.colorScaleInterval(.3, (1, 1, 1, 0)), Func(stars.cleanup), Func(stars.removeNode))
