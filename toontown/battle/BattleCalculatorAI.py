...
from toontown.quest.Quests import Tier2ReviveAmount, TT_TIER
...
class Effect:
    ...
    def __init__(self):
        ...
        self.dmg = 0
        self.rounds = 0
        # self.effectExpiresAfterSuitDeath
        
    def setSuitId(self, suitId):
        self.suitId = suitId
...
class BattleCalculatorAI:
    AccuracyBonuses = [0, 20, 40, 60, 60, 60]
    DamageBonuses = [0, 20, 20, 20, 20, 20]
    ...
    def __init__(self, battle, tutorialFlag=0):
        ...
        self.gagsHit2Cogs = {}
        self.gagsMissed2Cogs = {}
        self.taunts = []
        self.sues = []
        self.cogName2Ids = {}
        self.postEffects = {}
        self.expiredEffects = []
        self.suit_postEffect2Id = {}
        self.toon_postEffect2Id = {}
        self.toon2Jellybean = {}
    ...
    def removeBattleSOS(self, toonId):
        if toonId in self.sosCalls:
            self.sosCalls.remove(toonId)
    ...
    def __calcToonAtkHit(self, attackIndex, atkTargets):
        ...
        if (atkTrack, atkLevel) in self.gagsHit2Cogs:
            if atkTargets[0] in self.gagsHit2Cogs[(atkTrack, atkLevel)]:
                return (1, 95)
        if (atkTrack, atkLevel) in self.gagsMissed2Cogs:
            if atkTargets[0] in self.gagsMissed2Cogs[(atkTrack, atkLevel)]:
                return (0, 0)
        if atkTrack == NPCSOS:
            return (1, 95)
        if atkTrack == FIRE:
        ...
        atkMiss = attack[TOON_ACCBONUS_COL]
        atkHit = not atkMiss
        if atkTrack not in [HEAL, TRAP, SOUND] and attack[TOON_TRACK_COL] != NPCSOS and len(self.battle.activeToons) >= 2:
            if atkHit:
                if (atkTrack, atkLevel) not in self.gagsHit2Cogs:
                    self.gagsHit2Cogs[(atkTrack, atkLevel)] = []
                self.gagsHit2Cogs[(atkTrack, atkLevel)].append(atkTargets[0])
            elif atkMiss:
                if (atkTrack, atkLevel) not in self.gagsMissed2Cogs:
                    self.gagsMissed2Cogs[(atkTrack, atkLevel)] = []
                self.gagsMissed2Cogs[(atkTrack, atkLevel)].append(atkTargets[0])

        return (atkHit, atkAccResult)
    ...
    def __targetDefense(self, suit, atkTrack):
        if atkTrack == HEAL:
            return 0
        if suit.dna.name == 'null':
            suitDef = SuitBattleGlobals.NullLevel2Defense[suit.getActualLevel()]
        else:
            suitDef = SuitBattleGlobals.SuitAttributes[suit.dna.name]['def'][suit.getLevel()]
            if suit.getVirtual() == SuitBattleGlobals.VIRTUAL_BLUE:
                suitDef = int(suitDef * SuitBattleGlobals.VIRTUAL_BLUE_DEF_MULT)
        return -suitDef
        ...
        for currTarget in range(len(targetList)):
            ...
            if atkTrack == LURE:
                ...
                if attack[TOON_TRACK_COL] == NPCSOS and lureDidDamage != 1 or attack[TOON_TRACK_COL] == PETSOS:
                    ...
                    suit = self.battle.findSuit(targetId)
                    if self.__combatantDead(targetId, toon=0):
                        self.clearAttack(toonId)
                        continue
                    if suit:
                        suit.setSued(True)
                    abilityToSue = toon.getSummons()
                    if not toon.removeSummons():
                        simbase.air.writeServerEvent('suspicious', toonId, 'Toon sueing cog without summons!')
                    attackDamage = 0
                else:
                ...
                if not self.__combatantDead(targetId, toon=toonTarget):
                    if self.__suitIsLured(targetId) and atkTrack == DROP:
                        ...
                        validTargetAvail = 1
            if attackLevel == -1 and not atkTrack in [FIRE, TAUNT, SUE]:
    ...
    def __postProcessToonAttacks(self):
        ...
        for currToonAttack in self.toonAtkOrder:
            if currToonAttack != -1:
                ...
                atkTrack, atkLevel = self.__getActualTrackLevel(attack)
                if atkTrack != HEAL and atkTrack != SOS and atkTrack != NO_ATTACK and atkTrack != TAUNT and atkTrack != SUE and atkTrack != NPCSOS and atkTrack != PETSOS:
    ...
    def __calcSuitAtkType(self, theSuit):
        if theSuit.dna.name == 'null':
            atk = random.choice(list(theSuit.attacks.keys()))
        else:
    ...
    def __calcSuitTarget(self, suitId):
        if self.taunts:
            if random.random() <= 0.4:
                return self.__pickTauntingToon(suitId)
        if suitId in self.SuitAttackers and random.randint(0, 99) < 75:
            ...
            if toonId in self.battle.activeToons:
                if self.__combatantDead(toonId, toon=1):
    ...
    def __pickTauntingToon(self, suitId):
        targets = []
        for toon in self.taunts:
            if toon in self.battle.activeToons:
                if not self.__combatantDead(toon, toon=1):
                    targets.append(self.battle.activeToons.index(toon))
        if len(targets) == 0:
            return self.__pickRandomToon(suitId)
    ...
    def __suitAtkHit(self, theSuit, attack, attackName):
        if self.suitsAlwaysHit:
            return 1
        else:
            if self.suitsAlwaysMiss:
                return 0
        if theSuit.getVirtual() == SuitBattleGlobals.VIRTUAL_YELLOW:
            return 1
        if theSuit.dna.name == 'null':
            atkType = attack[SUIT_ATK_COL]
            acc = theSuit.attacks[atkType]['acc']
        else:
            atkInfo = SuitBattleGlobals.getSuitAttack(theSuit.dna.name, theSuit.getLevel(), attackName)
            atkAcc = atkInfo['acc']
    ...
    def __suitAtkAffectsGroup(self, attack):
        ...
        name = SuitBattleGlobals.SuitAttackNames[atkId]
        group = SuitBattleGlobals.SuitAttacks[name]
        return group[1] != SuitBattleGlobals.ATK_TGT_SINGLE
    ...
    def __calcSuitAtkHp(self, theSuit, attack, attackName):
        ...
        for currTarget in range(len(targetList)):
            ...
            if toon and toon.immortalMode:
                ...
                if self.TOONS_TAKE_NO_DAMAGE:
                    ...
                    if self.__suitAtkHit(theSuit, attack, attackName):
                        if theSuit.dna.name == 'null':
                            atkType = attack[SUIT_ATK_COL]
                            result = theSuit.attacks[atkType]['dmg']
                            ...
                            if result_type == tuple:
                                result = random.randint(result[0], result[1])
                            if result_type == str:
                                if result == 'h':
                                    result = math.ceil(toon.getMaxHp() / 2)
                            else:
                                if theSuit.getVirtual() == SuitBattleGlobals.VIRTUAL_RED:
                                    result *= SuitBattleGlobals.VIRTUAL_RED_DMG_MULT
                                if 'dmgAdd' in atkInfo:
                                    if attack[SUIT_ATK_COL] in theSuit.atk2DmgAdd:
                                        result += theSuit.atk2DmgAdd[attack[SUIT_ATK_COL]]
                        if result_type != str:
                            if toonId in self.taunts:
                                result = math.ceil(result * SuitBattleGlobals.TAUNT_DMG_MULT)
                            if theSuit.doId in self.sues:
                                result = math.ceil(result * SuitBattleGlobals.SUE_DMG_MULT)
                                
                        if 'dmgAdd' in atkInfo:
                            if attack[SUIT_ATK_COL] in theSuit.atk2DmgAdd:
                                theSuit.atk2DmgAdd[attack[SUIT_ATK_COL]] += atkInfo['dmgAdd']
                            else:
                                theSuit.atk2DmgAdd[attack[SUIT_ATK_COL]] = atkInfo['dmgAdd']                            
    ...
    def __applySuitPostEffects(self, theSuit, attack, attackName):
        atkInfo = SuitBattleGlobals.getSuitAttack(theSuit.dna.name, theSuit.getLevel(), attackName)
        if atkInfo['status']:
            targetList = self.__createSuitTargetList(attack)
        
            attackId = SuitBattleGlobals.SuitAttackNames.index(atkInfo['status']['atk'])
            toonIds = []
            for currTarget in range(len(targetList)):
                toonId = targetList[currTarget]
                toon = self.battle.getToon(toonId)
                if toon:
                    position = self.battle.activeToons.index(toonId)
                    if attack[SUIT_HP_COL][position] <= 0:
                        continue
                    if attackId in theSuit.status2ToonIds:
                        if toonId not in theSuit.status2ToonIds[attackId]:
                            toonIds.append(toonId)
                    else:
                        theSuit.status2ToonIds[attackId] = [toonId]
                    toonIds.append(toonId)
            ...
            if toonIds:
                ...
                effect = Effect()
                effect.setSuitId(suitId)
                effect.setAttackId(attackId)
                effect.setRounds(atkInfo['status']['rounds'])
                effect.setToonIds(toonIds)
                effect.setDmg(atkInfo['status']['dmg'])

                effectId = self.battle.attackAllocator.allocateId()
                self.postEffects[effectId] = effect
                for toonId in toonIds:
                    if toonId in self.toon_postEffect2Id:
                        self.toon_postEffect2Id[toonId].append(effectId)
                    else:
                        self.toon_postEffect2Id[toonId] = [effectId]
                        
                if suitId in self.suit_postEffect2Id:
                    self.suit_postEffect2Id[suitId].append(effectId)
                else:
                    self.suit_postEffect2Id[suitId] = [effectId]
                    
                attack[SUIT_STATUS_COL] = attackId
    ...
    def __applySuitAttackDamages(self, attack):
        if self.APPLY_HEALTH_ADJUSTMENTS:
            for t in self.battle.activeToons:
                ...
                if t in self.toon2Jellybean:
                    money = self.toon2Jellybean[t]
                else:
                    money = 0
    ...
    def __generateSuitAttacks(self):
        updatedSuitAttacks = []
        suitAttacksPending = self.battle.suitAttacks[:]
        dittoSuit = None
        i = 0
        
        while suitAttacksPending != []:
            attack = suitAttacksPending[i]
            suitId = attack[SUIT_ID_COL]
            suit = self.battle.findSuit(suitId)
            if not suit:
                del suitAttacksPending[i]
                continue
            ...
            if self.battle.pendingSuits.count(suit) > 0 or self.battle.joiningSuits.count(suit) > 0:
                del suitAttacksPending[i]
                continue
            if attack[SUIT_ATK_COL] == -1:
                attack[SUIT_ATK_COL] = self.__calcSuitAtkType(suit)
            attack[SUIT_TGT_COL] = self.__calcSuitTarget(suitId)
            if attack[SUIT_TGT_COL] == -1:
                del suitAttacksPending[i]
                self.notify.debug('skipping suit attack, no available targets')
                continue
            attackName = SuitBattleGlobals.SuitAttackNames[attack[SUIT_ATK_COL]]
            if attack[SUIT_DITTO_COL] == 1 and dittoSuit:
                self.__calcSuitAtkHp(dittoSuit, attack, attackName)
            else:
                self.__calcSuitAtkHp(suit, attack, attackName)
            if attack[SUIT_ATK_COL] != NO_ATTACK:
            ...
            if self.__attackHasHit(attack, suit=1):
                ...
                self.__applySuitPostEffects(suit, attack, attackName)
            if self.notify.getDebug():
                self.notify.debug('Suit attack: ' + str(attack))
            attack[SUIT_BEFORE_TOONS_COL] = 0
            updatedSuitAttacks.append(attack)
            del suitAttacksPending[i]
            if 'ym' in self.cogName2Ids and attack[SUIT_DITTO_COL] == 0 and suit.dna.body == 'a':
                dittoSuit = suit
                for otherSuitId in self.cogName2Ids['ym']:
                    if otherSuitId != suitId:
                        newAttack = getDefaultSuitAttack()
                        newAttack[SUIT_ID_COL] = otherSuitId
                        newAttack[SUIT_HP_COL] = [-1] * len(self.battle.activeToons)
                        newAttack[SUIT_ATK_COL] = attack[SUIT_ATK_COL]
                        newAttack[SUIT_DITTO_COL] = 1
                        suitAttacksPending.insert(0, newAttack)
            
        self.battle.suitAttacks = updatedSuitAttacks
                
    def buildEffect(self):
        attack = getDefaultEffectAttack()
        attack[EFFECT_TGT_COL] = [0] * len(self.battle.activeToons)
        attack[EFFECT_HP_COL] = [-1] * len(self.battle.activeToons)
        attack[EFFECT_DIED_COL] = [0] * len(self.battle.activeToons)
        return attack
        
    def removeToonFromSuitStatus(self, suitId, toonId, attackId):
        suit = self.battle.findSuit(suitId)
        if not suit:
            return
        if attackId in suit.status2ToonIds:
            if toonId in suit.status2ToonIds[attackId]:
                suit.status2ToonIds[attackId].remove(toonId)
            if suit.status2ToonIds[attackId] == []:
                del suit.status2ToonIds[attackId]    

    def __applyEffectDamages(self, attack, position, id):
        toonHp = self.__getToonHp(id)
        if toonHp - attack[EFFECT_HP_COL][position] <= 0:
            self.toonLeftBattle(id, inLoop=True)
            attack[EFFECT_DIED_COL][position] = 1 
        self.toonHPAdjusts[id] -= attack[EFFECT_HP_COL][position]
        
    def processPostEffects(self):
        allSuitsDead = 1
        ...
        if allSuitsDead and len(self.battle.pendingSuits) == 0:
            return
        
        idsToRemove = []
    
        for id, effect in self.postEffects.items():
            attack = self.buildEffect()
            attackId = effect.getAttackId()
            suitId = effect.getSuitId()
            realToonIds = []
            
            for toonId in effect.getToonIds():
                toon = self.getToon(toonId)
                if not toon:
                    continue
                if toon.getHp() <= 0:
                    continue
                realToonIds.append(toonId)
                position = self.battle.activeToons.index(toonId)
                attack[EFFECT_SUIT_ID_COL] = suitId
                attack[EFFECT_ATK_COL] = attackId
                attack[EFFECT_TGT_COL][position] = 1
                attack[EFFECT_HP_COL][position] = effect.getDmg()
                self.__applyEffectDamages(attack, position, toonId)
        
            if not realToonIds:
                if suitId in self.suit_postEffect2Id:
                    if id in self.suit_postEffect2Id[suitId]:
                        self.suit_postEffect2Id[suitId].remove(id)
                    if self.suit_postEffect2Id[suitId] == []:
                        del self.suit_postEffect2Id[suitId]
                    
                for toonId in effect.getToonIds():
                    if toonId in self.toon_postEffect2Id:
                        if id in self.toon_postEffect2Id[toonId]:
                            self.toon_postEffect2Id[toonId].remove(id)
                        if self.toon_postEffect2Id[toonId] == []:
                            del self.toon_postEffect2Id[toonId]
                            
                    self.removeToonFromSuitStatus(suitId, toonId, attackId)

                self.battle.attackAllocator.deallocateId(id)
                del self.postEffects[id]
                return                
        
            if effect.roundsExpired():
                if suitId in self.suit_postEffect2Id:
                    if id in self.suit_postEffect2Id[suitId]:
                        self.suit_postEffect2Id[suitId].remove(id)
                    if self.suit_postEffect2Id[suitId] == []:
                ...
                if id not in self.expiredEffects:
                    self.expiredEffects.append(id)
                self.battle.attackAllocator.deallocateId(id)
                
            attack[EFFECT_ROUNDS_COL] = effect.getRounds()
                
            self.battle.postEffects.append(attack)
            
        for effectId in self.expiredEffects:
            del self.postEffects[effectId]
    ...
    def __initRound(self):
        ...
        attacks = findToonAttack(self.battle.activeToons, self.battle.toonAttacks, TAUNT)
        for atk in attacks:
            self.toonAtkOrder.append(atk[TOON_ID_COL])
            self.taunts.append(atk[TOON_ID_COL])
        ...
        for atk in attacks:
            self.toonAtkOrder.append(atk[TOON_ID_COL])
            targetId = atk[TOON_TGT_COL]
            target = self.battle.findSuit(targetId)
            targetDoId = target.doId
            if targetDoId not in self.sues:
                self.sues.append(atk[TOON_TGT_COL])
        ...
        for special in specials:
            ...
            if npc_track == NPC_TOONS_HIT:
                ...
                cogsMiss = 1
        specials = findToonAttack(self.battle.activeToons, self.battle.toonAttacks, SOS)
        for special in specials:
            toonId = special[TOON_ID_COL]
            self.sosCalls.append(toonId)
    ...
    def calculateRound(self):
        ...
        for t in self.battle.activeToons:
            ...
            if toon:
                self.toon2Jellybean[t] = toon.getTotalMoney()
                
        self.battle.deadRevives = []
        self.battle.reviveResponses = {}

        toonsHit, cogsMiss = self.__initRound()
        for suit in self.battle.activeSuits:
            if suit.isGenerated():
                suit.b_setHP(suit.getHP())
            suitAttack = getDefaultSuitAttack()
            suitAttack[SUIT_ID_COL] = suit.doId
            suitAttack[SUIT_HP_COL] = [-1] * len(self.battle.activeToons)
            self.battle.suitAttacks.append(suitAttack)
            if suit.dna.name in self.cogName2Ids:
                self.cogName2Ids[suit.dna.name].insert(0, suit.doId)
            else:
                self.cogName2Ids[suit.dna.name] = [suit.doId]
        ...
        self.__generateSuitAttacks()
        self.processPostEffects()
        if toonsHit == 1:
            BattleCalculatorAI.toonsAlwaysHit = 0
        if cogsMiss == 1:
            BattleCalculatorAI.suitsAlwaysMiss = 0
        if self.gagsHit2Cogs:
            self.gagsHit2Cogs.clear()
        if self.gagsMissed2Cogs:
            self.gagsMissed2Cogs.clear()
        if self.taunts:
            self.taunts = []
        if self.sues:
            self.sues = []
        if self.cogName2Ids:
            self.cogName2Ids.clear()
        if self.notify.getDebug():
            self.notify.debug('Toon skills gained after this round: ' + repr(self.toonSkillPtsGained))
            self.__printSuitAtkStats()

    def toonLeftBattle(self, toonId, inLoop=False):
        if self.notify.getDebug():
        ...
        self.removeBattleSOS(toonId)

        if toonId in self.toon_postEffect2Id:
            for effectId in self.toon_postEffect2Id[toonId]:
                if effectId in self.postEffects:
                    effect = self.postEffects[effectId]
                    attackId = effect.getAttackId()
                    toonIds  = effect.getToonIds()
                    if toonId in toonIds:
                        toonIds.remove(toonId)
                    if toonIds == []:
                        suitId = effect.getSuitId()
                        if suitId in self.suit_postEffect2Id:
                            if effectId in self.suit_postEffect2Id[suitId]:
                                self.suit_postEffect2Id[suitId].remove(effectId)
                            if self.suit_postEffect2Id[suitId] == []:
                                del self.suit_postEffect2Id[suitId]
                                
                        self.removeToonFromSuitStatus(suitId, toonId, attackId)
                        self.expiredEffects.append(effectId)
                        self.battle.attackAllocator.deallocateId(effectId)
