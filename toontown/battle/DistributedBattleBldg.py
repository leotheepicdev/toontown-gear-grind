...
class DistributedBattleBldg(DistributedBattleBase.DistributedBattleBase):
    ...
    def setBossBattle(self, value):
        ...
        if self.bossBattle:
            if self.suitBldgType == CogBuildingGlobals.SUIT_BLDG_UNDERGROUND:
                self.battleMusic = base.loader.loadMusic('phase_7/audio/bgm/encntr_suit_winning_indoor_underground.ogg')
            else:
                self.battleMusic = base.loader.loadMusic('phase_7/audio/bgm/encntr_suit_winning_indoor.ogg')
        else:
            if self.suitBldgType == CogBuildingGlobals.SUIT_BLDG_UNDERGROUND:
                self.battleMusic = base.loader.loadMusic('phase_7/audio/bgm/encntr_general_bg_indoor_underground.ogg')
            else:
