...
class DistributedFactory(DistributedLevel.DistributedLevel, FactoryBase.FactoryBase):
    ...
    def getFloorOuchLevel(self):
        factoryRoom = SellbotLegFactorySpec.GlobalEntities
        factorySfxRoom = self.lastToonZone
        if factorySfxRoom == 10:
            self.ouchSound = base.loader.loadSfx('phase_9/audio/sfx/CHQ_FACT_paint_splash.ogg')
            base.playSfx(self.ouchSound, volume=0.5)
        elif factorySfxRoom == 19:
            self.ouchSound = base.loader.loadSfx('phase_9/audio/sfx/CHQ_FACT_lava_fall_in.ogg')
            base.playSfx(self.ouchSound, volume=0.5)
        return 2
