...
class LevelSuitPlannerAI(DirectObject.DirectObject):
    ...
    def __genSuitObject(self, suitDict, reserve):
        ...
        null = suitDict.get('null')
        dna = SuitDNA.SuitDNA()
        if null:
            dna.newSuitNull(suitDict['level'])
            suit.dna = dna
            suit.setupNullCog(suitDict['level'])
        else:
            dna.newSuitBuilding(suitDict['level'], suitDict['track'], 0.8)
            suit.dna = dna
        ...
        if suitDict['skeleton']:
            suit.setSkelecog(suitDict['skeleton'])
        if suitDict.get('randomVirtual', 0) == 1:
            suit.setVirtual(random.randint(1, 4))
