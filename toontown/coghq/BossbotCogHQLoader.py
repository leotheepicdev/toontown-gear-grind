...
class BossbotCogHQLoader(CogHQLoader.CogHQLoader):
    ...
    def loadPlaceGeom(self, zoneId):
        ...
        if zoneId == ToontownGlobals.BossbotHQ:
            ...
            gzLinkTunnel.setName('linktunnel_dl_9252_DNARoot')
            self.makeSigns()
    ...
    def makeSigns(self):
        ...
        def makeSign(topStr, signStr, textId):
            ...
            if textId == 9000:
                text = TTLocalizer.lDonaldsDreamland
                scale = TTLocalizer.BCHQLsignText - 0.2
            else:
                text = TTLocalizer.GlobalStreetNames[textId][-1]
                scale = TTLocalizer.BCHQLsignText
            signText = DirectGui.OnscreenText(text=TextEncoder.upper(text), font=ToontownGlobals.getSuitFont(), scale=scale, fg=(0, 0, 0, 1), parent=sign)
            signText.setPosHpr(locator, 0, -0.1, -0.25, 0, 0, 0)
            signText.setDepthWrite(0)

        makeSign('Gate_2', 'Sign_6', 10700)
        makeSign('TunnelEntrance', 'Sign_2', 9000)
