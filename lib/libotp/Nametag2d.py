...
class Nametag2d(Nametag, MarginPopup):
    ...
    def generateName(self):
        v4 = self.getState()
        if self.m_group.getNametagColor() != -1:
            v84 = Vec4(NametagGlobals.getPlayerNameFg(self.m_group.getAvatar().getNametagColor()))
            v75 = Vec4(NametagGlobals.getPlayerNameBg(self.m_group.getAvatar().getNametagPanelColor(), v4))
        else:
            v84 = Vec4(NametagGlobals.getNameFg(self.m_group.getColorCode(), v4, Settings['oldNametagColors']))
        ...
        if NametagGlobals._arrow_model:
            ...
            if self.m_group.getNametagColor() != -1:
                color = Vec4(NametagGlobals.getPlayerNameFg(self.m_group.getAvatar().getNametagColor()))
            else:
                color = Vec4(NametagGlobals.getArrowColor(self.m_group.getColorCode(), Settings['oldNametagColors']))
    ...
    def generateChat(self, balloon):
        ...
        if self.m_group.m_chat_flags & CFQuicktalker:
            ...
            if self.m_group.getAvatarType() == 'toon':
                balloon_color = self.m_group.getBalloonModulationColor()
