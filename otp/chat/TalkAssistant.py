...
class TalkAssistant(DirectObject.DirectObject):
    ...
    def receiveOpenTalk(self, senderAvId, avatarName, accountId, accountName, message, scrubbed = 0):
        ...
        if base.localAvatar.chatLog:
            base.localAvatar.chatLog.log(avatarName, message)
        return error

    def receiveWhisperTalk(self, avatarId, avatarName, accountId, accountName, toId, toName, message, scrubbed = 0):
        error = None
        if base.localAvatar.chatLog:
            base.localAvatar.chatLog.log(avatarName, message, color='caribbeanGreen')
    ...
    def receiveAvatarWhisperSpeedChat(self, type, messageIndex, senderAvId, name = None):
        ...
        if type == SPEEDCHAT_NORMAL:
            ...
            message = self.SCDecoder.decodeSCCustomMsg(messageIndex)
        elif type == SPEEDCHAT_DISGUISE_EMOTE:
            message = self.SCDecoder.decodeSCDisguiseEmoteWhisperMsg(messageIndex, name)
    ...
    def sendOpenTalk(self, message):
        ...
        if base.cr.wantMagicWords and len(message) > 0 and message[0] == '~':
            ...
            if self.isThought(message):
                chatFlags = CFThought
                removedThoughtPrefixMessage = self.removeThoughtPrefix(message)
                if removedThoughtPrefixMessage.isspace():
                    return
            else:
                if message.isspace():
                    return
            if base.cr.chatRelay:
                base.cr.chatRelay.relayMessage(message)
            messenger.send('chatUpdate', [message, chatFlags])
        return error

    def sendWhisperTalk(self, message, toId):
        error = None
        if base.cr.chatRelay:
            base.cr.chatRelay.relayWhisper(message, toId)
        return error
