...
# Pypperoni DC imports.
from otp.uberdog.ClientManager import ClientManager
...
class OTPClientRepository(ClientRepositoryBase):
    ...
    def enterLogin(self):
        ...
        dialogClass = OTPGlobals.getGlobalDialogClass()
        self.connectingBox = dialogClass(message=OTPLocalizer.CRAuthenticating)
        self.connectingBox.show()
        self.renderFrame()
        if self.loginScreen:
            self.loginScreen.handleConnectDone()

    @report(types=['args', 'deltaStamp'], dConfigParam='teleport')
    def __handleLoginDone(self, doneStatus):
        if self.connectingBox:
            self.connectingBox.cleanup()
            self.connectingBox = None
 ...
 'backArgs': []}):
    ...
    def enterFailedToConnect(self, statusCode, statusString):
        ...
        self.notify.warning('Failed to connect to the server (%s %s). Notifying user.' % (statusCode, statusString))
        if statusCode == 1403 or statusCode == 1405 or statusCode == 1400:
            message = OTPLocalizer.CRNoConnectionProxyNoPort % (url.getPort())
            style = OTPDialog.CancelOnly
        else:
            message = OTPLocalizer.CRNoConnection
        ...
        if self.loginScreen:
            self.loginScreen.handleFailedToConnect()
    ...
    def __handleFailedToConnectAck(self):
        ...
        if doneStatus == 'ok':
            ...
            self.loginFSM.request('loginOff')
        else:
    ...
    def __handleNoShardsAck(self):
        ...
        if doneStatus == 'ok':
            ...
            if self.loginScreen.fsm.getCurrentState().getName() == 'login':
                self.cleanupWaitingForDatabase()
                self.showBooted = False
    ...
    def enterNoConnection(self):
        ...
        if self.connectingBox:
            self.connectingBox.cleanup()
            self.connectingBox = None
        self.loginScreen.handleBooted(self.showBooted)
        self.showBooted = True
    ...
    def enterWaitForDeleteAvatarResponse(self, potAv):
        self.clientManager.sendDeleteAvatar(potAv.id)
    ...
    def uberZoneInterestComplete(self):
        ...
        if self.timeManager == None:
            ...
            h = HashVal()
            hashPrcVariables(h)
            pyc = HashVal() # is there even a point of this
    ...
    def getStartingDistrict(self):
        ...
        if base.fillShardsToIdealPop:
            for s in list(self.activeDistrictMap.values()):
                lowPop, midPop, highPop = s.districtPopLimits
    ...
    # Custom exceptionLogged for Pypperoni.
    def exceptionLogged(append=False):
        if __debug__:
            return PythonUtil.exceptionLogged(append)
        else:
            # This is the Pypperoni client.
            # Return the function itself.
            # This results in zero runtime overhead, since decorators are evaluated at module-load.
            def nullDecorator(f):
                return f
