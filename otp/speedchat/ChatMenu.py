...
from .SCObject import ChatObject
from direct.showbase.PythonUtil import makeTuple
from direct.showbase import DConfig
import types

class ChatMenu(ChatObject, NodePath):
    config = DConfig
    ...
    def __init__(self, holder = None):
        ...
        self.SerialNum = ChatMenu.SerialNum
        ChatMenu.SerialNum += 1
        node = hidden.attachNewNode('ChatMenu%s' % self.SerialNum)
        NodePath.__init__(self, node)
        self.setHolder(holder)
        self.FinalizeTaskName = 'ChatMenu%s_Finalize' % self.SerialNum
        self.ActiveMemberSwitchTaskName = 'ChatMenu%s_SwitchActiveMember' % self.SerialNum
    ...
    def appendFromStructure(self, structure):
        from .SpeedChatTypes import ChatMenuHolder, SCStaticTextTerminal, SCGMTextTerminal
        ...
        def addChildren(menu, childList):
            for child in childList:
                ...
                if type(child) == type(0):
                    ...
                    holder = ChatMenuHolder(holderTitle, menu=subMenu)
                    menu.append(holder)
    ...
    def enterVisible(self):
        ...
        if ChatMenu.WantFade:
            if alreadyFaded:
                self.fadeFunc(1.0)
            else:
                self.stopFade()
                self.fadeIval = LerpFunctionInterval(self.fadeFunc, fromData=0.0, toData=1.0, duration=ChatMenu.FadeDuration)
                self.fadeIval.play()
    ...
    def memberGainedInputFocus(self, member):
        ...
        if member is self.activeMember:
            return
        if self.activeMember is None or ChatMenu.SpeedChatRolloverTolerance == 0 or member.posInParentMenu < self.activeMember.posInParentMenu:
            self.__setActiveMember(member)
        else:
            ...
            minFrameRate = 1.0 / ChatMenu.SpeedChatRolloverTolerance
            if globalClock.getAverageFrameRate() > minFrameRate:
                taskMgr.doMethodLater(ChatMenu.SpeedChatRolloverTolerance, doActiveMemberSwitch, self.ActiveMemberSwitchTaskName)
    ...
    def privScheduleFinalize(self):
        ...
        taskMgr.remove(self.FinalizeTaskName)
        taskMgr.add(finalizeMenu, self.FinalizeTaskName, priority=ChatMenuFinalizePriority)
