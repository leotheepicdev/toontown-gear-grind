...
class ChatElement(SCObject, NodePath):
    font = OTPGlobals.getInterfaceFont()
    SerialNum = 0

    def __init__(self, parentMenu = None):
        SCObject.__init__(self)
        self.SerialNum = ChatElement.SerialNum
        ChatElement.SerialNum += 1
        node = hidden.attachNewNode('ChatElement%s' % self.SerialNum)
        NodePath.__init__(self, node)
        self.FinalizeTaskName = 'ChatElement%s_Finalize' % self.SerialNum
    ...
    def onMouseClick(self, cfType, event):
        pass
    ...
    def privScheduleFinalize(self):
        ...
        taskMgr.add(finalizeElement, self.FinalizeTaskName, priority=47)
    ...
    def finalize(self, dbArgs = {}):
        ...
        btn.bind(DGG.B1PRESS, self.onMouseClick, extraArgs=[CFSpeech])
        btn.bind(DGG.B3PRESS, self.onMouseClick, extraArgs=[CFThought])
        self.button = btn
