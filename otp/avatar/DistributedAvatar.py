...
from lib.libotp import Nametag
...
class DistributedAvatar(DistributedActor, Avatar):
    ...
    def takeDamage(self, hpLost, bonus = 0, wantRelocate=False):
    ...
    def getHpChangeName(self):
        return self.uniqueName('hpChange')
    ...
    def showStatusString(self, text, duration = 0.85, scale = 0.7):
        if self.HpTextEnabled and not self.ghostMode:
            if text != '':
                if self.statusText:
                    self.hideStatusText()
                ...
                self.HpTextGenerator.setTextColor(r, g, b, a)
                self.hpTextNode = self.HpTextGenerator.generate()
                self.statusText = self.attachNewNode(self.hpTextNode)
                self.statusText.setScale(scale)
                self.statusText.setBillboardAxis()
                self.statusText.setPos(0, 0, self.height / 2)
                seq = Sequence(self.statusText.posInterval(1.0, Point3(0, 0, self.height + 1.18), blendType='easeOut'), Wait(0.85), self.hpText.colorScaleInterval(0.1, Vec4(r, g, b, 0)), Func(self.hideStatusText))
                seq.start()
