...
from lib.libotp import Nametag, WhisperPopup
...
from panda3d.core import *
from . import Avatar
from otp.avatar import ControlManager
from otp.avatar.GravityWalker import GravityWalker
...
class LocalAvatar(DistributedAvatar.DistributedAvatar, DistributedSmoothNode.DistributedSmoothNode):
    ...
    def startSprinting(self):
        if self.isSprinting or not self.avatarControlsEnabled:
            return
        self.lerpCameraFov(70, 0.3)
        self.isSprinting = 1

    def stopSprinting(self):
        if self.isSprinting:
            self.lerpCameraFov(self.fov, 0.3)
            self.isSprinting = 0
